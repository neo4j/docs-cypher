...
[.description]
Show all databases in Neo4j DBMS and return default columns (`name`, `type`, `aliases`, `access`, `address`, `role`, `writer`, `requestedStatus`, `currentStatus`, `statusMessage`, `default`, `home`, and `constituents`).
...
[.description]
Drop the database `topology-example` and any database aliases referencing the database.
