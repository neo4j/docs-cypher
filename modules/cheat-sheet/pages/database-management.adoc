...
====
[source, cypher, role=noheader, source=test-skip]
----
dba
`db1`
`database-name`
...
[source, cypher, role=noheader]
----
ALTER DATABASE `database-name` SET DEFAULT LANGUAGE CYPHER 25
----
...
[.description]
Delete the database `actors`, if it exists. This command can delete both standard and composite databases.
====
