...
[NOTE]
====
For information on upgrading and migrating Neo4j, see link:/https://neo4j.com/docs/upgrade-migration-guide/current/[Neo4j Upgrade and Migration Guide]. +
...
+
You can find more details in the xref:syntax/expressions.adoc#label-expressions[section on Label expressions] and in the section on xref:clauses/where.adoc[`WHERE` clause].
...
These two privileges are now hierarchically related. +
...
+ 
Refer to this section to learn more about xref:indexes-for-search-performance.adoc[new index types]. +
