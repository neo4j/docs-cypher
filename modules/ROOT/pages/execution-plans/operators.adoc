...
The example looks for the presence of a _works in_ relationship with the supplied duration and title, for a specific person and location.
If part of the pattern can't be found, then it will be created.
Owing to the existence of two property uniqueness constraints on `:WORKS_IN(duration)` and `:WORKS_IN(title)`, any relationship that would be found by the `DirectedRelationshipUniqueIndexSeek` operator must be the very same relationship or the constraints would be violated.
