...
[source, cypher, role=test-setup]
----
CREATE CONSTRAINT constraint_Team_name IF NOT EXISTS
FOR (t:Team)
...
CREATE RANGE INDEX index_range_node_location
FOR (l:Location) ON (l.name)
----
////
...
| |               +----+-----------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     |
| +NodeIndexSeek  |  5 | RANGE INDEX matt:Person(name) WHERE name = $autostring_1        |              1 |    1 |       2 |            120 |                    2/0 |     0.284 | Fused in Pipeline 0 |
+-----------------+----+-----------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+
...
.Distinct
======
// need to check with Cyphet Team
.Query
[source, cypher, role="noplay"]
----
PROFILE
