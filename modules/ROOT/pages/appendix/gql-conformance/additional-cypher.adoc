...
| xref:functions/list.adoc#functions-coll-distinct[`coll.distinct()`]
| Returns the given list with all duplicate values removed.

| xref:functions/list.adoc#functions-coll-flatten[`coll.flatten()`]
| Returns a list flattened to the given nesting depth.

| xref:functions/list.adoc#functions-coll-indexOf[`coll.indexOf()`]
| Returns the index of the first match of a value in the given list or -1 if the value is not present.

| xref:functions/list.adoc#functions-coll-insert[`coll.insert()`]
| Returns a list with the given value inserted at the given index.

| xref:functions/list.adoc#functions-coll-max[`coll.max()`]
| Returns the largest value present in the given list.

| xref:functions/list.adoc#functions-coll-min[`coll.min()`]
| Returns the smallest value present in the given list.

| xref:functions/list.adoc#functions-coll-remove[`coll.remove()`]
| Returns a list with the value at the given index removed.

| xref:functions/list.adoc#functions-coll-sort[`coll.sort()`]
| Returns a sorted list.

| xref:functions/list.adoc#functions-coll-zip[`coll.zip()`]
...
| xref:planning-and-tuning/execution-plans.adoc#reading-explain-and-profile-query-plans[`EXPLAIN`/`PROFILE`]
