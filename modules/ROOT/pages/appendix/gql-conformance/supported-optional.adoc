...
| xref:patterns/unique-relationship-patterns.adoc[`DIFFERENT RELATIONSHIPS`]
| The semantic for this feature is the default Cypher semantic.

| G003
| Explicit `REPEATABLE ELEMENTS` keyword
| xref:patterns/repeatable-relationship-and-node-patterns.adoc[`REPEATABLE ELEMENTS`]
|
...
| xref:patterns/acyclic-patterns.adoc[Acyclic patterns]
|
