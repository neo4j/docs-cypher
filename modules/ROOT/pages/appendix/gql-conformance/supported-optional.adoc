...
| xref:patterns/fixed-length-paths.adoc#relationship-patterns[Relationship patterns]
|  Cypher supports GQL's directed forms (e.g. `-\->`) but not its tilde forms (e.g. `~>`.)
...
| xref:syntax/comments.adoc#identifier-length-limit[Comments]
|
...
| xref:clauses/order-by.adoc#aggregation-distinct[`ORDER BY`:  Ordering aggregated or DISTINCT results]
|
...
| GL04
| Exact number in common notation
| xref:values-and-types/boolean-numeric-string.adoc#numerical[Boolean, numeric, and string literals: Numeric literals]
| Unlike GQL, Cypher does not support trailing decimals (e.g. `3.` is not supported in Cypher, but `3.0` is).
...
| GP02
| Inline procedure: implicit scope
|  xref:subqueries/call-subquery.adoc[`CALL` subqueries]
|
...
* GQL defines the literal `DATE`. Cypher instead uses the functions/temporal/index.adoc#functions-date[`date()`] function.
