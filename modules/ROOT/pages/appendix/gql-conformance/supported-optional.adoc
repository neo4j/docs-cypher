...
| xref:patterns/unique-relationship-matching.adoc[`DIFFERENT RELATIONSHIPS`]
| The semantic for this feature is the default Cypher semantic.

| G003
| Explicit `REPEATABLE ELEMENTS` keyword
| xref:patterns/repeatable-relationship-and-node-matching.adoc[`REPEATABLE ELEMENTS`]
|
