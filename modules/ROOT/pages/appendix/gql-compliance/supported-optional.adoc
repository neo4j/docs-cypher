:description: Information about which optional GQL features Cypher supports.
...
[NOTE]
Cypher and GQL format functions differently and, as a result, several Cypher functions offer the same (or very similar) functionality to their GQL counterpart, yet they are not conformant with GQL.
