...
The function will only return the name of the database if the user has either:

* access to the database

* the database is a standard database and the user has `DATATABASE MANAGEMENT`, `CREATE DATABASE`, `DROP DATABASE`, `ALTER DATABASE` or `SET DATABASE ACCESS` privileges

* the database is a composite database and the user has `COMPOSITE DATATABASE MANAGEMENT`, `CREATE COMPOSITE DATABASE`, `DROP COMPOSITE DATABASE` or `ALTER COMPOSITE DATABASE` privileges
