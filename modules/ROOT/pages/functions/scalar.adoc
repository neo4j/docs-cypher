...
[NOTE]
To calculate the length of a `PATH` as a relationship count only, see xref:functions/scalar.adoc#functions-length[`length()`] or xref:functions/scalar.adoc#functions-path-length[`path_length()`].
