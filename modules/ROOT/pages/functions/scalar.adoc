...
.2+| *Arguments* | *Name* | *Type* | *Description*
| `input` | `LIST<ANY> \| MAP \| PATH` | A list, path or map whose cardinality is to be calculated.
| *Returns* 3+| `INTEGER`
|===
...
[NOTE]
To calculate the length of a `PATH` in terms of traversed relationships only, see xref:functions/scalar.adoc#functions-length[`length()`] or xref:functions/scalar.adoc#functions-path-length[`path_length()`].
...
The cardinality and length of the path `p` are returned.
