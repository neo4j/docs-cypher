...
Listing what functions are available to a particular user requires the link:{neo4j-docs-base-uri}/operations-manual/current/authentication-authorization/dbms-administration/dbms-user-management-privileges/[`SHOW USER`] privilege.
...
For a full list of all available functions in Cypher, see the chapter on xref::functions/index.adoc[Functions].

[[list-functions-filter-on-output-columns]]
...
.Show functions with filtered results
[source, cypher]
----
...
[[list-functions-other-filtering]]
...
[[result-columns]]
== Result columns for listing functions

The below table contains the full information about all columns returned by the `SHOW FUNCTIONS YIELD *` command:
