...
[NOTE]
====
Neo4j maintains a transactional count store for holding count metadata, which significantly increases the speed of queries using the `count()` function.
More information about the count store can be read in this link:https://neo4j.com/developer/kb/fast-counts-using-the-count-store/[Neo4j Knowledge Base article].
====
