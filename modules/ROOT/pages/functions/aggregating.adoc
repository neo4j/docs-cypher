...
| Any `null` values are excluded from the calculation.
| As of Neo4j 2026.04, `stDev(null)` returns `null`.
In earlier versions, it returns `0`.
...
| Any `null` values are excluded from the calculation.
| As of Neo4j 2026.04, `stdev_samp(null)` returns `null`.
In earlier versions, it returns `0`.
...
| Any `null` values are excluded from the calculation.
| As of Neo4j 2026.04, `stDevP(null)` returns `null`.
In earlier versions, it returns `0`.
...
| Any `null` values are excluded from the calculation.
| As of Neo4j 2026.04, `stdev_pop(null)` returns `null`.
In earlier versions, it returns `0`.
...
Grouping keys are implicit in Cypher unless you control them explicitly with the xref:clauses/group-by.adoc[`GROUP BY` clause].
...
Cypher only recognizes a sub-expression in aggregating expressions as a grouping key if the grouping key is:
...
If you need more complex grouping keys as operands in aggregating expressions, you can project them with `AS` in `RETURN` and `WITH`.
