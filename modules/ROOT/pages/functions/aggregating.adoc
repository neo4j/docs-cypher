...
// TODO: adjust the next paragraph
[[grouping-keys]]
...
The rules:
* A projection item alias always shadows any incoming variable of the same name within the scope of a subclause expression.

* A non-aggregating subclause expression may reference a grouping key by its alias, an exact match of the full expression or by its expression if it is a recognizable grouping key expression.
The set of recognizable grouping key expressions is defined as: Variable, element property access on a variable, and static map access on a variable.

* An aggregating subclause expression can be any expression that follows the same rules as an aggregating projection item expression.