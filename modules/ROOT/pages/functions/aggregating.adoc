...
The function `avg()` returns the average of a set of `INTEGER`, `FLOAT`, or `DURATION` values.
...
| Either an `INTEGER`, `FLOAT`, or `DURATION` depending on the values returned by `input` and whether or not the calculation overflows.
...
`count(input)`:: returns the number of non-`null` values returned by an expression.
...
| A xref::values-and-types/property-structural-constructed.adoc#property-types[property type], or a list, depending on the values returned by `input`.
...
| An `INTEGER`, `FLOAT`, or `DURATION`, depending on the values returned by `input`.
