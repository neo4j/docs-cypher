...
For aggregating subclause expressions, the same rules apply as for aggregating expressions in `RETURN` and `WITH`.
...
Cypher only recognizes a sub-expression in aggregating expressions as a grouping key if the grouping key is either:
...
These are called _recognizable grouping key expressions_.

If more complex grouping keys are needed as operands in aggregating expression, it is always possible to project them with `AS` in `RETURN` and `WITH`.
...
|===
| link:https://neo4j.com/docs/status-codes/current/errors/gql-errors/42I18/[42I18]: error: syntax error or access rule violation - reference to non-grouping sub-expression. The expression contains a non-grouping sub-expression `p`. In an aggregating context only grouping sub-expressions and constants are allowed.
...
=== Visibility of expressions in WHERE and ORDER BY

The visibility of expressions in `WHERE` and `ORDER BY` has been streamlined:
