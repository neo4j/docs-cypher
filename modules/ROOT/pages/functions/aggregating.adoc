...
| Any `null` values are excluded from the calculation.
| As of Neo4j 2026.05, `stDev(null)` returns `null`.
...
| Any `null` values are excluded from the calculation.
| As of Neo4j 2026.05, `stdev_samp(null)` returns `null`.
In earlier versions, it returns `0`.
...
| Any `null` values are excluded from the calculation.
| As of Neo4j 2026.05, `stDevP(null)` returns `null`.
In earlier versions, it returns `0`.
...
| Any `null` values are excluded from the calculation.
| As of Neo4j 2026.05, `stdev_pop(null)` returns `null`.
In earlier versions, it returns `0`.
