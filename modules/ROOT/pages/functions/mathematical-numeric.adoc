...
`round()` returns the value of the given number rounded with the specified precision, with ties always being rounded away from zero (rounding mode `HALF_UP`).
...
| `HALF_DOWN`
| Round towards closest value of given precision, with ties always being rounded down (towards zero).

| `HALF_EVEN`
| Round towards closest value of given precision, with ties always being rounded to the even neighbor.

| `HALF_UP`
| Round towards closest value of given precision, with ties always being rounded up (away from zero).
