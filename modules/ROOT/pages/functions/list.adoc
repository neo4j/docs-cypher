...
.Details
|===
| *Syntax* 3+| `coll.zip(list1, list2, ...)`
| *Description* 3+| Returns a list of lists, where the i-th list contains the i-th element from each of the given lists.
.4+| *Arguments* | *Name* | *Type* | *Description*
| `list1` | `LIST<ANY>` | The first list to zip.
| `list2` | `LIST<ANY>` | The second list to zip.
| `...` | `LIST<ANY>` | Any additional lists to zip.
| *Returns* 3+| `LIST<ANY>`
|===

.Considerations
|===
| `coll.zip()` requires a minimum of two lists.
| If any of the given lists is `null`, `null` is returned.
| If the given lists are of different lengths, the missing values in the shorter lists are padded with `null` so that every inner list has the same length as the longest given list.
...
// tag::functions_list_zip[]
[source, cypher]
----
WITH [1, 2, 3] AS list1, ['a', 'b', 'c'] AS list2
RETURN coll.zip(list1, list2)
----
// end::functions_list_zip[]

A `LIST<ANY>` containing a list for each pair of elements at the same index in the given lists is returned.

.Result
[role="queryresult",options="header,footer",cols="1*<m"]
|===

| coll.zip(list1, list2)
| [[1, 'a'], [2, 'b'], [3, 'c']]
1+d|Rows: 1

|===

.Zip more than two lists together
[source, cypher]
----
WITH [1, 2] AS list1, ['a', 'b'] AS list2, [true, false] AS list3
RETURN coll.zip(list1, list2, list3)
----
...
[source, cypher]
----
WITH [1, 2, 3, 4] AS list1, ['a', 'b', 'c'] AS list2
RETURN coll.zip(list1, list2)
----

If the given lists are of different lengths, the missing values are padded with `null`.
...
.Zip with a null list
[source, cypher]
----
WITH [1, 2, 3] AS list1, null AS list2
RETURN coll.zip(list1, list2)
----
...
| *Description* 3+| Converts a `LIST<ANY>` to a `LIST<STRING>` values.
.2+| *Arguments* | *Name* | *Type* | *Description*
...
| noList | nullsInList | mixedList
|  NULL   | ["null", "null"] | ["already a string", "2", "1955-11-05", "null", "[A, B]"]
3+d|Rows: 1
