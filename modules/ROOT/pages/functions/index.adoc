...
| The `STRING` to convert to a `UUID`, this should be 32 hexadecimal digits displayed in 5 groups, separated by 4 hyphens. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.08]

1.1+| xref::functions/uuid.adoc#functions-uuid[`uuid()`]
| `uuid(mostSigBits :: INTEGER, leastSigBits :: INTEGER) :: UUID`
| Converts the given `INTEGER` values to a `UUID`. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.08]
