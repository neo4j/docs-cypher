...
|===
| Function | Signature | Description
1.1+| xref:functions/security-functions.adoc#functions-security-oidc-user-attribute[`abac.oidc.user_attribute()`]
| `abac.oidc.user_attribute(claimKey :: STRING) :: ANY`
| Returns the value of the specified user attribute from the OIDC claims.  label:new[Introduced in Neo4j 2026.03]

1.1+| xref:functions/security-functions.adoc#functions-security-native-user-tags[`abac.native.user_tags()`]
| `abac.native.user_tags() :: LIST<STRING>`
| Returns the list of tags for the native user. label:new[Introduced in Neo4j 2026.06]
|===
