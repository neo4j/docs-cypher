...
|===
| Function | Signature | Description
1.1+| xref:genai-integrations.adoc#single-embedding[`genai.vector.encode()`]  | `genai.vector.encode(resource :: STRING, provider :: STRING, configuration :: MAP = {}) :: LIST<FLOAT>` | Returns a list of floats that represents the passed resource as a vector.
