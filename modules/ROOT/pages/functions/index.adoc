...
For more information about this command, see xref::functions/show-functions.adoc[].
...
label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2025.11]
...
1.1+| xref::functions/list.adoc#functions-coll-zip[`coll.zip()`]
| `coll.zip(list1 :: LIST<ANY>, list2 :: LIST<ANY>, ... :: LIST<ANY>) :: LIST<ANY>`
| Returns a list of lists, where the i-th list contains the i-th element from each of the given lists.
label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.10]
...
| Returns the number of keys in a `MAP`, the number of elements in a `LIST<ANY>`, or the number of nodes and relationships in a `PATH`. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.07]
...
[[header-query-functions-security]]
== Security functions
...
These functions cannot be called directly in queries; they can only be used in the condition of a `CREATE AUTH RULE` or `ALTER AUTH RULE` command.

[options="header"]
|===
| Function | Signature | Description

1.1+| xref:functions/security.adoc#functions-security-native-user-tags[`abac.native.user_tags()`]
| `abac.native.user_tags() :: LIST<STRING>`
| Returns the list of tags for the native user. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.06]

1.1+| xref:functions/security.adoc#functions-security-oidc-user-attribute[`abac.oidc.user_attribute()`]
| `abac.oidc.user_attribute(claimKey :: STRING) :: ANY`
| Returns the value of the specified user attribute from the OIDC claims. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.03]
...
1.1+| xref::functions/string.adoc#functions-string.indexof[`string.indexOf()`]
| `string.indexOf(input :: STRING, value :: STRING) :: INTEGER`
...
1.1+| xref::functions/string.adoc#functions-string.join[`string.join()`]
| `string.join(input :: STRING, delimiter : STRING) :: STRING`
| Concatenates the elements in `input`, inserting `delimiter` between each element, and returns the resulting `STRING`.
label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.05]

1.1+| xref::functions/string.adoc#functions-string.regexreplace[`string.regexReplace()`]
| `string.regexReplace(original :: STRING, regex :: STRING, replacement :: STRING) :: STRING`
...
| Converts a value to a `STRING`, or `null` if the value cannot be converted.
...
UUID functions allow you to create xref:values-and-types/uuid.adoc[`UUID` values] and return their most/least significant types.
...
| `uuid() :: UUID`
| Returns a randomly generated `UUID`. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.08]

1.1+| xref::functions/uuid.adoc#functions-uuid[`uuid()`]
| `uuid(name :: STRING) :: UUID`
| The `STRING` to convert to a `UUID`.
Converts a `STRING` value to a `UUID`. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.08]

1.1+| xref::functions/uuid.adoc#functions-uuid[`uuid()`]
| `uuid(mostSigBits :: INTEGER, leastSigBits :: INTEGER) :: UUID`
| Converts two `INTEGER` values to a `UUID`. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.08]

1.1+| xref::functions/uuid.adoc#functions-uuid-leastsignificantbits[`uuid.leastSignificantBits()`]
| `leastSignificantBits(uuid :: UUID) :: INTEGER`
| Returns the least significant bits from the given `UUID`. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.08]

1.1+| xref::functions/uuid.adoc#functions-uuid-mostsignificantbits[`uuid.mostSignificantBits()`]
| `mostSignificantBits(uuid :: UUID) :: INTEGER`
| Returns the most significant bits from the given `UUID`. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.08]
...
| Constructs a `VECTOR` value. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2025.10]
...
| Returns the dimension of a `VECTOR`. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2025.10]
...
| Returns a `FLOAT` representing the distance between the two vector values based on the selected `vectorDistanceMetric` algorithm. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2025.10]
...
| Returns a `FLOAT` representing the distance between the given vector and a vector of the same dimension with all coordinates set to zero, calculated using the specified `vectorDistanceMetric`. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2025.10]
