...
| `cardinality(input :: LIST<ANY> \| MAP \| PATH) :: INTEGER`
| Returns the number of keys in a `MAP`, the number of elements in a `LIST<ANY>`, or the number of nodes and relationships in a `PATH`.
