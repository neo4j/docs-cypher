...
[[functions-string.indexof]]
== string.indexOf()

.Details
|===
| *Syntax* 3+| `string.indexOf(input, value)`
| *Description* 3+| Returns the index of the first occurrence of the given `value` within the `input` `STRING`, or -1 if no match is found.
.3+| *Arguments* | *Name* | *Type* | *Description*
...
| `string.indexOf(null, value)` returns `null`.
| `string.indexOf(input, null)` returns `null`.
| If `value` is not present in `input`, `-1` is returned.
...
.Query
// tag::functions_string_indexof[]
[source, cypher, indent=0]
----
RETURN string.indexOf('hello', 'l')
----
// end::functions_string_indexof[]
...
.Details
|===
| *Syntax* 3+| `string.join(input, delimiter)`
| *Description* 3+| Concatenates the elements in `input`, inserting `delimiter` between each element, and returns the resulting `STRING`.
.3+| *Arguments* | *Name* | *Type* | *Description*
| `input` | `LIST<STRING>` | A list of strings to be joined.
| `delimiter` | `STRING` | The joining delimiter.
| *Returns* 3+| `STRING`
|===
...
| `string.join(null, delimiter)` returns `null`.
| `string.join(input, null)` returns `null`.
...
.Query
// tag::functions_string_join[]
[source, cypher, indent=0]
----
RETURN string.join(['one', 'two'], ', ')
----
// end::functions_string_join[]
...
[role=label--new-Neo4j-2026.05 label--cypher-25-only]
[[functions-string.regexreplace]]
== string.regexReplace()

.Details
|===
| *Syntax* 3+| `string.regexReplace(original, regex, replacement)`
| *Description* 3+| Returns a `STRING` where all matches of `regex` in the `original` `STRING` are replaced with the `replacement` `STRING`.
.4+| *Arguments* | *Name* | *Type* | *Description*
| `original` | `STRING` | The string to be modified.
| `regex` | `STRING` | The regex pattern to replace in the original `STRING`.
| `replacement` | `STRING` | The value to be inserted in the original string.
| *Returns* 3+| `STRING`
|===

[Note]
For more information on how the regular expression pattern works, see xref:expressions/predicates/string-operators.adoc#regular-expressions[Regular Expressions].
...
.Query
// tag::functions_string_regexreplace[]
[source, cypher, indent=0]
----
RETURN string.regexReplace("hello", "l.", "w")
----
// end::functions_string_regexreplace[]

.Result
[role="queryresult",options="header,footer",cols="1*<m"]
|===

| string.regexReplace("hello", "l.", "w")
| "hewo"
1+d|Rows: 1
...
| *Description* 3+| Converts a value to a `STRING`.
.2+| *Arguments* | *Name* | *Type* | *Description*
...
a| The following stringification rules apply:

* A `LIST` is all its types recursively stringified, surrounded by square brackets `[]` and comma-separated, `null` values will be stringified inside the list. For example: `['a', 1, true, null]` becomes `"[a, 1, true, null]"`. An empty `LIST` becomes `"[]"`.
* A `MAP` has its keys sorted alphabetically, and all values stringified, including `null` values. For example: `{a: 'hi', b: true, d: 1, c: 1.5, e: null}` becomes `"{a: hi, b: true, c: 1.5, d: 1, e: null}"`. An empty `MAP` becomes `"{}"`.
* A `NODE` is represented by parentheses around its labels sorted alphabetically and colon separated. For example: `(:LabelB:LabelA)` becomes `"(:LabelA:LabelB)"`.
* A `RELATIONSHIP` is represented by square brackets surrounding its type. For example: `[:TYPE]` becomes `"[:TYPE]"`.
* A `PATH` is a combination of the above, using Cypher's ascii art arrow syntax. For example: `"(:LabelA)-[:TYPE]->(:LabelB)"`.
| Prior to Neo4j 2026.09, this function did not accept `LIST`, `MAP`, `NODE`, `RELATIONSHIP`, and `PATH` types.
...
RETURN
  ...
  toString([1, 'a', true]) AS listString,
  toString({a: 1, b: 'hi'}) AS mapString,
  toString(null) AS nullValue
----
...
| toString(11.5) | toString('already a string') | toString(true) | dateString | datetimeString | durationString | listString | mapString | nullValue
| "11.5" | "already a string" | "true" | "1984-10-11" | "1984-10-11T12:31:14.341+01:00[Europe/Stockholm]" | "PT11M" | "[1, a, true]" | "{a: 1, b: hi}" | null
9+d|Rows: 1
...
| *Description* 3+| Converts a value to a `STRING`, or `null` if the value cannot be converted.
.2+| *Arguments* | *Name* | *Type* | *Description*
...
toStringOrNull([1, 'a', true]) AS list,
toStringOrNull({a: 1, b: 'hi'}) AS map,
toStringOrNull(null) AS nullValue
----
...
| toStringOrNull(11.5) | toStringOrNull('already a string') | toStringOrNull(true) | dateString | datetimeString | durationString | list | map | nullValue
| "11.5" | "already a string" | "true" | "1984-10-11" | "1984-10-11T12:31:14.341+01:00[Europe/Stockholm]" | "PT11M" | "[1, a, true]" | "{a: 1, b: hi}" | null
9+d|Rows: 1
