...
| As of Neo4j 2026.09, any `null` element in `input` is converted to the `STRING` `"null"`
In earlier versions, any `null` element in `input` is preserved without conversion.
| If `input` is a `STRING`, it will be returned unchanged.
a| The following stringification rules apply:

* A `LIST` is all its types recursively stringified, surrounded by square brackets `[]` and comma-separated. For example: `['a', 1, true]` becomes `"[a, 1, true]"`. An empty `LIST` becomes `"[]"`.
* A `MAP` has its keys sorted alphabetically, and all values stringified. For example: `{a: 'hi', b: true, d: 1, c: 1.5}` becomes `"{a: hi, b: true, c: 1.5, d: 1}"`. An empty `MAP` becomes `"{}"`.
...
| toString(11.5) | toString('already a string') | toString(true) | dateString | datetimeString | durationString | listString | mapString | nullString
