:description: Cypher provides functions allowing for the creation and manipulation of values for each temporal type -- _Date_, _Time_, _LocalTime_, _DateTime_, and _LocalDateTime_.
...
| `timezone`
| A string expression that represents the xref::syntax/temporal.adoc#cypher-temporal-specify-time-zone[time zone].
...
| `year`
| An expression consisting of at xref::syntax/temporal.adoc#cypher-temporal-year[least four digits] that specifies the year.
