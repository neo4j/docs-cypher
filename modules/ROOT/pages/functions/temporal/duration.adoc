:description: Cypher provides functions allowing for the creation and manipulation of values for a `DURATION` temporal type.
...
* xref::functions/temporal/duration.adoc#functions-duration[Creating a `DURATION` from duration components]
* xref::functions/temporal/duration.adoc#functions-duration-create-string[Creating a `DURATION` from a string]
* xref::functions/temporal/duration.adoc#functions-duration-computing[Computing the `DURATION` between two temporal instants]

Information regarding specifying and accessing components of a `DURATION` value can be found xref::values-and-types/temporal.adoc#cypher-temporal-durations[here].
