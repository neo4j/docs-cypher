...
| `graph.names()` only returns the name of a graph reference if the user has access to the database.
|===
...
| `graph.propertiesByName()` only returns the name of a graph reference if the user has access to the database.
|===
...
| `graph.byName()` only returns a graph reference if the user has access to it.
|===
...
| `graph.byElementId()` only returns a graph reference if the user has access to it.
|===
