...
| `graph.names()` only returns the name of a graph reference if the user has access to it.
|===
...
| `graph.propertiesByName()` only returns the properties of a graph reference if the user has access to it.
|===
