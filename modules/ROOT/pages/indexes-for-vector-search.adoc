...
Vector indexes are powered by the link:https://lucene.apache.org/[Apache Lucene] indexing and search library, and can be used to index nodes by `LIST<FLOAT>` properties valid to the index.
A vector index allows you to write queries that match a _neighborhood_ of nodes based on the similarity of those node properties to the query.
...
A common semantic of a vector is as an embedding.
This is a numerical representation of a resource such as a text, image, audio, or document.
Embedding generators such as https://cloud.google.com/vertex-ai[Vertex AI] or https://openai.com/[OpenAI] can produce 768, 1024, 1536, etc., dimensional vector embeddings.
...
This model suggests a xref:indexes-vector-similarity-cosine[cosine similarity] (for more information see OpenAI's documentation on https://platform.openai.com/docs/guides/embeddings/which-distance-function-should-i-use[Which distance function I use?]).
