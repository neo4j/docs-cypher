...
Storing `VECTOR` type values as properties is only supported in the Neo4j Enterprise Edition or on Aura instances. This functionality is not available in Neo4j Community Edition.
...
** `BOOLEAN`
** `STRING`
** `INTEGER8`* label:new[Introduced in Neo4j 2025.xx]
** `INTEGER16`* label:new[Introduced in Neo4j 2025.xx]
** `INTEGER32`* label:new[Introduced in Neo4j 2025.xx]
** `INTEGER` (equivalent to `INTEGER64`)
** `FLOAT32`* label:new[Introduced in Neo4j 2025.xx]
** `FLOAT` (equivalent to `FLOAT64`)
** `DATE`
** `LOCAL TIME`
** `ZONED TIME`
** `LOCAL DATETIME`
...
** `NODE`
** `RELATIONSHIP`
** `VECTOR` (ordered by coordinate type and dimension) label:new[Introduced in Neo4j 2025.xx]
