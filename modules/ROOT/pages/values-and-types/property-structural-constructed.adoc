...
These types (and the synonyms) can be used in the type predicate expressions and the property type constraints.
However, not all types can be used in all places.
// TODO: add link to type predicate expression section (once exists)
// TODO: add link to property type constraint section (once exists)
Some of these types are not yet supported in either type predicate expressions nor property type constraints.
