...
[TIP]
You can convert `UUID` values into `STRING` values with the xref:functions/string.adoc#functions-tostring[`toString()`] function.
...
.Create a node with a `UUID` property using parameters
====
