...
`VECTOR` values can be stored as xref:indexes/semantic-indexes/vector-indexes.adoc#embeddings[embedding] properties on nodes and relationships, and used for efficient semantic retrieval using xref:indexes/semantic-indexes/vector-indexes.adoc[vector indexes] and the link:https://neo4j.com/docs/genai/plugin/5/[GenAI plugin].
...
Also note that storing `VECTOR` properties requires using link:https://neo4j.com/docs/operations-manual/current/database-internals/store-formats/#store-format-overview[block format], which is the default store format for Neo4j Enterprise Edition and Aura databases.
Neo4j Community Edition cannot use block format and therefore cannot store `VECTOR` values as properties.
If you are using Community Edition, store embeddings as `LIST <INTEGER | FLOAT>` properties and index them with vector indexes.
====
...
* link:{neo4j-docs-base-uri}/cypher-manual/current/values-and-types/ordering-equality-comparison/#ordering-vector-types[Cypher 25 Manual -> Ordering vector types]
* link:{neo4j-docs-base-uri}/cypher-manual/current/values-and-types/property-structural-constructed/#vector-type-normalization[Cypher 25 Manual -> Vector type normalization]

Also note that Cypher 25 is required to use `VECTOR` supertypes in link:{neo4j-docs-base-uri}/cypher-manual/current/expressions/predicates/type-predicate-expressions/#type-predicate-vector[type predicate expressions].
...
For information about how to store embeddings as `VECTOR` values with the GenAI plugin, see link:https://neo4j.com/docs/genai/plugin/5/embeddings/[Create and store embeddings in a Neo4j database].
