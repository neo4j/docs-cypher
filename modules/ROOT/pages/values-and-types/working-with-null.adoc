...
All data types in Cypher are nullable, which leads to the xref::syntax/expressions.adoc#type-predicate-expressions-null[type predicate expression] always  returning `true` for `null` values.
