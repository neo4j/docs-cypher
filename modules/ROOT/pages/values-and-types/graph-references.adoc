...
In Cypher, graph references are values used to specify which graph to operate on in a query.
...
The following rules apply for string expressions for graph references when using identifiers (static graph references, administration commands) or a string (`graph.byName function`):
...
For string literals, both the escaping rules for xref:values-and-types/numeric-string-boolean.adoc#string-literal-escape-sequences[string literals] (during query parsing) and xref::syntax/naming.adoc#symbolic-names-escaping-rules[symbolic names] (during graph reference evaluation) are applied.
