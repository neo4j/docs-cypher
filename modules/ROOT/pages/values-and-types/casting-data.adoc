...
| `toStringList()` | Converts a `LIST<ANY>` and returns a `LIST<STRING>`.

| `toStringOrNull()` | Converts a value to a `STRING`, or `null` if the value cannot be converted.
|===
...
image::values-and-types-converting-data-graph.svg["Example graph of two Person nodes, Keanu Reeves and Carrie-Anne Moss, connected by a KNOWS relationship with a since property",width=400,role=popup]
