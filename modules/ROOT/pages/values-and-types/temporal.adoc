...
Starting from Neo4j 5.9, some temporal types have been renamed.
The table below shows the current as well as the old names of the temporal types.
...
`d.quarters` and `d.months` add 4 quarters and 12 months respectively of the year from the query.
`d.quartersOfYear` is the remaining quarter, counting towrads the next full year. Similarly, `d.monthsOfYear` and `d.monthsOfQuarter` count towards the next full year and quarter respectively.
See tables _Most significant components_ and _Less significant components_ in <<cypher-temporal-accessing-components-durations>>.
...
See tables _Most significant components_ and _Less significant components in <<cypher-temporal-accessing-components-durations>>.

======


.Get the most significant second-based components of a `DURATION` value
======
...
`d.minutes` has a value of 61 because `duration.hours` and `duration.minutes` are both second-based components.
It's the sum of the 60 minutes of the hour and the minute from the query.
Similarly, `d.seconds`, `d.milliseconds`, `d.microseconds` and `d.nanoseconds` are sum values of the relevant second-based components from the query.

`d.hours` doesn't take the day from the query into account because `duration.days` is a day-based component.

See table _Most significant components_ in <<cypher-temporal-accessing-components-durations>>.

======


.Get the less significant second-based components of a `DURATION` value
======
...
For example, `d.microsecondsOfSecond` has a value of 111111 because it's the 111111111 nanoseconds from the query in microseconds (rounded down) but it's not another full second.

See table _Less significant components_ in <<cypher-temporal-accessing-components-durations>>.
