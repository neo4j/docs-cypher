...
.Get the most significant seconds-based components of a `DURATION` value
======
...
`d.hours` doesn't take the day from the query into account because `duration.days` is a days-based component.
...
.Get the less significant seconds-based components of a `DURATION` value
======
