...
For example, `()-[:A]\->()` is be ordered before `()-[:B]\->()`.

If one path is a prefix of another, the shorter path is considered smaller (i.e.  it is padded with a special empty value, which is considered less than any other value (including `null`)).
