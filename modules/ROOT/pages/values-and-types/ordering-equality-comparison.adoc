...
`UUID` values support equality (`=`) and inequality (`<>`) comparisons.
However, the ordering operators `<` and `>` are not defined for `UUID` values and always return `null`.
The operators `<=` and `>=` return `true` if the two `UUID` values are equal, and `null` otherwise.
...
* xref::patterns/fixed-length-paths.adoc#path-patterns[`PATH`]
...
* xref::values-and-types/uuid.adoc[`UUID`]
...
`PATH` values are compared element-by-element (nodes and relationships alternating); the first differing element determines the order.
For example, `()-[:A]\->()` is ordered before `()-[:B]\->()`.

If one path is a prefix of another, the shorter path is considered smaller.
For example, `()-[:REL]\->()` is ordered before `()-[:REL]\->()-[:REL]\->()`.
...
[[ordering-uuid-values]]
== Ordering UUID values

`UUID` values are ordered by comparing their most significant bits first (the first 64 bits of the 128-bit value), then their least significant bits (the last 64 bits), treating each as an unsigned `INTEGER` (per RFC 9562). 
For example, `uuid('00000000-0000-0000-0000-000000000001')` is ordered before `uuid('00000000-0001-0000-0000-000000000000')`, because the most significant bits differ (`0000...0000` versus `0000...0001`) and that comparison is decisive regardless of the least significant bits.

[[ordering-vector-types]]
=== Ordering `VECTOR` types

The xref:values-and-types/vector.adoc[`VECTOR`] supertypes are ordered in the following ascending order:
