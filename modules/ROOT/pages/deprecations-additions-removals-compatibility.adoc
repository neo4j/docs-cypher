...
Two new performance-improving operators: xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-merge-into[`MergeInto`] and xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-merge-unique-node[`MergeUniqueNode`].
a| Both are similar to the `Merge` operator, but:
