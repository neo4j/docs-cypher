...
[[cypher-deprecations-additions-removals-2026.10]]
== Neo4j 2026.10
...
----
GRANT READ {*} ON GRAPH * FOR (n:Document)
WHERE n.classification IN abac.native.user_tags() TO regularUsers
----

| The link:{neo4j-docs-base-uri}/cypher-manual/{page-version}/functions/security/#functions-security-native-user-tags[`abac.native.user_tags()`^] function can now also be used in the `WHERE` clause of a property-based privilege `GRANT` command.
This allows a single privilege to grant different users different data access based on their tags, as long as the native provider is configured for ABAC.
For more information, see the link:{neo4j-docs-base-uri}/operations-manual/current/authentication-authorization/property-based-access-control/#grant-privilege-using-native-user-tags[Operations Manual ->  Grant a property-based privilege using native user tags].
|===
...
[source, cypher, role="noheader"]
----
WITH {a: 1, b: 2} AS map1, {c: 3, d:4} AS map2
RETURN map1 + map2
----

| Introduction of xref:expressions/map-expressions.adoc#map-addition[map addition], allowing the merging of two `MAP` values.
...
[source, cypher, role="noheader"]
----
RETURN toString(), toStringList(), toStringOrNull()
----
| The functions xref:functions/string.adoc#functions-tostring[toString()], xref:functions/list.adoc#functions-tostringlist[toStringList()], xref:functions/string.adoc#functions-tostringornull[toStringOrNull()] have been updated to accept `LIST`, `MAP`, `NODE`, `RELATIONSHIP`, and `PATH` types.
...
----
| Show users can now return the command needed to recreate each user in the DBMS, including their authentication configuration and credentials.
For more information, see the link:{neo4j-docs-base-uri}/operations-manual/current/authentication-authorization/manage-users/[Operations Manual -> Manage users].
...
----
| Show roles can now return the commands needed to recreate each role in the DBMS, optionally including its assigned users and associated auth rules.
For more information, see the link:{neo4j-docs-base-uri}/operations-manual/current/authentication-authorization/manage-roles/[Operations Manual -> Manage roles].

a|
label:updated[]
[source, cypher, role="noheader"]
----
SHOW AUTH RULE ruleName PRIVILEGES [AS [REVOKE] COMMANDS]
----
| The `SHOW PRIVILEGES` and `SHOW PRIVILEGES AS [REVOKE] COMMANDS` commands now allow filtering the privileges based on auth rules, similar to the existing role and user filters.
For more information, see the link:{neo4j-docs-base-uri}/operations-manual/current/authentication-authorization/manage-privileges/#access-control-list-privileges[Operations Manual -> `SHOW PRIVILEGES`].

a|
label:updated[]
[source, cypher, role="noheader"]
----
SHOW [ROLE roleName] PRIVILEGES AS [REVOKE] COMMANDS
YIELD access, action, roles;

SHOW USER [username] PRIVILEGES AS [REVOKE] COMMANDS
YIELD access, action, roles, users;

SHOW AUTH RULE ruleName PRIVILEGES AS [REVOKE] COMMANDS
YIELD access, action, roles, authRules;
----
| Several new columns for filtering have been added to the `SHOW PRIVILEGES AS [REVOKE] COMMANDS` commands output.
Most are non-default but the `users` and `authRules` columns are returned by default when adding the `USER` or `AUTH RULE` filtering.
For more information, see the link:{neo4j-docs-base-uri}/operations-manual/current/authentication-authorization/manage-privileges/#access-control-list-privileges[Operations Manual -> `SHOW PRIVILEGES`].
|===
...
----
a|
New privilege that, together with the `SHOW USER` privilege, allows a user to show users as commands.
This is a sub-privilege of the `USER MANAGEMENT` privilege.
Like all `GRANT`/`DENY` commands this is only available in Enterprise Edition.
For more information, see the link:{neo4j-docs-base-uri}/operations-manual/current/authentication-authorization/dbms-administration/dbms-user-management-privileges/#_grant_privilege_to_show_users_as_commands[Operations Manual -> Grant privilege to show users as commands].
...
| Introduced the new `FULLTEXT` keyword in Cypher's xref::clauses/search.adoc[`SEARCH`] clause to enable full-text search.
|===
