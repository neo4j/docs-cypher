...
| Introduced `OPTIONAL CALL` clause for optionally executing a xref:clauses/call.adoc[procedure] or xref:subqueries/call-subquery.adoc[subquery] call.
Similar to xref:clauses/optional-match.adoc[`OPTIONAL MATCH`], any empty rows will return `null` and not affect the remainder of the procedure or subquery evaluation.
