...
Current constraint type for node uniqueness constraints, `UNIQUENESS`, will be updated to `NODE_UNIQUENESS` in 6.0.
...
The existing `UNIQUE` filter will now return both node and relationship uniqueness constraints.
All uniqueness constraint type filters now allow both `UNIQUE` and `UNIQUENESS` keywords.
...
----
a|
Uniqueness constraints now allow multiple properties, ensuring that the combination of property values are unique.
...
----
a|
Uniqueness constraints now allow multiple properties.
