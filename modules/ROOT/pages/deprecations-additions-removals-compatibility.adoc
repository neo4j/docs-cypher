...
Closed dynamic union types (`type1 \| type2 \| ...`) are now supported, allowing, for example, the following query which evaluates to true if a value is either of type `INTEGER` or `FLOAT`:
