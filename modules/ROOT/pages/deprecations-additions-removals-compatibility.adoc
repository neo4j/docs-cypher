...
----
a| Show current user can be combined with the other composable show commands and general Cypher clauses in a single query.
However, as for `SHOW DATABASES`, all parts of the query must be allowed on the system database.
For more information about composable show commands, see xref:clauses/show.adoc#composable-show-commands[SHOW -> Composable `SHOW` commands].
