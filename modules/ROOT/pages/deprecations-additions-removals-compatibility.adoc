...
* Subclause expressions which reference projection item expressions which are more complex than a variable or a property access on a variable.
* Cases with an ambiguity between an incoming variable and an alias in the subclause expression.
...
----
| A new xref:clauses/group-by.adoc[GROUP BY] subclause used in conjunction with `WITH` and `RETURN` clauses has been added. It allows a user to explicitly state the grouping keys for the query.
