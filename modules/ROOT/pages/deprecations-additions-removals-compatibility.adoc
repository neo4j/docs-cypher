...
a| Introduced a `UUID` value type.
...
| xref:functions/aggregating.adoc#functions-stdev[`stDev()`] and xref:functions/aggregating.adoc#functions-stdevp[`stDevP()`] now return `null` instead of `0` when given a `null` argument.
...
a| Introduced a `VECTOR` value type.
...
Although vectors can still be inserted in and retrieved from the database with Cypher 5 queries using a link:https://neo4j.com/docs/create-applications/[Neo4j client library] version >= 6.0, the Cypher functions to create and store `VECTOR` type properties are only available using link:https://neo4j.com/docs/cypher-manual/25/values-and-types/vector/[Cypher 25].
...
[[cypher-deprecations-additions-removals-5.26.26]]
== Neo4j 5.26.26
