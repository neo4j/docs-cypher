...
----
a|
Introduced new GQL conformant keyword `OFFSET` as a synonym to `SKIP`.

See xref::clauses/offset.adoc[offset] for more.
...
----
a|
Introduced new GQL conformant standalone `ORDER BY`, `SKIP`, `OFFSET`, and `LIMIT` clause.

See xref::clauses/order-by.adoc#order-standalone-clause[order by], xref::clauses/offset.adoc[offset], xref::clauses/skip.adoc#skip-standalone-clause[skip], xref::clauses/limit.adoc[limit] for more.
|===
...
RETURN a.name, a.year
----
| Extension of the xref:clauses/load-csv.adoc#import-from-an-aws-s3-uri[LOAD CSV] clause to allow loading CSV files from AWS S3 URIs.
