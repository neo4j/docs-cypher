...
a| xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-merge-unique-node[`MergeUniqueNode`] is similar to the `Merge` operator, but it is  is used when there is a property uniqueness constraint on the property used in the `MERGE` statement.
