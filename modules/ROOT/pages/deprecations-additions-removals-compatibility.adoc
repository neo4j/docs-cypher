...
----
MATCH (a)-->(b)
RETURN a.p AS a, COUNT(*) AS c 
ORDER BY a.p
----
a| Deprecated the following:

* Subclause expressions which reference projection item expressions more complex than a variable or a property access on a variable.
* Cases where the subclause expression is ambiguous between an incoming variable and an alias.

`ORDER BY` and `WHERE` now support the use of aggregation functions that are not projection items as long as the containing projection clause is aggregating.
|===
...
In earlier versions, they return `0`.
