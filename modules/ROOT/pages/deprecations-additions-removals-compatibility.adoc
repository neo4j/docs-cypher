...
RETURN trim(BOTH 'x' FROM 'xxhelloxx'),
       ...
       btrim('xxhelloxx', 'x')
----
| Extended the xref:functions/string.adoc#functions-trim[trim()], xref:functions/string.adoc#functions-ltrim[ltrim()]
and xref:functions/string.adoc#functions-rtrim[rtrim()] functions to accept alternative trim character strings as well
as introduced a new xref:functions/string.adoc#functions-btrim[btrim()] function.
