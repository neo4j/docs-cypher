...
YIELD name, aliases
WHERE name = home
RETURN user, name AS database, aliases
----
a| `SHOW CURRENT USER` can now be combined with the other composable show commands and general Cypher clauses in a single query.
However, as for `SHOW DATABASES`, all parts of the query must be allowed on the system database.
For more information about composable `SHOW` commands, see xref:clauses/show.adoc#composable-show-commands[Composable `SHOW` commands].
For more information about show current user, see the link:{neo4j-docs-base-uri}/operations-manual/current/authentication-authorization/manage-users/#access-control-current-users[Operations Manual -> Show current user].
|===
...
Introduced a xref:planning-and-tuning/caching.adoc#query-size-limit[query size limit].
a| Introduced a xref:planning-and-tuning/caching.adoc#query-size-limit[query size limit] for a query to be considered for query caching. This limit is used to prevent large generated query text strings from occupying memory in the query cache.
|===
