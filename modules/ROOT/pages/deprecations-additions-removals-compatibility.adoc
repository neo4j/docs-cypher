...
:description: Cypher is a language that is constantly evolving. New features are added to the language continuously, and occasionally, some features become deprecated and are subsequently removed. 
...
See xref:administration/security/administration.adoc#administration-security-administration-dbms-privileges-execute[The dbms `EXECUTE` privileges].
...
label:functionality[]
label:new[] +
xref:administration/security/index.adoc[Security administration]
a|
...
----
a|
xref:administration/indexes-for-search-performance.adoc#administration-indexes-drop-an-index[New command] for dropping an index by name.
...
| xref:administration/indexes-for-search-performance.adoc[Composite indexes] | Index | Added |
| xref:administration/constraints.adoc#administration-constraints-node-key[Node Key] | Index | Added | Neo4j Enterprise Edition only
...
These features will result in runtime errors. See the table at xref:deprecations-additions-removals-compatibility.adoc#cypher-deprecations-additions-removals-4.0[Cypher Version 4.0] for the list of affected features.
