...
|
Introduction of xref:patterns/repeatable-relationship-and-node-matching.adoc[`REPEATABLE ELEMENTS`], which ensures that relationships as well as nodes can be repeatedly traversed in a graph pattern.
...
a|
New keyword, xref:patterns/unique-relationship-matching.adoc[`DIFFERENT RELATIONSHIPS`], which enables explicitly specifying Cypher's default behavior of not allowing relationships to be re-traversed in a graph pattern.
