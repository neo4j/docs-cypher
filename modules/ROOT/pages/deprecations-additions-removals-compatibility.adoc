...
----
a|
Three new columns were introduced, `lastRead`, `readCount`, and `trackedSince`. Both `lastRead` and `readCount` are returned by default.
...
And the `trackedSince` column returns the time when usage statistics tracking started for this index.
