...
The variables declared in a subquery are available to the outer scope of the enclosing query.
...
[NOTE]
=====
As of Neo4j 5.22, naked subqueries without a scope clause are deprecated.
...
* The scope clause may not contain aliasing of variables from the outer scope.
...
[%collapsible]
====
The importing `WITH` clause abides by the same xref:subqueries/call-subquery.adoc#variable-scope-rules[rules] as the variable scope clause.
