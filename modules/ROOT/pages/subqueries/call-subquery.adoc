...
A subsequent `WITH` within the subquery cannot descope an imported variable.
The deprecated xref:subqueries/call-subquery.adoc#importing-with[importing `WITH` clause] behaves differently because imported variables can only be referenced from the first line and can be descoped by subsequent clauses.
...
* The importing `WITH` clause must be the first clause of a subquery (or the second clause, if directly following a `USE` clause).
...
* When using a `UNION [ALL]` directly within the `CALL` any imported variables must be imported per `UNION` branch.
...
|===
| link:https://neo4j.com/docs/status-codes/current/errors/gql-errors/42I28/[42I28]: error: syntax error or access rule violation - invalid use of importing WITH. Importing WITH can consist only of direct references to outside variables. 'WHERE' is not allowed.
...
Attempting to use an outer variable only imported into the first `UNION` branch in the second throws an error:

.Not Allowed
[source, cypher, role=test-fail]
----
WITH "Team A" AS teamName
CALL {
  WITH teamName
  MATCH (p:Player)-[:PLAYS_FOR]->(t {name: teamName})
  RETURN p.name AS associatedName, "Player" AS relationshipType
  UNION
  MATCH (t {name: teamName})-[:OWES]->(other:Team)
  RETURN other.name AS associatedName, "Owed Team" AS relationshipType
}
RETURN teamName, associatedName, relationshipType 
----

.GQLSTATUS error chain
[role="queryresult",stripes="all",cols="1"]
|===
| link:https://neo4j.com/docs/status-codes/current/errors/gql-errors/42N62/[42N62]: error: syntax error or access rule violation - variable not defined. Variable `teamName` not defined.

link:https://neo4j.com/docs/status-codes/current/errors/gql-errors/42001/[42001]: error: syntax error or access rule violation - invalid syntax
|===

To fix this, import the needed variables separately using an importing `WITH` in all branches.
...
WITH "Team A" AS teamName
CALL {
  WITH teamName // Import teamName here
  MATCH (p:Player)-[:PLAYS_FOR]->(t {name: teamName})
  RETURN p.name AS associatedName, "Player" AS relationshipType
  UNION
  WITH teamName // Import teamName here as well
  MATCH (t {name: teamName})-[:OWES]->(other:Team)
  RETURN other.name AS associatedName, "Owed Team" AS relationshipType
}
RETURN teamName, associatedName, relationshipType 
...
[role="queryresult",options="header,footer",cols="3*<m"]
|===
| teamName | associatedName | relationshipType
| "Team A" | "Player A"     | "Player"
| "Team A" | "Player B"     | "Player"
| "Team A" | "Team B"       | "Owed Team"
