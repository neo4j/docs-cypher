...
* When using a `UNION [ALL]` directly within the `CALL` any imported variables need to be imported per `UNION` branch.
...
Attempting to use an outer variable only imported into the first `UNION` branch in the second will throw an error:
