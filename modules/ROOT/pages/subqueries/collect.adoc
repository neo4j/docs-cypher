...
[[conditional-collect]]
== Conditional COLLECT subqueries
...
`WHEN` operators can be used inside a `COLLECT` subquery to execute branches only when a condition evaluates to `true`.
Note that the names and number of columns returned by the different `WHEN` branches must be identical.
