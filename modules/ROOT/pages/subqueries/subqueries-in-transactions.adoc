...
For this import, xref:schema/constraints/create-constraints.adoc#create-key-constraints[key constraints] are in place to ensure the existence of unique `movieId` properties for `Movie` nodes and `value` properties for `Year` nodes, and for those properties to be indexed (key constraints are xref:schema/constraints/create-constraints.adoc#constraints-and-backing-indexes[backed by indexes]).
