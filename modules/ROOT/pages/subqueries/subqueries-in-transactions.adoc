...
The expressions must use only variables imported from the outer query, since they are evaluated before the subquery runs; for example, `elementId(n)` or `n.id`, where `n` is an imported variable.
They act as constraints on batch composition: the xref:planning-and-tuning/runtimes/index.adoc[runtime] ensures that no two concurrent batches share a value for any of the declared expressions.
...
With a batch size of `2` (i.e. `IN CONCURRENT TRANSACTIONS OF 2 ROWS DISJOINT BY (n.id, n.category)`), the batching and execution could look something like this:
...
row 4: id=4, category=A
row 5: id=5, category=B
row 6: id=6, category=B

Batches:
  Batch A: rows 1, 2  (category=A)
  Batch B: rows 3, 4  (category=A)
  Batch C: rows 5, 6  (category=B)
...
This example imports movies and their release years from a CSV file, creating `Movie` and `Year` nodes and `RELEASED_IN` relationships between them.
For this import, xref:schema/constraints/create-constraints.adoc#create-key-constraints[key constraints] are in place to ensure the existence of unique `movieId` properties for `Movie` nodes and `year` properties for `Year` nodes, and to provide index-backed lookups for these properties (key constraints are xref:schema/constraints/create-constraints.adoc#constraints-and-backing-indexes[backed by indexes]).

With these constraints and indexes, two concurrent batches attempting to `MERGE` the same `Movie` or `Year` node simultaneously will contend on the key constraint lock, causing a deadlock.

.Create node key constraints
[source, cypher]
----
CREATE CONSTRAINT movie_id IF NOT EXISTS FOR (m:Movie) REQUIRE m.movieId IS KEY;
CREATE CONSTRAINT year IF NOT EXISTS FOR (y:Year) REQUIRE y.year IS KEY;
----
...
Indexes on the merged properties are required in order for `DISJOINT BY AUTO` to infer locking expressions.
If the runtime cannot infer any lock expressions, the subquery executes with no scheduling restriction.

To see which lock expressions (if any) were inferred by the planner, prepend the statement with xref:planning-and-tuning/execution-plans.adoc#reading-explain-and-profile-query-plans[`EXPLAIN`].
...
With key constraints (and backing indexes) on the `name` properties of `Person` nodes and the `title` properties of `Movie` nodes, the runtime infers the lock expressions `(line.person, line.movie)` automatically from the `MERGE` patterns as can be seen in the `TransactionForEach` operator `Details` column in the query plan produced by prepending the statement with `EXPLAIN` (the same expressions a user would write manually with `DISJOINT BY (line.person, line.movie)`).
...
----
EXPLAIN
LOAD CSV WITH HEADERS FROM 'https://data.neo4j.com/intro/movies/actors.csv' AS line
CALL (line) {
...
| +EmptyResult         |  1 |                                                                                             |             10 |
| |                    +----+---------------------------------------------------------------------------------------------+----------------+
| +TransactionForeach  |  2 | IN CONCURRENT TRANSACTIONS OF 1000 ROWS DISJOINT BY (line.person, line.movie) ON ERROR FAIL |             10 |
| |\                   +----+---------------------------------------------------------------------------------------------+----------------+
| | +Create            |  3 | (p)-[anon_0:ACTS_IN {role: line.roles}]->(m)                                                |             10 |
| | |                  +----+---------------------------------------------------------------------------------------------+----------------+
| | +Apply             |  4 |                                                                                             |             10 |
| | |\                 +----+---------------------------------------------------------------------------------------------+----------------+
| | | +MergeUniqueNode |  5 | UNIQUE m:Movie(title) WHERE title = line.movie,                                             |             10 |
| | |                  +----+---------------------------------------------------------------------------------------------+----------------+
| | +MergeUniqueNode   |  6 | UNIQUE p:Person(name) WHERE name = line.person,                                             |             10 |
| |                    +----+---------------------------------------------------------------------------------------------+----------------+
| +LoadCSV             |  7 | line                                                                                        |             10 |
+----------------------+----+---------------------------------------------------------------------------------------------+----------------+
----
