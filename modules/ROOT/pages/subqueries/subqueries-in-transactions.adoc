...
The examples above are generally valid for composite databases, but they must be slightly
modified to accommodate the nature of composite databases.
...
Even though composite databases allow accessing multiple graphs in a single query, only
one graph can be modified in a single transaction.
...
Calls targeting different graphs can't be interleaved so if a `USE` clause evaluates to a different
target than the current one, the batch is committed and the next batch is created.
