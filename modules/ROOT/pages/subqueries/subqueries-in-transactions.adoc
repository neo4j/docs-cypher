...
The expressions must use only variables imported from the outer query, since they are evaluated before the subquery body runs; for example, `elementId(n)` or `n.id`, where `n` is an imported variable.
