...
This example tries to `Movie` and `Year` nodes connected by a `RELEASED_IN` relationship type.
Note that the CSV file contains several rows with the same `year`.
...
The query fails because two or more transactions are trying to create the same relationship concurrently:
