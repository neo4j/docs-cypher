...
[NOTE]
====
Since the batching is performed on the input rows fed into `CALL { ... } IN TRANSACTIONS`, the data must be supplied from outside the `CALL { ... } IN TRANSACTIONS` in order for the batching to have an effect.
That is why the nodes are matched outside `CALL { ... } IN TRANSACTIONS` in the example above.
====
...
====
Since the batching is performed on the input rows fed into `CALL { ... } IN TRANSACTIONS`, the data must be supplied from outside the `CALL { ... } IN TRANSACTIONS` in order for the batching to have an effect.
That is why the nodes are matched in a `CALL` preceding the `CALL { ... } IN TRANSACTIONS` in the example above.
====
...
Since queries on composite databases can interact with more than one graph, batching is slightly more complex.
