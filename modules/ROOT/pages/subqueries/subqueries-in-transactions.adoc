...
[NOTE]
====
`CALL { ... } IN CONCURRENT TRANSACTIONS` currently provides no extra parallelism for composite databases. The subquery is executed in parallel on each target graph per default.
====
...
[NOTE]
When using `CONCURRENT TRANSACTIONS` on a composite databases, there is no additional parallelism in the query execution. See xref:subqueries/subqueries-in-transactions.adoc#composite-databases[CALL subqueries in transactions for composite databases] for more information.
