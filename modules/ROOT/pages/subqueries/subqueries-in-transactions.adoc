...
By default, `CALL { ... } IN TRANSACTIONS` uses a single thread assigned to one CPU processor to sequentially execute batched queries.
However, `CALL` subqueries can also execute batched inner transactions in parallel by appending `IN [n] CONCURRENT TRANSACTIONS`, where `n` is the concurrency value used to set the maximum number of transactions that can be executed in parallel.
...
.Syntax for `CALL` subqueries executed in concurrent transactions
[source, syntax]
----
CALL {
    subQuery
...
This example creates `Person` nodes from a csv file with over 400 lines in concurrent transactions of 200 rows.
...
=== Concurrent semantics and non-deterministic results
...
That is,  in `CALL { <I> } IN CONCURRENT TRANSACTIONS [...]`, writes committed in  execution of `<I~1~>` may not be observed by 
`<I~2~>`, and so on.
...
For example, when creating a node, a write lock is taken for the specific node, and when creating or deleting a relationship, a write lock is taken on both the specific relationship and its connected nodes.

A deadlock happens when two transactions are blocked by each other because they are attempting to concurrently modify a node or a relationship that is locked by the other transaction.
This may occur when using `CALL { ... } IN CONCURRENT TRANSACTIONS` if the transactions for two or more batches tries to take the same lock on a node or relationship.
