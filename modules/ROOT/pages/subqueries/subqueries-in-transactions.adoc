...
If any row in one batch shares a value for either expression with any row in another batch, those batches are run sequentially.
...
For an import workload like this, xref:schema/constraints/create-constraints.adoc#create-key-constraints[key constraints] (index-backed constraints ensuring both the existence and the uniqueness of a property) on `Movie.id` and `Year.year` are likely in place to prevent duplicate nodes.
...
[source, cypher]
----
CREATE CONSTRAINT movie_id IF NOT EXISTS FOR (m:Movie) REQUIRE m.id IS KEY;
...
`DISJOINT BY (row.year, row.movieId)` declares that `row.year` and `row.movieId` identify the locked resources for each batch.
`row.year` is the primary grouping key; since many movies share the same release year, it is low-cardinality and leads to efficient batch grouping.

Given that the CSV contains multiple movies released in the same year, the runtime groups rows into batches by `row.year` and serialises batches that share a year value:

.Simplified batch scheduling model
[source]
----
row 1: movieId=1, year=1995
row 2: movieId=2, year=1995
row 3: movieId=3, year=1995
row 4: movieId=4, year=1996
row 5: movieId=5, year=1996

Batches:
  Batch A: rows 1, 2  (year=1995)
  Batch B: row  3     (year=1995)
  Batch C: rows 4, 5  (year=1996)
...
Batch A and Batch B share `year=1995` and are therefore serialized; Batch C shares no values with either and runs concurrently throughout.
----
...
.`DISJOINT BY (<expr>, ...)`: importing relationships between pre-existing nodes
====

In this example, `Person` and `Movie` nodes have already been imported separately.
A list of rating records is passed in as the `$batch` parameter.
Even though no nodes are created, the `MERGE` on the `RATED` relationship acquires write locks on both endpoint nodes.
Two concurrent batches sharing the same `row.personId` or `row.movieId` can therefore deadlock on those endpoint locks.
However, `DISJOINT BY (row.personId, row.movieId)` ensures that no two concurrent batches share an endpoint node, thereby preventing deadlocks up front.
...
{
  "batch": [
    {"personId": "1", "movieId": "1", "score": 4.5, "timestamp": "2024-01-01"},
    {"personId": "1", "movieId": "2", "score": 3.0, "timestamp": "2024-01-02"},
    {"personId": "2", "movieId": "1", "score": 5.0, "timestamp": "2024-01-03"}
  ]
}
----

.Importing ratings between pre-existing nodes
[source, cypher]
----
UNWIND $batch AS row
CALL (row) {
  ...
  MERGE (a)-[r:RATED]->(m)
  SET r.score = row.score, r.timestamp = row.timestamp
} IN CONCURRENT TRANSACTIONS OF 500 ROWS
  DISJOINT BY (row.personId, row.movieId)
  ON ERROR RETRY THEN CONTINUE
----
====
