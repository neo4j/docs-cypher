...
To execute a `CALL` subquery in separate transactions, you add the modifier `IN TRANSACTIONS` after the subquery.
...
For more information, see xref:subqueries/subqueries-in-transactions.adoc#error-behavior[Error behavior].
...
[IMPORTANT]
====
The `CALL { ... } IN TRANSACTIONS` subquery is handled by the database to ensure optimal performance.
