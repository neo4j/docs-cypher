...
[[disjoint-by]]
=== Batch scheduling with `DISJOINT BY`
...
Alternatively, `DISJOINT BY` can be appended to individual `CALL { ... } IN CONCURRENT TRANSACTIONS` statements to control batch scheduling at the query level.
...
[NOTE]
`DISJOINT BY` can only be run together with concurrent transactions; it is not valid in a sequential `CALL` subquery.
...
The expressions are evaluated per input row, using only variables imported into the subquery scope, and before any subquery clause is executed.
...
The first expression in the list is used as the primary grouping identity for each batch.
Choosing a low-cardinality expression first leads to better batch grouping, though the order does not affect correctness.
No automatic lock expression inference is performed — the supplied list is the entire lock expression set for that subquery.

[[automatic-inference]]
==== Automatic lock expression inference with `DISJOINT BY AUTO`


: the runtime infers lock-prone resources automatically from the subquery body. If none can be inferred, the subquery executes with no scheduling restriction.
