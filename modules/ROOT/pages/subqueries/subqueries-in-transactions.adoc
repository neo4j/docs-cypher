...
offers a way to construct queries modifying multiple graphs.
...
The declared batch size is 3, but the evaluation of the `USE` clause causes the batch size to be 2 on 'composite.remoteGraph1', followed by 3 on 'composite.remoteGraph2', followed by 1 on 'composite.remoteGraph2' and finally 2 on 'composite.remoteGraph1'.
