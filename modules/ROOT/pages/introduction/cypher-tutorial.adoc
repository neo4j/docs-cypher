...
The tutorial uses the link:https://github.com/neo4j-graph-examples/movies/tree/main/documentation[Neo4j movie database].
...
The first clause takes as input the state of the graph before the query and a table initialized with a single empty row for intermediate results.
...
Most clauses are applied to each row of the table of intermediate results in turn, producing zero, one, or more output rows for each input row.
The exceptions to this rule are projections such as xref:clauses/return.adoc[`RETURN`] or xref:clauses/with.adoc#aggregations[`WITH`] with aggregation functions, which operate on the whole table of intermediate results at once.
