...
To search for patterns of a fixed length, specify the distance ("hops") between the nodes in the pattern by using a xref::patterns/reference.adoc#quantifiers[quantifier] (`{}`). 
For example, the following query matches all `Person` nodes exactly 2 "hops" away from `Tom Hanks` and returns the first five rows, ordered by the `born` property value of the `colleagues` nodes in an ascending order.
...
The below query matches all `Person` nodes between `1` and `4` "hops" away from `Tom Hanks` and returns the first five rows, ordered by the `born` property value of the `colleagues` nodes in an ascending order.
