...
It also goes through a short Cypher tutorial based on the Neo4j movie database.
...
SQL queries begin with what a user wants to return, whereas Cypher queries end with the return clause. For example, consider the following two queries, both searching a database for titles of movies with a rating of greater than 7:
...
For example, consider the following two queries, both searching a database for the names of the actors in the movie The Matrix:
...
In this short tutorial, users will learn how to develop a create, query, and delete a property graph database using Cypher. 
The tutorial uses the Neo4j movie database. 
