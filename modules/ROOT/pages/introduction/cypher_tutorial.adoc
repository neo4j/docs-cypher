...
For example, they share many keywords, such as `WHERE` and `ORDER BY`. 

However, there are important differences between the two which are important to consider when composing queries.

* Cypher queries are run in transactions. 
For more information, see the section on xref:introduction/cypher_neo4j.adoc#cypher-neo4j-transactions[Transactions in Neo4j].
...
More specifically, nodes and relationships in a Neo4j database do not have to have a specific property set to them because other nodes or relationships in the same graph have that property (unless there is a existence constraint created on the specific property).
...
image::introduction_schema.svg[width="700",role="middle"]
...
_For a practical tool to develop data models, visit the https://arrows.app[Arrows app] developed by Neo4j engineers. 
...
It is also possible to search a graph for several nodes. 
...
More specifically, each clause has as input the state of the graph and a table of intermediate results consisting of the current variables.
...
Note that if one of the clauses returns `null`, there will be nothing to pass on to subsequent clauses, and the query will effectively end.
...
Queries can chose to whether to specify relationship types, relationship directions, as well as the start and end nodes.

For example, the below query searches the graph for `Movie` nodes connected to `Tom Hanks` through an outgoing relationship. 
...
The graph returned shows that he has 13 outgoing relationships connected to `Movie` nodes (12 `ACTED_IN` relationships and one `DIRECTED` relationship).

image::introduction_example1.svg[width="500",role="middle"]

The below query specifies a relationship type to exclude from the returned results:
...
For example, the following query matches all `Person` nodes up to 3 "hops" away from `Tom Hanks` and returns the first five rows returned. The `DISTINCT` operator ensures the result contain no duplicate values. 
...
This example looks for new co-actors to recommend for `Keanu Reeves`. 
It tries to find connections past his immediate neighboring `Person` nodes which are themselves well connected to other actors, and it orders the results by the strength  of the recommendation. 
...
RETURN cocoActors.name AS Recommended, count(coc0Actors) AS Strength ORDER BY Strength DESC LIMIT 5
----
...
There are several connections between the `Keanu Reeves` and `Tom Hanks` nodes.
The following query matches coactors who could introduce the two:
