...
| `VECTOR` values can be stored as properties if the instance uses Neo4j's link:{neo4j-docs-base-uri}/operations-manual/current/database-internals/store-formats/#store-format-overview[block format].
