...
| Store xref:values-and-types/vector.adoc[`VECTOR`] values as properties.
...
If you want to delete it, first set another database as the default using the procedure link:{neo4j-docs-base-uri}/operations-manual/current/procedures/built-in-procedures/#procedure_dbms_setDefaultDatabase[`dbms.setDefaultDatabase`] (this must be done from the `system` database).
...
** Schema modifications, which contain xref:clauses/index.adoc#index-and-schema-clauses[index and constraint clauses],
