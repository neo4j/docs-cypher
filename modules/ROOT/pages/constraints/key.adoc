...
[role=label--new-5.16]
=== Create key constraints using a parameters
...
=== Creating data that complies with an existing key constraint
...
* xref:constraints/key.adoc#create-a-node-or-relationship-that-violates-a-node-key-constraint[]
* xref:constraints/key.adoc#removing-a-key-constrained-property[]
* xref:constraints/key.adoc#fail-to-create-a-key-constraint-due-to-existing-node[]
...
[discrete]
[[create-a-node-or-relationship-that-violates-a-node-key-constraint]]
=== Creating a node or relationship that violates an existing constraint will fail
...
=== Removing a constrained property will fail
...
[discrete]
[[fail-to-create-a-key-constraint-due-to-existing-node]]
=== Creating a constraint when there exist conflicting node or relationship will fail
