...
Key constraints are backed by xref:indexes/manage-indexes.adoc#create-range-index[range indexes].
This means that creating a key constraint will create a range index with the same name as its owning constraint.

These indexes can be viewed in the `owningConstraint` column returned by the xref:indexes/manage-indexes#list-indexes[`SHOW INDEX`] command, and the `ownedIndex` column returned by the xref:constraints/list-drop-constraints.adoc#list-constraints[`SHOW CONSTRAINT`] command.



[[create-key-constraint-with-index-provider]]
==== Specify index provider when creating key constraints
