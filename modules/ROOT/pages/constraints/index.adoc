...
* Adding key or uniqueness constraints on a single property also adds a xref:indexes/manage-indexes.adoc#create-range-index[range index] on that property, and therefore, an index of the same index type, label/relationship type, and property combination cannot be added separately.
* Adding a key or uniqueness constraint for a set of properties adds a xref:indexes/using-indexes.adoc#composite-indexes[composite range index] on those properties, and therefore, an index of the same index type, label/relationship type, and properties combination cannot be added separately.
