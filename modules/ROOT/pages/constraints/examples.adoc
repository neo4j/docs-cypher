...
----
Node(0) already exists with label `Book` and property `title` = 'Graph Databases'
----
...
////
TODO: Re-add this part when adding back relationship key and uniqueness constraints
TODO: Remode 'test-skip' message on queries when feature is introduced
...
[source, cypher]
----
CREATE (user:User)-[like:LIKED {day: 'yesterday', since:2020}]->(book:Book {title:'Jane Eyre', isbn: 9789186579296})
----
...
[source, cypher, role=test-setup]
----
CREATE CONSTRAINT preExistingUniqueNodeKey FOR (p:Person) REQUIRE (p.firstname, p.age) IS UNIQUE
----
////
...
----
Constraint already exists:
Constraint( id=4, name='preExistingUniqueNodeKey', type='UNIQUENESS', schema=(:Person {firstname, age}), ownedIndex=3 )
----
