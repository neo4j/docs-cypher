...
A node property uniqueness constraint ensures that certain nodes have a set of certain properties whose combined value is unique when all properties exists on the node.
...
Either use xref::indexes-for-search-performance.adoc[] instead, or remove or correct the offending nodes and then re-apply the constraint.
...
A relationship property uniqueness constraint ensures that certain relationships have a set of certain properties whose combined value is unique when all properties exists on the relationship.
...
A node property existence constraint ensures that certain nodes have a certain property.
...
A relationship property type constraint ensures that certain relationhips have a property of the required property type when the property exists on the relationhip.
...
A node key constraint ensures that certain nodes have a set of certain properties whose combined value is unique and all properties in the set are present.
