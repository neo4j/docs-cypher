...
In this case, the constraint cannot be created because the `MAP` type is not allowed in property type constraints.
...
----
CREATE CONSTRAINT score
FOR (movie:Movie) REQUIRE movie.imdbScore IS :: LIST<LIST<FLOAT>>
----
...
[source, error]
----
Failed to create node property type constraint: Invalid property type `LIST<LIST<FLOAT>>`. Lists cannot have lists as an inner type.
----
...
----
CREATE CONSTRAINT score
FOR ()-[part:PART_OF]-() REQUIRE part.releaseOrder IS :: LIST<LIST<INTEGER>>
----
...
[source, error]
----
Failed to create relationship property type constraint: Invalid property type `LIST<LIST<INTEGER>>`. Lists cannot have lists as an inner type.
----
