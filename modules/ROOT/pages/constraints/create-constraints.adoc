...
[NOTE]
The detailed statistics view says `Unique constraints added: 1`.
It is updated to say `Node property uniqueness constraints added: 1` in Neo4j 2025.01.
...
[NOTE]
The detailed statistics view says `Relationship uniqueness constraints added: 1`.
It is updated to say `Relationship property uniqueness constraints added: 1` in Neo4j 2025.01.
======
...
For the types that properties can be constrained by, see xref:constraints/create-constraints.adoc#type-constraints-allowed-properties[], and for information about different data types in Cypher, see xref:values-and-types/index.adoc[].
...
[NOTE]
The constraint type is updated to say `RELATIONSHIP PROPERTY UNIQUENESS` in Neo4j 2025.01.
======
...
[NOTE]
The constraint type is updated to say `NODE PROPERTY UNIQUENESS` in Neo4j 2025.01.
...
These indexes are listed in the `owningConstraint` column returned by the xref:indexes/search-performance-indexes/list-indexes.adoc[`SHOW INDEX`] command, and the `ownedIndex` column returned by the xref:constraints/list-constraints.adoc[`SHOW CONSTRAINT`] command.
