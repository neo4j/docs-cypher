...
.Create node type property constraint using a parameter
======
...
.Create node property type constraint with a union type
======
...
.Create node that complies with existing node property type constraint
======
...
* xref:constraints/type.adoc#create-an-already-existing-property-type-constraint[]
...
[discrete]
[[create-a-different-than-existing-property-type-constraint]]
...
[discrete]
[[create-property-type-constraints-on-invalid-types]]
...
=== Creating data that violates an existing constraint will fail


.Create node that violates an existing node property type constraint
======
...
[discrete]
[[fail-to-create-a-property-type-constraint-due-to-existing-data]]
=== Creating a constraint when there exist conflicting data will fail
