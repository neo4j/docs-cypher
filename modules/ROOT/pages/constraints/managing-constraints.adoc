...
* Relationship property uniqueness constraints: `CREATE CONSTRAINT constraint_name
FOR ()-[r:REL_TYPE]-() REQUIRE (r.propertyName_1, ..., r.propertyName_n) IS UNIQUE`.
...
.Create a constraint requiring `Book` nodes to have unique `title` and `publicationYear` properties
[source, cypher]
----
...
.Create a constraint requiring `PREQUEL_OF` relationships to have unique `order` and `author` properties
[source, cypher]
----
...
.Create a `SEQUEL_OF` relationship with an `order` property not already in the graph
[source, cypher]
----
...
Key constraints ensure that property values are unique and exists for all nodes with a specific label or all relationships with a specific type
...
* Node key constraints: `CREATE CONSTRAINT constraint_name
...
.Create a composite relationship key constraint label on multiple properties :new[Introduced in 5.7]
======
