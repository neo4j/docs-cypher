...
For composite property uniqueness constraints on multiple properties, the combination of property values must be unique.
...
.Create a constraint requiring all nodes with a `Book` label to have a unique `isbn` property
[source, cypher]
----
...
.Create a constraint requiring all `SEQUEL_OF` relationships to have a unique `order` property
[source, cypher]
----
...
* Relationship property uniqueness constraints: `CREATE CONSTRAINT constraint_name
FOR ()-[r:RE:_TYPE]-() REQUIRE (r.propertyName_1, ..., r.propertyName_n) IS UNIQUE`.

.Create a composite node property uniqueness constraint on several properties
======

.Create a constraint requiring all `Book` nodes to have unique `title` and `publicationYear` properties
[source, cypher]
----
...
.Create a constraint requiring all `PREQUEL_OF` relationships to have unique `order` and `author` properties
[source, cypher]
----
...
[[creating-uniqueness-constraint-compliant-data]]
...
.Create a `SEQUEL_OF` relationship with `order` property values are not already in the graph
[source, cypher]
----
...
.Create a constraint requiring all `Author` nodes to have a `name` property
[source, cypher]
----
...
.Create a constraint requiring all `WROTE` relationships to have a `year` property
[source, cypher]
----
...
[[creating-existence-constraint-compliant-data]]
...
.Create a `WROTE` relationship with a `year` location
[source, cypher]
----
...
FOR (n:Label) REQUIRE n.propertyIS NODE KEY`.
* Relationship property key constraints: `CREATE CONSTRAINT constraint_name
FOR ()-[r:REL_TYPE]-() REQUIRE r.property IS RELATIONSHIP KEY`.
...
.Create a node property key constraint on a single property
======
...
.Create a constraint requiring all `Director` nodes to have a unique `imdbId` property as a node key.
[source, cypher]
----
...
.Create a relationship property key constraint on a single property label:new[Introduced in 5.7]
======
...
.Create a constraint requiring all `OWNS` relationships to have a unique `ownershipId` property as a relationship key
[source, cypher]
----
...
==== Create composite key constraints

Constraints created for multiple properties are called composite constraints.
Note that the constrained properties must be bracketed when creating composite key constraints.

Composite key constraints are created with the following commands:

* Node property key constraints: `CREATE CONSTRAINT constraint_name
FOR (n:Label) REQUIRE (n.propertyName_1, ..., n.propertyName_n)  NODE KEY`.
* Relationship property key constraints: `CREATE CONSTRAINT constraint_name
FOR ()-[r:REL_TYPE]-() REQUIRE (r.propertyName_1, ..., r.propertyName_n) IS RELATIONSHIP KEY`.

.Create a composite node property key constraint on multiple properties
======

.Create a constraint requiring all `Actor` nodes to have a unique combination of `firstname` and `surname` properties as a node key
[source, cypher]
----
...
.Create a composite relationship property key constraint label on multiple properties :new[Introduced in 5.7]
======

A relationship key constraint ensures that certain relationships have a set of defined properties whose combined value is unique.
It also ensures that all properties in the set are present.

.Create a constraint requiring all `KNOWS` relationships to have a unique combination of `since` and `how` properties as a relationship key
[source, cypher]
----
...
[[creating-key-constraint-compliant-data]]
...
.Create a `KNOWS` relationship with both `since` and `how` properties
[source, cypher]
----
...
.Create a constraint requiring all `title` properties on `Movie` nodes to be of type `STRING`
[source, cypher]
----
...
.Create a constraint requiring all `order` properties on `PART_OF` relationships to be of type `INTEGER`
[source, cypher]
----
...
[[creating-type-constraint-compliant-data]]
...
.Create a constraint requiring all `tagline` properties on `Movie` nodes to be either of type `STRING` or `LIST<STRING NOT NULL>`.
[source, cypher]
----
...
.Create a constraint requiring all `tags` properties on `PART_OF` relationships to either of type `STRING` or `LIST<STRING NOT NULL>`
[source, cypher]
----
...
| 15 | "ownershipId"           | "RELATIONSHIP_KEY"        | "RELATIONSHIP" | ["OWNS"]       | ["ownershipId"]              | "ownershipId"           | NULL         |
...
+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 16 | "actor_fullname"        | "ONLINE" | 100.0             | "RANGE" | "NODE"         | ["Actor"]      | ["firstname", "surname"]     | "range-1.0"   | "actor_fullname"        | 2024-09-19T09:26:11.886Z | 4         |
| 2  | "book_isbn"             | "ONLINE" | 100.0             | "RANGE" | "NODE"         | ["Book"]       | ["isbn"]                     | "range-1.0"   | "book_isbn"             | 2024-09-19T09:23:00.535Z | 2         |
| 6  | "book_title_year"       | "ONLINE" | 100.0             | "RANGE" | "NODE"         | ["Book"]       | ["title", "publicationYear"] | "range-1.0"   | "book_title_year"       | NULL                     | 0         |
| 12 | "director_imdbId"       | "ONLINE" | 100.0             | "RANGE" | "NODE"         | ["Director"]   | ["imdbId"]                   | "range-1.0"   | "director_imdbId"       | NULL                     | 0         |
| 18 | "knows_since_how"       | "ONLINE" | 100.0             | "RANGE" | "RELATIONSHIP" | ["KNOWS"]      | ["since", "how"]             | "range-1.0"   | "knows_since_how"       | 2024-09-19T09:26:11.887Z | 1         |
| 24 | "node_uniqueness_param" | "ONLINE" | 100.0             | "RANGE" | "NODE"         | ["Book"]       | ["prop1"]                    | "range-1.0"   | "node_uniqueness_param" | NULL                     | 0         |
| 14 | "ownershipId"           | "ONLINE" | 100.0             | "RANGE" | "RELATIONSHIP" | ["OWNS"]       | ["ownershipId"]              | "range-1.0"   | "ownershipId"           | NULL                     | 0         |
| 8  | "prequels"              | "ONLINE" | 100.0             | "RANGE" | "RELATIONSHIP" | ["PREQUEL_OF"] | ["order", "author"]          | "range-1.0"   | "prequels"              | NULL                     | 0         |
| 4  | "sequels"               | "ONLINE" | 100.0             | "RANGE" | "RELATIONSHIP" | ["SEQUEL_OF"]  | ["order"]                    | "range-1.0"   | "sequels"               | 2024-09-19T09:22:15.563Z | 1         |
+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
----
...
.Create a constraint requiring all Actor nodes to have a unique `surname` property as a node key, specifying the `range-1.0` index provider
[source, cypher]
----
...
.Create a constraint requiring all `SEQUEL_OF` relationships to have a unique combination of `order`, `seriesTitle`, and `number` properties, specifying the `range-1.0` index provider
[source, cypher]
----
...
.Create a constraint requiring all `order` properties on `PART_OF` relationships to be of type `INTEGER`, when that constraint already exists
[source, cypher, role=test-fail]
----
...
.Remove the `location` property from an existing `WROTE` relationship, given a property existence constraint `:WROTE(location)`
[source, cypher, role=test-fail]
----
...
.Create a constraint requiring all `Book` nodes to have unique `wordCount` properties, when an index already exists on that label and property combination
[source, cypher, role=test-fail]
----
...
.Create a constraint requiring all `KNOWS` relationships to have a unique level property as a relationship key, when an index already exists with the given name
[source, cypher, role=test-fail]
----
...
| 13 | "director_imdbId"             | "NODE_KEY"                        | "NODE"         | ["Director"]   | ["imdbId"]                         | "director_imdbId"             | NULL                             |
| 19 | "knows_since_how"             | "RELATIONSHIP_KEY"                | "RELATIONSHIP" | ["KNOWS"]      | ["since", "how"]                   | "knows_since_how"             | NULL                             |
| 22 | "movie_tagline"               | "NODE_PROPERTY_TYPE"              | "NODE"         | ["Movie"]      | ["tagline"]                        | NULL                          | "STRING | LIST<STRING NOT NULL>" |
| 20 | "movie_title"                 | "NODE_PROPERTY_TYPE"              | "NODE"         | ["Movie"]      | ["title"]                          | NULL                          | "STRING"                         |
| 25 | "node_uniqueness_param"       | "UNIQUENESS"                      | "NODE"         | ["Book"]       | ["prop1"]                          | "node_uniqueness_param"       | NULL                             |
| 15 | "ownershipId"                 | "RELATIONSHIP_KEY"                | "RELATIONSHIP" | ["OWNS"]       | ["ownershipId"]                    | "ownershipId"                 | NULL                             |
| 21 | "part_of"                     | "RELATIONSHIP_PROPERTY_TYPE"      | "RELATIONSHIP" | ["PART_OF"]    | ["order"]                          | NULL                          | "INTEGER"                        |
| 23 | "part_of_tags"                | "RELATIONSHIP_PROPERTY_TYPE"      | "RELATIONSHIP" | ["PART_OF"]    | ["tags"]                           | NULL                          | "STRING | LIST<STRING NOT NULL>" |
| 37 | "preExisting_actor_name_age"  | "UNIQUENESS"                      | "NODE"         | ["Actor"]      | ["firstname", "age"]               | "preExisting_actor_name_age"  | NULL                             |
| 32 | "preExisting_book_published"  | "UNIQUENESS"                      | "NODE"         | ["Book"]       | ["published"]                      | "preExisting_book_published"  | NULL                             |
| 9  | "prequels"                    | "RELATIONSHIP_UNIQUENESS"         | "RELATIONSHIP" | ["PREQUEL_OF"] | ["order", "author"]                | "prequels"                    | NULL                             |
...
| 5  | "sequels"                     | "RELATIONSHIP_UNIQUENESS"         | "RELATIONSHIP" | ["SEQUEL_OF"]  | ["order"]                          | "sequels"                     | NULL                             |
| 33 | "wrote_locations"             | "RELATIONSHIP_PROPERTY_EXISTENCE" | "RELATIONSHIP" | ["WROTE"]      | ["location"]                       | NULL                          | NULL                             |
...
+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 17 | "actor_fullname"              | "NODE_KEY"                        | "NODE"         | ["Actor"]      | ["firstname", "surname"]           | "actor_fullname"              | NULL                             | {indexConfig: {}, indexProvider: "range-1.0"} | "CREATE CONSTRAINT `actor_fullname` FOR (n:`Actor`) REQUIRE (n.`firstname`, n.`surname`) IS NODE KEY"                                  |
...
| 19 | "knows_since_how"             | "RELATIONSHIP_KEY"                | "RELATIONSHIP" | ["KNOWS"]      | ["since", "how"]                   | "knows_since_how"             | NULL                             | {indexConfig: {}, indexProvider: "range-1.0"} | "CREATE CONSTRAINT `knows_since_how` FOR ()-[r:`KNOWS`]-() REQUIRE (r.`since`, r.`how`) IS RELATIONSHIP KEY"                           |
| 22 | "movie_tagline"               | "NODE_PROPERTY_TYPE"              | "NODE"         | ["Movie"]      | ["tagline"]                        | NULL                          | "STRING | LIST<STRING NOT NULL>" | NULL                                          | "CREATE CONSTRAINT `movie_tagline` FOR (n:`Movie`) REQUIRE (n.`tagline`) IS :: STRING | LIST<STRING NOT NULL>"                         |
| 20 | "movie_title"                 | "NODE_PROPERTY_TYPE"              | "NODE"         | ["Movie"]      | ["title"]                          | NULL                          | "STRING"                         | NULL                                          | "CREATE CONSTRAINT `movie_title` FOR (n:`Movie`) REQUIRE (n.`title`) IS :: STRING"                                                     |
...
| 15 | "ownershipId"                 | "RELATIONSHIP_KEY"                | "RELATIONSHIP" | ["OWNS"]       | ["ownershipId"]                    | "ownershipId"                 | NULL                             | {indexConfig: {}, indexProvider: "range-1.0"} | "CREATE CONSTRAINT `ownershipId` FOR ()-[r:`OWNS`]-() REQUIRE (r.`ownershipId`) IS RELATIONSHIP KEY"                                   |
| 21 | "part_of"                     | "RELATIONSHIP_PROPERTY_TYPE"      | "RELATIONSHIP" | ["PART_OF"]    | ["order"]                          | NULL                          | "INTEGER"                        | NULL                                          | "CREATE CONSTRAINT `part_of` FOR ()-[r:`PART_OF`]-() REQUIRE (r.`order`) IS :: INTEGER"                                                |
...
| 37 | "preExisting_actor_name_age"  | "UNIQUENESS"                      | "NODE"         | ["Actor"]      | ["firstname", "age"]               | "preExisting_actor_name_age"  | NULL                             | {indexConfig: {}, indexProvider: "range-1.0"} | "CREATE CONSTRAINT `preExisting_actor_name_age` FOR (n:`Actor`) REQUIRE (n.`firstname`, n.`age`) IS UNIQUE"                            |
| 32 | "preExisting_book_published"  | "UNIQUENESS"                      | "NODE"         | ["Book"]       | ["published"]                      | "preExisting_book_published"  | NULL                             | {indexConfig: {}, indexProvider: "range-1.0"} | "CREATE CONSTRAINT `preExisting_book_published` FOR (n:`Book`) REQUIRE (n.`published`) IS UNIQUE"                                      |
...
| 33 | "wrote_locations"             | "RELATIONSHIP_PROPERTY_EXISTENCE" | "RELATIONSHIP" | ["WROTE"]      | ["location"]                       | NULL                          | NULL                             | NULL                                          | "CREATE CONSTRAINT `wrote_locations` FOR ()-[r:`WROTE`]-() REQUIRE (r.`location`) IS NOT NULL"                                         |
