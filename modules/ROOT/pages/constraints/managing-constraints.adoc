...
.Create a `Book` node with an `isbn` property that is not already in the database
[source, cypher]
----
...
.Create a `SEQUEL_OF` relationship with an `order` property not already in the database
[source, cypher]
----
...
.Create an `Actor` node with `firstname` and `surname` properties that are not already in the database
[source, cypher]
----
...
.Create a `KNOWS` relationship with `since` and `how` properties that are not already in the database
[source, cypher]
----
...
* Creating a constraint with a different name but on the same schema as an already existing constraint.
* Creating a constraint with the same name but on a different schema as an already existing constraint.
...
.Create a constraint identical to a constraint already in the database
======
...
.Create a relationship property uniqueness constraint with the same name as a different type of constraint already in the database
======
...
Creating a constraint on the same label or relationship type and properties that are already constrained will fail.
...
.Create a constraint with a different name but on the same schema as a constraint already in the database.
======

.Create a constraint requiring all `SEQUEL_OF` relationships to have unique `order` properties, given that a constraint on that schema already exists
[source, cypher, role=test-fail]
----
...
==== Creating key and property uniqueness constraints on the same schema is not allowed
