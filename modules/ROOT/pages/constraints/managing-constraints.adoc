...
For composite key constraints on multiple properties, it is the combination of property values that must be unique.
Queries that try to create new nodes of the specified label, or relationships of the specified type, without the constrained property will fail.
...
* Creating a constraint with a different name but on the same constraint type and schema as an already existing constraint.
...
Creating a constraint on the same node label or relationship type and properties that are already constrained by a constraint of the same type will fail.
