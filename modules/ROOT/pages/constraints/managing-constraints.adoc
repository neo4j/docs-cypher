...
.Create a `Book` node with an `isbn` property that is not already in the graph
[source, cypher]
----
...
* Node property key constraints: `CREATE CONSTRAINT constraint_name FOR (n:Label) REQUIRE n.property IS NODE KEY`.
* Relationship property key constraints: `CREATE CONSTRAINT constraint_name FOR ()-[r:REL_TYPE]-() REQUIRE r.property IS RELATIONSHIP KEY`. label:new[Introduced in 5.7]
...
.Create an `Actor` node with `firstname` and `surname` properties that are not already in the graph
[source, cypher]
----
...
.Create a `KNOWS` relationship with `since` and `how` properties that are not already in the graph
[source, cypher]
----
...
.Create a constraint requiring `Book` nodes to have unique combinations of `title` properties, when there already exists two nodes with the same `title`
[source, cypher, role=test-fail]
----
...
This can be used to drop the constraint with the xref::constraints/syntax.adoc#drop-constraint[`DROP CONSTRAINT` command].
