...
| state
| Information of the state of the server: `free`, `enabled`, `deallocating`, `deallocated`, or `dropped`.
| STRING
| {check-mark}
| {check-mark}
...
The server changes state to `deallocating` and all databases that the server is hosting are moved to other servers.
When a server no longer hosts any databases besides system is in the `deallocated` state.
...
When a server is in the `deallocated` state. it can be removed from the cluster with `DROP SERVER 'name'`.
Either the id or name of the server can be used.
