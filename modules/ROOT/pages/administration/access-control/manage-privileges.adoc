...
This lists the privileges that are possible to grant or deny on this server, together with the structure of the privilege.
...
The qualifiers are used according to the table below:

[options="header", width="100%", cols="2m,6a"]
|===
| qualifier | example
...
If a privilege has a scope, it is optional to specify it. If it isn't explicitly set it will be `ELEMENT *`.
