...
The metrics of data modifications made by the inner queries will be aggregated in their outer query on commit. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.03]
m| MAP

|===

.Contents of the currentQueryProgress map
* `nodesCreated` Total number of nodes created by the query currently executing in this transaction.
* `nodesDeleted` Total number of nodes deleted by the query currently executing in this transaction.
* `relationshipsCreated` Total number of relationships created by the query currently executing in this transaction.
* `relationshipsDeleted` Total number of relationships deleted by the query currently executing in this transaction.
* `propertiesSet` Total number of properties set by the query currently executing in this transaction.
* `labelsAdded` Total numbers of labels added by the query currently executing in this transaction.
* `labelsRemoved` Total numbers of labels removed by the query currently executing in this transaction.
* `fileLinesRead` Total number of CVS file lines read by the query currently executing in this transaction.
* `transactionsStarted` The total number of inner transactions started by the CIT query, if this query is an outer transaction, otherwise null. For details, see xref:subqueries/subqueries-in-transactions.adoc[`CALL { ... } IN TRANSACTIONS`].
* `transactionsCommitted` The total number of inner transactions committed by the CIT query, if this query is an outer transaction, otherwise null. For details, see xref:subqueries/subqueries-in-transactions.adoc[`CALL { ... } IN TRANSACTIONS`].
* `transactionsRolledBack` The total number of inner transactions rolled back by the CIT query, if this query is an outer transaction, otherwise null. For details, see xref:subqueries/subqueries-in-transactions.adoc[`CALL { ... } IN TRANSACTIONS`].
