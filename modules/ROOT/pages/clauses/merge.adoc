...
Here, `MERGE` matches on the full pattern including the relationship, so for each `Person` node it finds no existing match and creates a new `Chauffeur` node, even if one with the same name already exists.
