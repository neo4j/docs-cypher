...
It is also recommended to create constraints prior to merging data: they provide index-backed performance improvements while also preventing the creation of data that differ in unintended ways from pre-existing data.
...
To prevent the unintended creation of nodes or relationships that differ in unintended ways from existing ones, it is therefore recommended to use constraints when merging data.
...
Also note that the query removes the existing `chauffeurName` properties on any `Person` nodes.
...
There are two ways to ensure the uniqueness of a property value in a database: xref:schema/constraints/create-constraints.adoc#create-property-uniqueness-constraints[property uniqueness constraints] and xref:schema/constraints/create-constraints.adoc#create-key-constraints[key constraints].
Both are backed by indexes, which mean they provide significant performance improvements.
...
.Merge a new node that abides to key constraint
[source, cypher]
----
...
[role="queryresult",stripes="all",cols="1"]
|===
|link:https://neo4j.com/docs/status-codes/current/errors/gql-errors/22N80/[22N80]:data exception - index entry conflict. Index entry conflict: Node(104) already exists with label `Product` and property `id` = 12.

link:https://neo4j.com/docs/status-codes/current/errors/gql-errors/22N79/[22N79]: data exception - property uniqueness constraint violated. Property uniqueness constraint violated: Node(104) already exists with label `Product` and property `id` = 12.
|===
...
[role="queryresult",stripes="all",cols="1"]
|===
| link:https://neo4j.com/docs/status-codes/current/errors/gql-errors/22N77/[22N77]: data exception - property presence verification failed. NODE (106) with label 'Product' must have the following properties: `id`.
|===

[role=label--enterprise-edition]
=== Enforce property value types with property type constraints

xref:schema/constraints/create-constraints.adoc#create-property-type-constraints[Property type constraints] are useful to enforce type correctness when merging data.
...
.Merge a new node that abides to property type constraint
[source, cypher]
----
...
[role="queryresult",stripes="all",cols="1"]
|===
| link:https://neo4j.com/docs/status-codes/current/errors/gql-errors/22N77/[22N78]: data exception - property type verification failed. NODE (108) with label 'Product' must have the property `id` with value type INTEGER.
|===
...
.Merge a new node that casts data type and abides to property type constraint 
[source, cypher]
----
