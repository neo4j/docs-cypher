...
If there isn't a node with the specific `name` property, a new node will be created with the specified `name` property.
...
See xref::clauses/merge.adoc#query-merge-using-unique-constraints[Using property uniqueness constraints with `MERGE`] for more information on how `MERGE` can be used in combination with a property uniqueness constraint.
====
...
The last part of a `MERGE` clause is the the `ON CREATE` and/or `ON MATCH` operators.
...
In the above query, three nodes labeled `City` are created, each of which contains a `name` property with the value of `'New York'`, `'Ohio'`, and `'New Jersey'` respectively.
...
As `'Charlie Sheen'` and `'Oliver Stone'` do not know each other in our example graph, this `MERGE` query will create a `KNOWS` relationship between them.
...
As `MERGE` found no matches -- in our example graph, there are no nodes labeled with `Chauffeur` and no `HAS_CHAUFFEUR` relationships -- `MERGE` creates six nodes labeled with `Chauffeur`, each of which contains a `name` property whose value corresponds to each matched `Person` node's `chauffeurName` property value.
...
The query creates a `Person` node with the `name` property set to `'Laurence Fishburne'`.
If a `'Laurence Fishburne'` node had already existed, `MERGE` would just match the existing node.
...
A `Person` node with the `name` `'Oliver Stone'` already exists, so `MERGE` just matches it.
