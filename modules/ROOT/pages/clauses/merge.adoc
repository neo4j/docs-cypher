...
For more information about the differences between the two clauses, see xref:clauses/create.adoc#comparing-create-and-merge[`CREATE` -> Comparing `CREATE` and `MERGE`].
...
[NOTE]
Multiple labels can also be separated by an ampersand `&`, in the same manner as it is used in xref:patterns/reference/node-and-relationship-patterns.adoc#label-expressions[label expressions].
...
Both are xref:schema/constraints/create-constraints.adoc#constraints-and-backing-indexes[backed by indexes], which provide significant performance improvements.
