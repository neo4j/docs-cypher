...
[source, cypher]
----
MERGE (robert:Critic:Person)
RETURN labels(robert)
----

A new node is created because there are no nodes labeled both `Critic` and `Person` in the database:
...
As of Neo4j 5.17, multiple labels can also be separated by an ampersand `&`, in the same manner as it is used in xref:patterns/reference.adoc#label-expressions[label expressions].
...
No new node is created because there was already a node labeled both `Critic` and `Person` in the database:
