...
The number of rows can be expressed using any expression that evaluates to a positive integer -- however the expression cannot refer to nodes or relationships.

The following is the same example as above for loading a CSV file but with one transaction every `2` input rows:
