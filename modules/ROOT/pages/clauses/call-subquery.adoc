...
.Execute for each incomming row
======
...
* `ON ERROR CONTINUE` to ignore a recoverable error and continue the execution of subsequent inner transactions. The outer transaction succeeds.
* `ON ERROR BREAK` to ignore a recoverable and stop the execution of subsequent inner transactions. The outer transaction succeeds.
* `ON ERROR FAIL` to fail the outer transaction. This is the default behaviour if we do not specify a flag explicitly.

[IMPORTANT]
====
On error, any previously committed inner transactions remain committed, and are not rolled back. Any failing inner transactions are rolled back.
====
...
Example with `ON ERROR CONTINUE`, where we can appreciate we execute the remaining transactions after the failing one:
...
Example with `ON ERROR BREAK`, where after the failing inner transaction, we do not execute any more work:
...
Example with `ON ERROR FAIL`, where the outer transaction would fail:
...
=== Status report
The user can also report the execution status of the inner transactions using `REPORT STATUS AS <v>`.
This flag is disallowed for `ON ERROR FAIL` (see <<txs_error_behaviour, error behaviour>>).
...
* If the inner execution produces one or more rows as output,
then a binding to this status value is added to each row, under the variable name `<v>`.
* If the inner execution fails then a single row is produced containing a binding to this status value under the variable name `<v>`, and null bindings for all variables that should have been returned in the inner query (if any).
...
* `committed`, true when the changes were successfully committed, false otherwise
* `transactionId`, string representing the transaction id.
* `errorMessage`, error message, null in case of no error.
...
CALL {
  ...
  ON ERROR BREAK
  REPORT STATUS AS s
RETURN n.num, s.started, s.committed;
----

.Result
[role="queryresult",options="header,footer",cols="3*<m"]
|===
| +n.num+ | +s.started+ | +s.committed+
| 100     | true        | true
| null    | true        | false
| null    | false       | false
| null    | false       | false
