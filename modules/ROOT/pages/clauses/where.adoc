...
- All  xref::patterns/reference.adoc[pattern] semantics apply to path patterns in a `MATCH` clause.
Path pattern expressions in a `WHERE` clause can only use
xref::patterns/reference.adoc#relationship-patterns[relationship] and
xref::patterns/reference.adoc#variable-length-relationships[variable-length relationship] semantics.

- Multiple path patterns in a `MATCH` clause can be separated by commas.
Multiple path pattern expressions in a `WHERE` clause can only be separated by xref::syntax/operators.adoc#query-operators-boolean[Boolean operators] such as `AND` and `OR`.
