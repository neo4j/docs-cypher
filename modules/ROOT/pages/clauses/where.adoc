...
Path patterns used in a MATCH clause should generally be thought of as retrieving data. Path patterns expressions used
in a WHERE clause should on the other hand be thought of as eliminating data. The clause `MATCH (a)-[]->(b)` is very
different from `WHERE (a)-[]->(b)`. The first will produce a record for every path it can find between `a` and `b`,
whereas the latter will eliminate records where `a` and `b` do not have a directed relationship between them.

There are several limitations for what path patterns can do in contrast to their path pattern counterparts.
