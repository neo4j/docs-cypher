...
Path pattern expressions used in a `WHERE` clause should be thought of as filtering data.
...
The latter will filter records where `a` and `b` do not have a directed relationship between them.
