...
`WHERE` clauses can be added to xref:patterns/fixed-length-paths.adoc[fixed-length] and xref:patterns/variable-length-paths.adoc[variable-length patterns] in order to specify additional constraints.
...
If matching for variable length patterns, `WHERE` can only be used together with the xref:patterns/variable-length-paths.adoc#quantified-path-patterns[quantified path pattern] or xref:patterns/variable-length-paths.adoc#quantified-relationships[quantified relationships] syntax.
...
For more information about using `WHERE` predicates in quantified path patterns, see xref:patterns/variable-length-paths.adoc#quantified-path-patterns-predicates[Predicates in quantified path patterns].

`WHERE` is not allowed in variable-length patterns using the non-GQL compliant xref:patterns/reference/variable-length-paths.adoc#variable-length-relationships[variable-length relationship] syntax.
