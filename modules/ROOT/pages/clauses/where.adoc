...
Path pattern expressions can be used to assert whether a specified path exists at least once in a graph. Path pattern expressions are similar to xref::subqueries/existential.adoc[existential subqueries].
On one hand, xref::subqueries/existential.adoc[existential subqueries] are the more powerful feature and can do anything path pattern expressions can, while on the other hand path pattern expressions are more concise.

Path pattern expressions come with several restrictions which are highlighted below.
Use cases which require more permissive functionality should consider using xref::subqueries/existential.adoc[existential subqueries] instead.

Path pattern expressions may only use a subset of xref::patterns/reference.adoc#graph-patterns[graph patterns] semantics.
A path pattern expressions must be a xref::patterns/reference.adoc#path-patterns[path pattern] of length greater than zero.
