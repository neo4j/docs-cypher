...
are applied only after resolving the escaped character sequences in the given <<cypher-expressions-string-literals, string literal>>.
It is sometimes necessary to add additional backslashes to express regular expression
constructs. This list attempts to clarify the combination of these two definitions,
...
A full list of supported regular expression constructs can be found
link:https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/regex/Pattern.html[here].
