...
[abstract]
--
The `WHERE` clause adds constraints to the patterns in a `MATCH` or `OPTIONAL MATCH` clause. 
It can also filter the results of a `WITH` clause.
--
...
The following boolean operations can be used with the `WHERE` clause: `AND`, `OR`, `XOR`, and `NOT`.
...
Returns the `name` and `age` values for nodes that have a relationship with the type `KNOWS` connected to `Timothy`.
...
To test if a value or variable is not `null`, use the `IS NOT NULL` operator (`NOT(IS NULL x)` also works).
