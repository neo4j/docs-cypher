...
To remove all data, including indexes and constraints, recreate the database using the following command: link:{neo4j-docs-base-uri}/operations-manual/current/database-administration/standard-databases/create-databases/#_create_databases_with_if_not_exists_or_or_replace[`CREATE OR REPLACE DATABASE name`].
...
Alternatively, you can delete a database using the link:{neo4j-docs-base-uri}/operations-manual/current/database-administration/standard-databases/delete-databases/[`DROP DATABASE`] command, which deletes everything in a database, includes constraints and indexes, without recreating a new one.
