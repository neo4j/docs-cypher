...
.Query
// tag::clauses_delete_node[]
[source, cypher, indent=0]
----
MATCH (n:Person {name: 'Tom Hanks'})
DELETE n
----
// end::clauses_delete_node[]
...
.Query
// tag::clauses_delete_relationship[]
[source, cypher, indent=0]
----
MATCH (n:Person {name: 'Laurence Fishburne'})-[r:ACTED_IN]->()
DELETE r
----
// end::clauses_delete_relationship[]
...
.Query
// tag::clauses_delete_node_and_connected_relationships[]
[source, cypher, indent=0]
----
MATCH (n:Person {name: 'Carrie-Anne Moss'})
DETACH DELETE n
----
// end::clauses_delete_node_and_connected_relationships[]
...
`DETACH DELETE` can be used to delete all nodes and relationships in a graph.

.Delete all nodes and relationships
// tag::clauses_delete_all[]
[source, cypher, indent=0]
----
MATCH (n)
DETACH DELETE n
----
// end::clauses_delete_all[]
...
.Delete all nodes with a specific label and all relationships connected to them using `CALL` subqueries
[source, cypher]
----
MATCH (n:Person)
CALL (n) {
...
// tag::clauses_delete_in_transactions[]
[source, cypher]
----
MATCH (n)
CALL (n) {
...
----
// end::clauses_delete_in_transactions[]

To remove all data, including indexes and constraints, you can either:

* Recreate the database using the link:{neo4j-docs-base-uri}/operations-manual/current/database-administration/standard-databases/create-databases/#_create_a_database_with_if_not_exists_or_or_replace[`CREATE OR REPLACE DATABASE <database-name>`] command.
If you intend to recreate the existing indexes and constraints in the new database, first run xref:schema/constraints/list-constraints.adoc[`SHOW CONSTRAINTS`]/xref:indexes/search-performance-indexes/list-indexes.adoc[`SHOW INDEXES`] and return the specific `createStatement`.

* Delete a database using the link:{neo4j-docs-base-uri}/operations-manual/current/database-administration/standard-databases/delete-databases/[`DROP DATABASE <database-name>`] command, which deletes everything in a database, includes constraints and indexes, without recreating a new one.

////
[source, cypher, role=test-setup]
----
CREATE CONSTRAINT person_name_key
FOR (p:Person)
REQUIRE p.name IS NODE KEY;
----
////

.Show the `createStatement` for constraints
[source, cypher]
----
...
+------------------------------------------------------------------------------------------------------+
| "person_name_key" | "CREATE CONSTRAINT `person_name_key` FOR (n:`Person`) REQUIRE (n.`name`) IS KEY" |
+------------------------------------------------------------------------------------------------------+
----
...
[source, cypher]
----
DROP DATABASE actors IF EXISTS
----

== Syntax
:syntax-filename: clauses/delete
