...
Projected aliases make variables defined prior to `RETURN` or `WITH` in accessible, if these variables share the same names.
====
...
`title` cannot be used as a grouping key because the query aggregates with `avg()` which requires grouping keys to be projected expressions or constants.
