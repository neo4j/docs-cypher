...
image:graph_match_clause.svg[width="700"]
...
Nodes with a label on them can be found with a single node pattern where the node has a label on it. 
...
Returns any nodes connected by an outgoing relationship to `Oliver Stone`.
...
It is possible to introduce a variable to a patterns, either for filtering on relationship properties or to return a relationship. 
...
Databases occasionally contain relationship types including non-letter characters, or with spaces in them. 
These are created using ``` (backticks).
For example, the following query creates an additional relationship between `Martin Sheen` and `Rob Reiner` which contains a space:
...
image:graph_match_clause_backtick.svg[width="700"]
...
The following query adds two new relationships between `Charlie Sheen` and his father `Martin Sheen`, where a `lead` property is added to the `:ACTED_IN` relationships.
...
MATCH
  (charlie:Person {name: 'Charlie Sheen'}),
  (martin:Person {name: 'Martin Sheen'})
CREATE (charlie)-[:ACTED_IN {role: 'Bud', lead: true}]->(:Movie {title: 'Free Money'})<-[:ACTED_IN {role: 'lead:false}]-(martin)
...
image:graph_match_clause_variable_length.svg[width="700"]
...
The above query finds the paths between `Charlie Sheen` and `Martin Sheen` where all relationships have the `lead` property set to `true`. 
The following graph and text are returned:

image:MATCH_properties_on_variable_length_path.svg[width="400"]
...
=== Named paths
 ...
 Introduce a named path to return or filter on a path in the pattern graph.
 For example:
...
image:MATCH_named_path_example.svg[width="400"]
...
image:MATCH_shortestpath_example.svg[width="400"]

.Result
[role="queryresult",options="header,footer",cols="1*<m"]
|===
| +p+
| +│[{"name":"Martin Sheen"},{"role":"Carl Fox"},{"title":"Wall Street"},{│
...
image:MATCH_shortestpath_with_predicates_example.svg[width="400"]
...
image:MATCH_allshortestpaths_example.svg[width="400"]
