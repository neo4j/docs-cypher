...
To demonstrate this we can add an additional relationship between `Martin Sheen` and `Rob Reiner` that contains a space:
...
Thq query makes evident that both actors had a leading role in the movie *No Code of Conduct*, but only Martin Sheen had a leading role in the movie *Free Money*.
