...
For more information about finding node patterns, see xref:patterns/fixed-length-paths.adoc#node-patterns[Patterns -> Node patterns].
...
For a list of all label expressions supported by Cypher, see xref:patterns/reference/node-and-relationship-patterns.adoc#label-expressions[Patterns -> Label expressions].
...
For more information about relationship patterns, see xref:patterns/fixed-length-paths.adoc#relationship-patterns[Patterns -> Relationship patterns].

[NOTE]
By default, Cypher will only match a relationship once inside a single pattern.
This behavior can be bypassed using `REPEATABLE ELEMENTS`, which allows both nodes and relationships to occur more than once in a given `MATCH` result.
For more information, see xref::patterns/repeatable-node-and-relationship-paths.adoc[`REPEATABLE ELEMENTS`].
...
For a list of all relationship type expressions supported by Cypher, see xref:patterns/reference/node-and-relationship-patterns.adoc#label-expressions[Patterns -> Label expressions].
...
For more information about how `MATCH` is used to find patterns of varying complexity (including xref:patterns/variable-length-paths.adoc#quantified-path-patterns[quantified path patterns], xref:patterns/variable-length-paths.adoc#quantified-relationships[quantified relationships], and the xref:patterns/shortest-paths.adoc[shortest paths] between nodes), see the section on xref::patterns/index.adoc[Patterns].
...
* The planner does not combine multiple index seeks for a dynamic label query.
If several labels passed to `$any` share an index on the same property, the planner picks just one of those indexes (based on selectivity) and filters the rest of the results manually.
For example, the below `MATCH` query will only use one of the indexes on `birthYear` for either `Actor` or `Director`, but not both.
...
MATCH (p:$any(["Actor", "Director"]) { birthYear: 1983 })
RETURN p.name
----
