...
Unlike xref:clauses/merge.adoc[`MERGE`], which first checks for a match and only creates anything if nothing is found, `CREATE` always creates the given pattern, regardless of whether matching ones exist.
...
`CREATE` does not check for existing matches, so it will create duplicates unless a xref:schema/constraints/create-constraints.adoc#create-property-uniqueness-constraints[uniqueness constraint]/xref:schema/constraints/create-constraints.adoc#create-key-constraints[key constraint]key prevents it.
...
Checking whether a node or relationship already exists before running `CREATE` does not prevent duplicates under link:{neo4j-docs-base-uri}/operations-manual/current/database-internals/concurrent-data-access/[concurrent transactions]: concurrent transactions can each perform the check, find no existing node or relationship, and both go on to create one.
