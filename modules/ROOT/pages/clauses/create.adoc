...
Multiple labels can also be separated by `&`, in the same manner as it is used in xref:patterns/reference.adoc#label-expressions[label expressions].
Separation by colon and `&` cannot be mixed in the same query.
...
Both the above queries creates two nodes, bound to the variables `charlie` and `oliver`, each with a `Person` label and a `name` property.
...
While using `INSERT`, multiple labels must be separated by `&` and not by colon.

.Query
[source, cypher, indent=0]
----
INSERT (charlie:Person&Actor {name: 'Charlie Sheen'}), (oliver:Person&Director {name: 'Oliver Stone'})
----
