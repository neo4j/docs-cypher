...
`CREATE` does not check for existing matches, so it will create duplicates unless a xref:constraints/create-constraints.adoc#create-property-uniqueness-constraints[uniqueness constraint] or xref:constraints/create-constraints.adoc#create-key-constraints[key constraint] prevents it.
