...
`CREATE` does not check for existing matches, so it will create duplicates unless a xref:schema/constraints/create-constraints.adoc#create-property-uniqueness-constraints[uniqueness constraint] or xref:schema/constraints/create-constraints.adoc#create-key-constraints[key constraint]key prevents it.
