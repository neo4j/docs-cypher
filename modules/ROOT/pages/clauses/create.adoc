...
Unlike xref:clauses/merge.adoc[`MERGE`], which first checks for a match and only creates anything if nothing is found, `CREATE` always creates the given pattern, regardless of whether matching patterns exist.
For more information, see xref:clauses/create.adoc#comparing-create-and-merge[Comparing `CREATE` and `MERGE`] below.
...
[[comparing-create-and-merge]]
== Comparing `CREATE` and `MERGE`

`CREATE` and `MERGE` are both used to add nodes and relationships to the graph, but they differ in how they handle existing data.

`CREATE` unconditionally creates a new node or relationship, without checking whether a matching one already exists.
Running the same `CREATE` statement multiple times will therefore produce duplicate entries.

.Two identical `CREATE` statements will create two separate but identical entities
[source, cypher]
----
...
`CREATE` does not check for existing matches, so it will create duplicates unless a xref:schema/constraints/create-constraints.adoc#create-property-uniqueness-constraints[uniqueness constraint] or xref:schema/constraints/create-constraints.adoc#create-key-constraints[key constraint] prevents it.
With a uniqueness constraint on `:Person(name)`, the second `CREATE` statement above would fail instead.

`MERGE` first attempts to match an existing pattern in the graph.
If a match is found, it uses the existing node or relationship; if not, it creates a new one.
This makes `MERGE` idempotent; running it repeatedly with the same pattern will not create duplicates.

.Two identical `MERGE` statements will result in only one entity, as the second matches the first
[source, cypher]
----
...
| Creates only if no match exists; safe for incremental imports and re-runs.

| *Performance*
| Fast — no lookup required.
| Requires a lookup per row; without an xref:indexes/search-performance-indexes/index.adoc[index], this means a full label scan for every row.

| *Best for*
a|
* Loading into a fresh database with data you're certain is clean.
* IDs guaranteed unique, import won't be re-run.
a|
* Incremental imports or re-runnable jobs.
* Ensuring uniqueness when data may already exist.
|===
...
.Query
// tag::clauses_create_parameters[]
[source, cypher]
----
CREATE (n:Person $props)
RETURN n
----
// end::clauses_create_parameters[]
...
.Query
// tag::clauses_create_insert[]
[source, cypher, indent=0]
----
INSERT (tom:Person&Actor&Director {name: 'Tom Hanks'})
----
// end::clauses_create_insert[]

Creates a node, bound to the variable `tom`, with the labels `Person`, `Actor`, and `Director` and a `name` property.

[[create-and-concurrent-transactions]]
== `CREATE` and concurrent transactions

Checking whether a node or relationship already exists before running `CREATE` does not prevent duplicates under link:{neo4j-docs-base-uri}/operations-manual/current/database-internals/concurrent-data-access/[concurrent transactions]: concurrent transactions can each perform the check, find no existing node or relationship, and proceed to create identical entities.

To reliably create an entity only if it does not already exist, use `MERGE` instead, combined with a xref:schema/constraints/create-constraints.adoc#create-property-uniqueness-constraints[uniqueness constraint] on the merged property when merging nodes (this forces an index lock), or with both endpoint nodes bound by a preceding `MATCH` when merging relationships (forces a locking merge with locks taken on both endpoints).
== Syntax
:syntax-filename: clauses/create
