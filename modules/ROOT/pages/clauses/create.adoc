...
Nodes and relationships can be created dynamically by using variables or parameters for the node labels and relationship types.
...
.Create a node using dynamic labels
[source, cypher]
----
UNWIND ["Person", "Director"] AS nodeLabels
CREATE (n:$(nodeLabels) {name: 'Greta Gerwig'})
RETURN labels(n) AS createdLabels
----
