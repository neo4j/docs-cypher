...
It is possible to limit and filter on the same `WITH`, note that the `LIMIT` clause is applied before the `WHERE`.
...
The limit is first applied, reducing the rows to the first 5 items in the list. Then the filter is then applied, reducing the final result as seen below:
...
This time the filter is applied first, reducing the rows to consist of the list `[3, 4, 5, 6]`, then the limit is applied.
As the limit is larger than the total remaining results, all rows are returned.
