...
The limit is first applied, reducing the results to the first 5 items in the list, the filter is then applied, reducing the final results as seen below:
...
This time the filter is applied first, reducing the results to consist of the list [3, 4, 5, 6], then the limit is applied.
As the limit is larger than the total remaining results, all are returned.
