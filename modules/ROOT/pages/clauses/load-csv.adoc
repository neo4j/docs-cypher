...
[TIP]
Because `null` values are not stored in the database, a strategy for selectively getting rid of some values, for example the empty `STRING` values from the last query, is to map them into `null` values.
...
SET m.title = row.title, m.imdbId = toInteger(row.movie_imdbId),
    m.languages = split(row.languages, '|'), m.genres = split(row.genres, '|')
RETURN m LIMIT 5
----
