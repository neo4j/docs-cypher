...
A CSV file to use with `LOAD CSV` must have the following characteristics:
...
In that case, the field delimiter character can be changed by using the option `FIELDTERMINATOR` available to the `LOAD CSV` command.
The hexadecimal representation of the unicode character can be used if prepended by `{backslash}u`.
The encoding must be written with four digits, for example, `{backslash}u003B` is equivalent to `;` (semicolon).
...
The example below has both additional quotes around the values as well as escaped quotes inside the second value.
...
To apply quotation to a String, wrap it with double quote characters, e.g. `"my_string"`.
To avoid escaping, double quote must be in a quoted string and escaped, either with the escape character or a second double quote.
...
=== Import data from a relational data base
