...
[source, cypher]
----
MATCH p = (keanu:Person {name: 'Keanu Reeves'})-[r]->(b)
RETURN *
----
...
To introduce a placeholder that is made up of characters that are not contained in the English alphabet, you can use the ``` to enclose the variable:
...
The `bornIn` property of the node with the `name` property `Keanu Reeves` is returned:
...
`DISTINCT` retrieves only unique rows depending on the columns that have been selected for output.
