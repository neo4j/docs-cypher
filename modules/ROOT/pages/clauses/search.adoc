...
- The `index_name` must be an identifier that matches the name of an existing vector index or, as of Neo4j 2026.09, full-text index in the Neo4j database.
The index must be a node vector/full-text index if the `binding_variable` refers to a node and a relationship vector/full-text index if the `binding_variable` refers to a relationship.
