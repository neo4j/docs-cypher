...
- The optional `WITH ANALYZER` part is only allowed in full-text searches. The `<analyzer_string>` is of type `STRING` add may not be null.

- The optional `SKIP` or `OFFSET` clause is only allowed in full-text searches. The `<skip_integer>` is of type `INTEGER`, may not be null and must be `0` or positive.
...
queryparsersyntax.html#Boolean%20operators[Lucene boolean operators] (`AND`, `OR`, `NOT`, `+`, `-`):
