...
.`SEARCH` clause syntax for vector and full-text search
[.tabbed-example]
=====
[.include-with-vector-search]
======
...
[OPTIONAL] MATCH pattern
  SEARCH binding_variable IN (
    ...
    FOR query_string [ WITH ANALYZER analyzer_string ]
    [ (SKIP | OFFSET) skip_integer ]
    LIMIT top_k
  ) [SCORE AS score_alias]
----
...
- The `index_name` must be an identifier that matches the name of an existing vector index or full-text index in the Neo4j database.
The index must be a node vector index if the `binding_variable` refers to a node and a relationship vector index if the `binding_variable` refers to a relationship.

- For vector searches, the `query_vector` can be any expression that evaluates to a `VECTOR` or `LIST<INTEGER NOT NULL | FLOAT NOT NULL>`, for example a literal, a parameter or a property.
...
- The optional `WHERE` subclause is only allowed in vector searches. It applies a filter to the vector index, and is a restricted version of the normal Cypher xref::clauses/where.adoc[`WHERE` clause].
...
- For full-text searches, the `query_string` is of type `STRING`.

- The optional `WITH ANALYZER` part is only allowed in full-text searches. The `<analyzer_string>` is of type `STRING NOT NULL`.

- The optional `SKIP` or `OFFSET` clause is only allowed in full-text searches. The `<skip_integer>` is of type `INTEGER NOT NULL`, and must be `0` or positive.
...
RETURN movie.title AS title
----
// end::clauses_search_label_predicate[]
...
RETURN movie.title AS title
----
// end::clauses_search_property_predicate[]
...
[role=label--new-Neo4j-2026.09 label--cypher-25-only]
[[full-text-search]]
== Full-text search

[[full-text-search-example-graph]]
=== Example graph and full-text indexes
...
The examples use the following full-text indexes:
...
CREATE FULLTEXT INDEX reviews FOR (n:Employee|Manager) ON EACH [n.peerReviews];
----

For more information about the creation of full-text indexes, see xref::indexes/semantic-indexes/full-text-indexes.adoc#create-full-text-indexes[Create full-text indexes].

[[full-text-search-examples]]
=== Full-text search examples

This query uses the `namesAndTeams` full-text index to look for "nils" among the employee and manager nodes:

.Query a full-text index for a node property
// tag::clauses_search_fulltext_node[]
[source, cypher]
----
MATCH (node)
  SEARCH node IN (
...
----
// end::clauses_search_fulltext_node[]
...
This query uses the `communications` full-text index for any `message` containing "meeting":

.Query a full-text index for a relationship property
[source, cypher]
// tag::clauses_search_fulltext_relationship[]
[source, cypher]
----
MATCH ()-[relationship]-()
...
----
// end::clauses_search_fulltext_relationship[]
...
| "EMAILED" | "I have booked a team meeting tomorrow." | 0.30376967787742615
...
[source, cypher]
// tag::clauses_search_fulltext_exact_match[]
[source, cypher]
----
MATCH (node)
  SEARCH node IN (
...
RETURN node.name, score
----
// end::clauses_search_fulltext_exact_match[]
...
[source, cypher]
// tag::clauses_search_fulltext_logical_operators[]
[source, cypher]
----
MATCH (node)
  SEARCH node IN (
...
----
// end::clauses_search_fulltext_logical_operators[]
...
[source, cypher]
// tag::clauses_search_fulltext_specific_properties[]
[source, cypher]
----
MATCH (node)
  SEARCH node IN (
...
RETURN node.name, node.team, score
----
// end::clauses_search_fulltext_specific_properties[]
...
For scoring purposes, the full-text index treats it as a single-property value, and the score represents how close the query is to matching the entire list.

.Query a full-text index for content present in a list of `STRING` properties
[source, cypher]
// tag::clauses_search_fulltext_list_of_strings[]
[source, cypher]
----
MATCH (node)
  SEARCH node IN (
...
----
// end::clauses_search_fulltext_list_of_strings[]
...
You can query full-text indexes with `SEARCH` and use an analyzer with `WITH ANALYZER`:

.Query a full-text index using an analyzer
[source, cypher]
// tag::clauses_search_fulltext_with_analyzer[]
[source, cypher]
----
MATCH ()-[relationship]-()
  SEARCH relationship IN (
...
RETURN type(relationship), relationship.message, score
----
// end::clauses_search_fulltext_with_analyzer[]
...
| "REVIEWED" | "Nils-Erik is reportedly difficult to work with." | 0.3239005506038666

3+d|Rows: 1
|===

See xref::indexes/semantic-indexes/full-text-indexes.adoc#tokenization-analyzers[tokenization and analyzers] for more information on analyzers.


== Vector and full-text searches and their index counterparts

Trying to query a vector index with `FULLTEXT INDEX` syntax and vice versa results in an error.

.Not allowed -- Trying to query a full-text index with `VECTOR INDEX` syntax
====
.Query
[source, cypher, role=test-fail]
----
...
[role="queryresult",stripes="all",cols="1"]
|===
| link:https://neo4j.com/docs/status-codes/current/errors/gql-errors/22NCG/[22NCG]: error: data exception - wrong index type. Expected the index `namesAndTeams` to be a vector index but was a fulltext index.
|===


.Not allowed -- Trying to query a vector index with `FULLTEXT INDEX` syntax
====
.Query
[source, cypher, role=test-fail]
----
...
[role="queryresult",stripes="all",cols="1"]
|===
| link:https://neo4j.com/docs/status-codes/current/errors/gql-errors/22NCG/[22NCG]: error: data exception - wrong index type. Expected the index `moviePlots` to be a fulltext index but was a vector index.
|===
