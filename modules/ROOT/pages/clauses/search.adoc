...
The `SEARCH` clause constraints the `pattern` from the enclosing `MATCH` or `OPTIONAL MATCH`, by only returning nodes or relationships, included in the results of the ANN vector search as described by the `SEARCH` clause.
...
- The optional `WHERE` subclause is used to describe a vector search with filters, where a filter is applied within the vector index, and is a restricted version of the normal Cypher xref::clauses/where.adoc[`WHERE` clause].
...
.Error: mismatch of a property predicate variable and a binding variable in the SEARCH clause
====
.Query
...
.Error: a property of the property predicate wasn't defined as an additional property in the vector index
====
.Query 
[source, cypher, role=test-fail]
----
...
.Error: query vector with only two dimensions when three are being used
====
.Query
[source, cypher, role=test-fail]
----
...
.Error: binding variable from earlier clause
====
.Query
[source, cypher, role=test-fail]
----
...
.Error: binding variable not previously defined
====
.Query
[source, cypher, role=test-fail]
----
