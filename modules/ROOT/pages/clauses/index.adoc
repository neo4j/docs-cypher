...
| List the graph type for a database. label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.02]

m| xref:schema/constraints/list-constraints.adoc[SHOW CONSTRAINTS].
| List the constraints set on database.

m| link:{neo4j-docs-base-uri}/operations-manual/current/database-administration/standard-databases/listing-databases/[SHOW DATABASES].
| List the databases in a Neo4j instance.
