...
If the query also contains a `GROUP BY` clause, `ORDER BY` can be used to order results after grouping, see xref:/clauses/group-by.adoc#group-by-and-order-by[`GROUP BY` and `ORDER BY`].
...
== Syntax
:syntax-filename: clauses/order-by
