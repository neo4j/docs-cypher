...
`ORDER BY` can be used to sort property values.
...
The nodes are returned, sorted by their name in an ascending order.
...
You can order the results of multiple property values by having two or more property variables listed in the the `ORDER BY` subclause.
...
This returns the nodes, sorted first by their `age` property, and then by their `name` property.
