...
* If the `RETURN` or `WITH` is not aggregating values or using `DISTINCT`, then `ORDER BY` has access to the projected variables from the preceding `RETURN` or `WITH` clause.
...
Also see xref:functions/aggregating.adoc#expression-visibility[Visibility of expressions in WHERE and ORDER BY] for details on expression visibility in the scope of `WHERE` and `ORDER BY`.

.GQLSTATUS error chain
[role="queryresult",stripes="all",cols="1"]
|===
| link:https://neo4j.com/docs/status-codes/current/errors/gql-errors/42N44/[42N44]: error: syntax error or access rule violation - inaccessible variable. It is not possible to access the variable `o` declared before the WITH clause when using `DISTINCT`, an aggregation, or a `GROUP BY` clause.
