...
| xref::planning-and-tuning/query-tuning/using.adoc#query-using-index-hint[USING INDEX variable:Label(property)]
| Hint
| Index hints are used to specify which index, if any, the planner should use as a starting point.

| xref::planning-and-tuning/query-tuning/using.adoc#query-using-index-hint[USING INDEX SEEK variable:Label(property)]
| Hint
| Index seek hint instructs the planner to use an index seek for this clause.

| xref::planning-and-tuning/query-tuning/using.adoc#query-using-join-hint[USING JOIN ON variable]
| Hint
| Join hints are used to enforce a join operation at specified points.

| xref::planning-and-tuning/query-tuning/using.adoc#query-using-scan-hint[USING SCAN variable:Label]
| Hint
...
| xref:syntax/expressions.adoc#type-predicate-expressions[IS :: `type`]
...
| xref::planning-and-tuning/runtimes/concepts.adoc#runtimes-slotted-runtime[CYPHER runtime=slotted]
| Runtime
|
This will cause the query planner to use the slotted runtime.
This is only available in Neo4j Enterprise Edition.

| xref::planning-and-tuning/runtimes/concepts.adoc#runtimes-pipelined-runtime[CYPHER runtime=pipelined]
| Runtime
|
This will cause the query planner to use the pipelined runtime.
This is only available in Neo4j Enterprise Edition.

| xref::planning-and-tuning/runtimes/concepts.adoc#runtimes-parallel-runtime[CYPHER runtime=parallel]
| Runtime
|
This will cause the query planner to use the parallel runtime.
This is only available in Neo4j Enterprise Edition.
|===
