...
image::list_expressions_graph.svg["Example graph of six Person nodes connected by WORKS_FOR relationships, each node carrying name, age, role, and skills properties",width="700",role="middle"]
...
For removing `null` values when concatenating `LIST` values, see xref:expressions/list-expressions.adoc#list-comprehension[].
...
Pattern comprehension can also match for xref:patterns/variable-length-paths.adoc[variable-length patterns].
However, pattern comprehension does not support the xref:patterns/reference/variable-length-paths.adoc#quantifiers[GQL conformant quantifier syntax].
...
Pattern comprehension only supports only the xref:patterns/reference/variable-length-paths.adoc#variable-length-relationships[variable-length relationships] syntax.
