...
The iteration step (`item IN list`) ensures that each element of a `list` is accessed one by one, while the `keyExpression` and `valueExpression` optionally applies a transformation to each `item`, creating new `MAP` of key, value pairs containing the modified elements.
