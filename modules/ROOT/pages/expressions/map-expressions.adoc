...
[role=label--new-Neo4j-2026.10 label--cypher-25-only]
[[map-addition]]
=== Merging `MAP` values

Two `MAP` values can be merged together using the `+` symbol.
They follow a "last update wins" logic for any repeated keys.

.Map addition
// tag::expressions_map_addition[]
[source, cypher]
----
WITH {a: 10, b: 20, c: 30} AS map1, {d: 40, e: 50, f: 60} AS map2
RETURN map1 + map2 AS mergedMaps
----
// end::expressions_map_addition[]
...
If either `MAP` value is `null`, then the result will be `null`:

.Map addition with null value
[source, cypher]
----
WITH {a: 10, b: 20, c: 30} AS map1, null AS map2
RETURN map1 + map2 AS mergedMaps
----
...
If the second map contains keys that are present in the first map, the result will favor the second map's keys using a "last update wins" logic:

.Map addition with matching keys
[source, cypher]
----
WITH {a: 10, b: 20, c: 30} AS map1, {a: 40, b: 50} AS map2
RETURN map1 + map2 AS mergedMaps
----
...
Map comprehension is used to create new `MAP` values by iterating over the elements of an existing `MAP` or `LIST`, and transforming them into key-value pairs.
The result is a new `MAP` where each entry is generated by the specified key and value expressions.

Map comprehension follows a similar syntax to xref:expressions/list-expressions.adoc#list-comprehension[list comprehension].

.Syntax (map variant)
[source, syntax]
----
{key: value IN map [WHERE predicate] | keyExpression : valueExpression}
----

The iteration step (`key: value IN map`) ensures that each key, value pair in a `map` is accessed one by one, while the `keyExpression` and `valueExpression` optionally applies a transformation to each `item`, creating new `MAP` of key, value pairs containing the modified elements.


.Syntax (list variant)
[source, syntax]
----
{item IN list [WHERE predicate] | keyExpression : valueExpression}
----

The iteration step (`item IN list`) ensures that each element of a `list` is accessed one by one, while the `keyExpression` and `valueExpression` apply a transformation to each `item`, creating new `MAP` of key-value pairs containing the modified elements.


.Considerations
|===
| Variables introduced inside a map comprehension (such as `key`, `value`, or `item`) shadow any outer variables with the same name and are not accessible outside the map comprehension.
a| As `MAP` values cannot have duplicate keys, map comprehension follows a "last update wins" logic.
If multiple elements result in the same key, the value from the last element processed will overwrite any previous value for that key.
However, the order in which elements are processed is not guaranteed, so which value is treated as "last" should not be relied upon.
| Unlike list comprehension, the `keyExpression` and `valueExpression` (and the `\|` separator) are not optional.
| The `keyExpression` must evaluate to a `STRING` value or an error will be thrown.
|===

.Simple map comprehension
// tag::expressions_map_comprehension_simple[]
[source, cypher]
----
WITH {a: 1, b: 2, c: 3} AS map
RETURN {k: v IN map | k : v * 10} AS result
----
// end::expressions_map_comprehension_simple[]
...
.Filter a `MAP`
// tag::expressions_map_comprehension_filter[]
[source, cypher]
----
WITH {a: 1, b: 2, c: 3, d: null, e: null} AS map
RETURN {k: v IN map WHERE v IS NOT NULL | k : v} AS result
----
// end::expressions_map_comprehension_filter[]
...
----
WITH {a: 1, b: 2, c: 3} AS map
RETURN {k: v IN map | 'a' : v * 10} AS result
----
...
The value in the result comes from whichever `MAP` entry is processed last (in this case, `{a:30}`).
That entry is not guaranteed, however, because Cypher does not define an order in which `MAP` entries are processed.

.Nested map comprehension
// tag::expressions_map_comprehension_nested[]
[source, cypher]
----
WITH {a: {aa: 1, ab: 2}, b: {ba: 1, bb: 2}} AS nestedMap
RETURN {k: v IN nestedMap | k : {innerK: innerV IN v | innerK : innerV * 10}} AS result
----
// end::expressions_map_comprehension_nested[]
...
| {a: {aa: 10, ab: 20}, b: {ba: 10, bb: 20}}

1+d|Rows: 1
|===

.Simple map comprehension (list variant)
// tag::expressions_map_comprehension_simple_list[]
[source, cypher]
----
WITH ['a', 'b', 'c'] AS list
RETURN {item IN list | item : item + '!' } AS result
----
// end::expressions_map_comprehension_simple_list[]
...
.Filter a `LIST` and transform into a `MAP`
// tag::expressions_map_comprehension_filter_list[]
[source, cypher]
----
WITH range(0, 10) AS list
RETURN {i IN list WHERE i % 2 = 0 | toString(i) : i * 10} AS result
----
// end::expressions_map_comprehension_filter_list[]
