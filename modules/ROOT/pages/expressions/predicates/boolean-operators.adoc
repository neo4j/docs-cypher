...
[NOTE]
====
Operator precedence defines how Cypher groups expressions when parentheses are not present.
Within predicates, the following order is relevant (higher precedence first):
...
* `NOT true = false` is parsed as `NOT (true = false)` and is valid.
* `true = NOT false` does not parse. Use `true = (NOT false)` if that is your intent.
* `true AND NOT false` is parsed as `true AND (NOT false)`.
