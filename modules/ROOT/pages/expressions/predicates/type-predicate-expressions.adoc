...
|===
| link:https://neo4j.com/docs/status-codes/current/errors/gql-errors/42N63/[42N63]: error: syntax error or access rule violation - inner type with different nullability. All inner types in a Closed Dynamic Union must be nullable, or be appended with 'NOT NULL'.
...
[NOTE]
Sized numeric values (such as `INT8`, `INT16`, `FLOAT32`) are not valid in type predicate expressions testing multiple types.
Only the base types `INTEGER` and `FLOAT` are supported. 
For example, `RETURN 1 IS :: FLOAT | INTEGER` is valid, but `RETURN 1 IS :: FLOAT | INT8` is not.
