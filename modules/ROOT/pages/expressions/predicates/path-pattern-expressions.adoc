...
* Path pattern expressions may only use a subset of xref::patterns/reference/path-patterns-and-graph-patterns.adoc#graph-patterns[graph pattern] semantics.

* A path pattern expression must be a xref::patterns/reference/path-patterns-and-graph-patterns.adoc#path-patterns[path pattern] of length greater than zero.
In other words, it must contain at least one xref::patterns/reference/node-and-relationship-patterns.adoc#relationship-patterns[relationship] or xref::patterns/reference/variable-length-paths.adoc#variable-length-relationships[variable-length relationship].
...
`(p:Person)` is not a path pattern expression since it does not have at least one xref::patterns/reference/node-and-relationship-patterns.adoc#relationship-patterns[relationship] or xref::patterns/reference/variable-length-paths.adoc#variable-length-relationships[variable-length relationship].
...
`(p)` is not a path pattern expression since it does not have at least one xref::patterns/reference/node-and-relationship-patterns.adoc#relationship-patterns[relationship] or xref::patterns/reference/variable-length-paths.adoc#variable-length-relationships[variable-length relationship].
