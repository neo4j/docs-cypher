...
If a property (or property value) is missing in an expression using a graph element operator, the whole expression will evaluate to `NULL`.
The below query performs a xref:expressions/string-operators.adoc[string concatentation] on the `firstName`, `middleName`, and `lastName` properties on `Person` nodes.
