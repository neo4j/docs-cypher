...
----
CREATE
  (alice:Person {name:'Alice', age: 65, email: 'alice@company.com', occupation: 'Project manager'}),
  (cecil:Person {name: 'Cecil', age: 25, email: 'bob@company.com', occupation: 'Software developer'}),
  ...
  (daniel:Person {name: 'Daniel', age: 39, email: 'daniel@company.com', occupation: 'Director'}),
  (eskil:Person {name: 'Eskil', age: 39, email: 'eskil@private.se'}),
...
The predicate boolean operators comprise:
...
The predicate comparison operators comprise:
...
| "Cecil" | "bob@company.com"
| "Daniel" | "daniel@company.com"
| "Eskil" | "eskil@private.se"
...
The predicate string operators comprise:
...
Attempting to use them operators on values which are not `STRING` values will return `null`.
...
----
MATCH (n:Person)
WHERE n.occupation ENDS WITH 'developer'
RETURN n.name AS name, n.occupation AS occupation
----
...
----
MATCH (n:Person)
WHERE n.occupation CONTAINS 'eng'
RETURN n.name AS name, n.occupation AS occupation
----
...
The names of both `Cecil` and `Cecilia` are returned because their name starts with `'CEC'` irrespective of casing:
...
`Eskil` is returned because his email ends with ".se":
