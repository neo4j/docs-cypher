...
[source, cypher]
----
MATCH (manager:Person {name: 'Alice'}),
      (employee:Person)
WHERE (employee)-[:WORKS_FOR]->(manager)
...
.Path pattern expressions including boolean operators
[source, cypher]
----
...
For more information about Cypher's boolean operators, see xref:expressions/predicate-expressions/operators.adoc#boolean-operators[Predicate expressions -> Boolean operators].
