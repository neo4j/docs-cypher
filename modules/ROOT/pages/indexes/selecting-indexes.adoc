...
= Selecting search-performance indexes

It is important to have a fundamental understanding of how indexes operate before knowing which to select in order to optimize query performance.
This section first discusses which predicates can be solved by different indexes, and then it describes the query plans that result from different index scenarios.
...
For instructions on how to create and maintain indexes, refer to xref:indexes/indexes-for-search-performance.adoc[].
...
See xref:indexes/indexes-for-full-text-search.adoc[].
====
...
But in these cases rewrites might happen depending on which properties have which predicates, see xref:indexes/indexes-for-search-performance.adoc#indexes-single-vs-composite-index[composite index limitations].
...
However, predicates after the range predicate may be rewritten as an existence check predicate and a filter as described in xref:indexes/indexes-for-search-performance.adoc#indexes-single-vs-composite-index[composite index limitations].
...
More information about how the rewriting works can be found in xref:indexes/indexes-for-search-performance.adoc#indexes-single-vs-composite-index[composite index limitations].
