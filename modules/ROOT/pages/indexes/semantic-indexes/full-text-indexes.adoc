...
// Is the below still true with the changes?

Unlike xref:indexes/search-performance-indexes/index.adoc[], full-text indexes are not automatically used by the xref:planning-and-tuning/execution-plans.adoc[Cypher query planner].
...
The same can be achieved with the following query which uses the `db.index.fulltext.queryNodes()` procedure:

.Query a full-text index for a node property label:deprecated[Deprecated in Neo4j 2026.04]
...
.Query a full-text index for a relationship property label:deprecated[Deprecated in Neo4j 2026.04]
[source, cypher]
----
...
.Query a full-text index for exact matches label:deprecated[Deprecated in Neo4j 2026.04]
[source, cypher]
----
...
.Query a full-text index for specific properties label:deprecated[Deprecated in Neo4j 2026.04]
[source, cypher]
----
...
.Query a full-text index for content present in a list of `STRING` properties label:deprecated[Deprecated in Neo4j 2026.04]
[source, cypher]
----
