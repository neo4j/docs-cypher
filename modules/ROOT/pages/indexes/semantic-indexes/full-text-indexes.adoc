...
Unlike xref:indexes/search-performance-indexes/create-indexes.adoc#create-range-index[range] and xref:indexes/search-performance-indexes/create-indexes.adoc#create-text-index[text] indexes, which can only perform limited `STRING` matching (exact, prefix, substring, or suffix matches), full-text indexes stores individual words in any given `STRING` property.
...
// tag::indexes_semantic_fulltext_create[]
[source, cypher]
----
CREATE FULLTEXT INDEX namesAndTeams IF NOT EXISTS
FOR (n:Employee|Manager) ON EACH [n.name, n.team]
----
// end::indexes_semantic_fulltext_create[]

This query highlights two key differences between full-text and xref:indexes/search-performance-indexes/index.adoc[]:
...
The link:{neo4j-docs-base-uri}/operations-manual/current/procedures/built-in-procedures/#procedure_db_index_fulltext_listAvailableAnalyzers[`db.index.fulltext.listAvailableAnalyzers()`] procedure shows all available analyzers.
...
// tag::indexes_semantic_fulltext_create_options[]
[source, cypher]
----
...
----
// end::indexes_semantic_fulltext_create_options[]
...
Unlike search-performance xref:indexes/search-performance-indexes/create-indexes.adoc#create-text-index[text indexes], full-text indexes capture the semantic meaning or context of the data in a database.
...
As of Neo4j 2026.09, the preferred way of querying full-text indexes is by using the xref::clauses/search.adoc[SEARCH] clause.
As an alternative, use either the link:{neo4j-docs-base-uri}/operations-manual/current/procedures/built-in-procedures/#procedure_db_index_fulltext_queryNodes[`db.index.fulltext.queryNodes()`] or the link:{neo4j-docs-base-uri}/operations-manual/current/procedures/built-in-procedures/#procedure_db_index_fulltext_queryRelationships[`db.index.fulltext.queryRelationships()`] procedure.
...
=== Querying a full-text index using the SEARCH clause

This query uses the previously created full-text index `namesAndTeams` to look for "nils" among the employee and manager nodes:
...
[[query-full-text-procedure]]
=== Querying a full-text index using the db.index.vector.queryNodes() procedure

This query uses the previously created full-text index `namesAndTeams` to look for "nils" among the employee and manager nodes:

.Query a full-text index for a node property
// tag::indexes_semantic_fulltext_query[]
[source, cypher]
----
CALL db.index.fulltext.queryNodes("namesAndTeams", "nils") YIELD node, score
RETURN node.name, score
----
// end::indexes_semantic_fulltext_query[]
...
=== Querying a relationship full-search index

To query a relationship full-text index, use the Cypher xref::clauses/search.adoc[SEARCH] clause or the link:{neo4j-docs-base-uri}/operations-manual/current/procedures/built-in-procedures/#procedure_db_index_vector_queryRelationships[`db.index.vector.queryRelationships`] procedure in a similar manner as shown for node full-text indexes above.
...
Query strings also support the use of the link:https://lucene.apache.org/core/10_5_1/queryparser/org/apache/lucene/queryparser/classic/package-summary.html#Boolean_operators[Lucene boolean operators] (`AND`, `OR`, `NOT`, `+`, `-`):
...
A complete description of the Lucene query syntax can be found in the link:https://lucene.apache.org/core/10_5_1/queryparser/org/apache/lucene/queryparser/classic/package-summary.html#package.description[Lucene documentation].
...
To show all full-text indexes in a database, use the `SHOW FULLTEXT INDEXES` command:

.Show all full-text indexes in a database
// tag::indexes_semantic_fulltext_show[]
[source, cypher, test-exclude-cols=id]
----
SHOW FULLTEXT INDEXES
----
// end::indexes_semantic_fulltext_show[]
...
For a full description of all return columns, see xref:indexes/search-performance-indexes/list-indexes.adoc#show-indexes-result-columns[Search-performance indexes -> Show indexes -> Return columns].


[[drop-full-text-indexes]]
== Drop full-text indexes

A full-text node index is dropped by using the xref:indexes/search-performance-indexes/drop-indexes.adoc[same command as for other indexes], `DROP INDEX`.

In the following example, the previously created `communications` full-text index is deleted from the database:

.Drop a full-text index
// tag::indexes_semantic_fulltext_drop[]
[source, cypher]
----
DROP INDEX communications
----
// end::indexes_semantic_fulltext_drop[]
...
| https://neo4j.com/docs/operations-manual/current/procedures/built-in-procedures/#procedure_db_index_fulltext_awaitEventuallyConsistentIndexRefresh[`db.index.fulltext.awaitEventuallyConsistentIndexRefresh()`]
| Wait for the updates from recently committed transactions to be applied to any eventually-consistent full-text indexes.

| List available analyzers.
| https://neo4j.com/docs/operations-manual/current/procedures/built-in-procedures/#procedure_db_index_fulltext_listAvailableAnalyzers[`db.index.fulltext.listAvailableAnalyzers()`]
| List the available analyzers that the full-text indexes can be configured with.

| Use full-text node index.
| https://neo4j.com/docs/operations-manual/current/procedures/built-in-procedures/#procedure_db_index_fulltext_queryNodes[`db.index.fulltext.queryNodes()`]
...
| https://neo4j.com/docs/operations-manual/current/procedures/built-in-procedures/#procedure_db_index_fulltext_queryRelationships[`db.index.fulltext.queryRelationships()`]
