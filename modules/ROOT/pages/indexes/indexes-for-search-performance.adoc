...
In combination with node label and relationship type predicates, text indexes only solve predicates operating on `STRING` values.
This means that text indexes are only used when it is known that the predicate evaluates to `null` for all non-string values.
...
In combination with node label and relationship type predicates, point indexes only solve predicates operating on `POINT` values.
