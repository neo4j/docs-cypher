...
.Dropping an index
// tag::indexes_search_performance_drop[]
[source, cypher]
----
DROP INDEX example_index
----
// end::indexes_search_performance_drop[]
...
.Dropping an index
// tag::indexes_search_performance_drop_parameter[]
[source, cypher]
----
DROP INDEX $name
----
// end::indexes_search_performance_drop_parameter[]
...
link:https://neo4j.com/docs/status-codes/current/errors/gql-errors/50N10/[50N10]: error: general processing exception - index drop failed. Unable to drop 'uniqueBookIsbn'.
|===

Dropping the index-backed constraint also removes the backing index.
For more information, see xref:schema/constraints/drop-constraints.adoc[Drop a constraint by name].
