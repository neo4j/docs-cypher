...
This means that a text index is not able to solve, for example, e.g. `a.prop = b.prop`, unless a xref:constraints/type.adoc[type constraint] also exists on the property.
...
For more information, see xref:constraints/list-drop-constraints.adoc#drop-constraint[Drop a constraint by name].
