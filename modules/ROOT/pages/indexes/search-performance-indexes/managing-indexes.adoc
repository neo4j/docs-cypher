...
This means that a text index is not able to solve, for example, e.g. `a.prop = b.prop`, unless a xref:schema/constraints.adoc#create-property-type-constraints[property type constraint] also exists on the property.
...
For more information, see xref:schema/constraints.adoc#drop-constraint[Drop a constraint by name].
