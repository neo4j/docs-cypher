...
=== Text indexes and dictionary variables

Text indexes are not used to solve predicates with dictionary variables, such as JSON.
For example, the following predicate would not be solved with a text index: 
...
This is because Cypher does not store type information for `MAP` values, which means the planner cannot prove that any given value is a `STRING`.
