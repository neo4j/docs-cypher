...
Since xref:constraints/managing-constraints.adoc#create-type-constraint[type constraints] guarantee that a property is always of the same type, they can be used to extend the scenarios in which text and point indexes are compatible with a predicate.
...
Note that xref:constraints/managing-constraints.adoc#create-existence-constraint[property existence constraints] do not currently leverage index use in the same way.
