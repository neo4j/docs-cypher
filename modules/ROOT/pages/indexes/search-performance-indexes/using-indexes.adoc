...
If any unused indexes are identified, it may be beneficial to delete them using the xref:indexes/search-performance-indexes/drop-indexes.adoc#drop-indexes[`DROP INDEX`] command.
