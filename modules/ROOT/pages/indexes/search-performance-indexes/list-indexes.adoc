...
The returned statistics are local to each instance and will differ for each member in a cluster.
label:default-output[]

| `ZONED DATETIME`

| `readCount`
| The number of read queries that have been issued to this index since `trackedSince`, or `null` if the statistics are not tracked.
The returned statistics are local to each instance and will differ for each member in a cluster.
label:default-output[]
