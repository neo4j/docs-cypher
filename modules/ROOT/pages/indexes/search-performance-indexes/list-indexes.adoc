...
* xref:indexes/search-performance-indexes/list-indexes.adoc#show-all-indexes[]
* xref:indexes/search-performance-indexes/list-indexes.adoc#specific-return-columns[]
* xref:indexes/search-performance-indexes/list-indexes.adoc#show-indexes-with-filtering[]
...
To show all indexes with the default output columns, the `SHOW INDEXES` command can be used.
If all columns are required, use `SHOW INDEXES YIELD *`.

.Show all indexes
// tag::indexes_search_performance_show[]
[source, cypher, role=test-result-skip]
----
SHOW INDEXES
----
// end::indexes_search_performance_show[]
...
.Show range indexes
// tag::indexes_search_performance_show_type_filter[]
[source, cypher, role=test-result-skip]
----
SHOW RANGE INDEXES WHERE owningConstraint IS NULL
----
// end::indexes_search_performance_show_type_filter[]
...
For vector indexes the vector property is the first element of the list, followed by the additional properties for filtering. label:default-output[]
| `LIST<STRING>`
...
| `options`
| Information retrieved from the `OPTIONS` map about the configuration settings for an index.
If neither is specified when creating the index, this column returns the default values.
| `MAP`
