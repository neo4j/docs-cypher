...
[NOTE]
A node element type requires either one or more implied labels or a property map.
...
[source, syntax]
-----
([nodeAlias | [nodeAlias] {IS | :} LabelName [=> | IMPLIES]])-”[“ [alias] {IS | :} IDENTIFYING_RELATIONSHIP_TYPE {=> | IMPLIES} [“{“ propertyName [[IS] :: | IS TYPED] <TYPE> [IS [REL | RELATIONSHIP] {KEY | UNIQUE}][, ...] “}”] “]”->([nodeAlias | [nodeAlias] {IS | :} LabelName [=> | IMPLIES]])
...
[[constraints-on-identifying-and-non-identifying-labels-and-types]]
=== Constraints on identifying and non-identifying node labels and relationship types
...
|===
| Property uniqueness and key constraints on node element types
a|
[source, syntax]
----
...
| Property uniqueness and key constraints on relationship element types
a|
[source, syntax]
----
CONSTRAINT [constraint_name]
...
* `DURATION`
* `POINT`
* `VECTOR<TYPE NOT NULL>(DIMENSION)` (where the inner `TYPE` is a valid coordinate type for vectors and `DIMENSION` is an `INTEGER` in (`[0, 4096]`))
...
[NOTE]
Any added element types, constraints on identifying node labels/relationship types, and constraints on non-identifying node labels/relationship types must, together with the existing ones, form a valid graph type.
...
[NOTE]
====
Altering a graph type requires the following privileges:
...
For more information, including full details of the return columns returned by this command, see xref:schema/graph-types/list-graph-types.adoc[].

[[drop-graph-type-elements]]
=== Drop elements from a graph type
...
For examples on how to create property uniqueness constraints, see  xref:schema/constraints/create-constraints.adoc#create-property-uniqueness-constraints[Create, show, and drop constraints -> Create property uniqueness constraint].
...
For examples on how to create property existence constraints, see  xref:schema/constraints/create-constraints.adoc#create-property-existence-constraints[Create, show, and drop constraints -> Create property existence constraints].
...
For examples on how to create property type constraints, see  xref:schema/constraints/create-constraints.adoc#create-property-type-constraints[Create, show, and drop constraints -> Create property type constraints].
...
For examples on how to create key constraints, see  xref:schema/constraints/create-constraints.adoc#create-key-constraints[Create, show, and drop constraints -> Create key constraints].
...
| NODE EXIST[ENCE]
| Returns node property existence and node label existence constraints.

| EXIST[ENCE]
| Returns node and relationship property existence constraints, and node label existence constraints.
...
For examples on how to list constraints, see  xref:schema/constraints/list-constraints.adoc[Show constraints -> SHOW CONSTRAINTS].
For full details of the result columns for the `SHOW CONSTRAINTS` command, see xref:schema/constraints/list-constraints.adoc#list-constraints-result-columns[Create, show, and drop constraints -> Result columns for listing constraints].
...
For examples on how to drop constraints, see  xref:schema/constraints/drop-constraints.adoc#drop-constraint[Create, show, and drop constraints -> DROP CONSTRAINT].