...
[role=label--cypher-25-only label--enterprise-edition label--new-Neo4j-2026.02]
[[graph-types]]
== Graph types
...
* `DURATION`
* `POINT`
* `UUID` label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.mm]
...
For full details of the columns returned by the `SHOW CURRENT GRAPH TYPE` command, see xref:schema/graph-types/list-graph-types.adoc#result-columns[Show graph types -> Return columns].
From 2026.05, it is possible to combine `SHOW CURRENT GRAPH TYPE` with general Cypher clauses in a single query.
`AS GRAPH` was introduced in Neo4j 2026.06.
...
For full details of the returned columns for the `SHOW CONSTRAINTS` command, see xref:schema/constraints/list-constraints.adoc#show-constraints-result-columns[Show constraints -> Return columns].
