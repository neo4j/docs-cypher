...
The `REQUIRE` keyword can also be used as an alternaConstraints on nontive syntax to define several property uniqueness and key constraints within the same node or relationship element type.
...
For more information, see xref:#constraints-on-non-identifying-labels-and-types[Constraints on non-identifying node labels and relationship types] below.
