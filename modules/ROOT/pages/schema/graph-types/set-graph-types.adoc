...
These categories are discussed in the section xref:schema/graph-types/set-graph-types.adoc#set-graph-type-empty-database[Set a graph type on an empty database].

Graph types can also include xref:schema/graph-types/set-graph-types.adoc#constraints-on-non-identifying-labels-and-types[property existence, property type, property uniqueness, and key constraints] defined on non-identifying node labels or relationship types.
These are discussed in the section xref:schema/graph-types/set-graph-types.adoc#set-graph-type-populated-database[Set a graph type on a populated database].
...
.Updated graph type, showing addition of key constraints on identifying node labels
image::graph-types-element-type-constraints.svg[Image of updated graph type, showing addition of key constraints on identifying node labels, width=700,role=popup]
...
.Add key and property uniqueness constraints on non-identifying node labels in a graph type
[source, cypher]
----
ALTER CURRENT GRAPH TYPE SET {
(p:Person => :Resident {name :: STRING, ssn :: INTEGER})
...
.Updated graph type showing new `Robot` node element type and addition of constraints on non-identifying node labels
...
To learn more about key constraints, and how to create them using the `CREATE CONSTRAINT` command, xref:schema/constraints/create-constraints.adoc#create-key-constraints[Constraints -> Create key constraints].
