...
| `nodes`
| A list of virtual nodes representing the node element types, node constraints not belonging to any node element type, and nodes used to represent empty endpoints on relationships. label:default-output[]
| `LIST<NODE NOT NULL>`
...
* *_Node constraint virtual nodes_* represent node labels that are not identifying a node element type.
...
Node constraint virtual nodes all have the label `NodeLabel`, with their information stored in their properties.
...
.Node constraint virtual nodes properties
[cols="3*", options="header"]
|===
| Property
| Description
...
For more information, see xref:schema/graph-types/list-graph-types.adoc#virtual-nodes[Show graph types -> Virtual nodes] and ref:schema/graph-types/list-graph-types.adoc#virtual-relationships[Virtual relationships].
