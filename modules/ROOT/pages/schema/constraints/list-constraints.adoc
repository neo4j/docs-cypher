...
To show all constraints with the default output columns, use `SHOW CONSTRAINTS`.
...
The constraints shown on this page are created on the page xref:schema/constraints/create-constraints.adoc[].
...
For more information, see the following sections of the xref:schema/graph-types/list-graph-types.adoc[Show graph types] page:
...
* xref:schema/constraints/list-constraints.adoc#show-all-constraints[]
* xref:schema/constraints/list-constraints.adoc#return-specific-columns[]
* xref:schema/constraints/list-constraints.adoc#show-constraints-with-filtering[]
* xref:schema/constraints/list-constraints.adoc#composable-show-constraints[]

[[show-all-constraints]]
=== Show all constraints

To show all constraints with the default output columns, use the `SHOW CONSTRAINTS`command.
If all columns are required, use `SHOW CONSTRAINTS YIELD *`.

.Show all constraints and return default columns
...
.Show all constraints and return all columns
[source, cypher, test-exclude-cols=id]
----
...
.Show only the `name`, `type`, and `createStatement` columns
[source, cypher, test-exclude-cols=id]
----
SHOW CONSTRAINTS
...
[[show-constraints-with-filtering]]
=== Show constraints with filtering
...
.Filter `SHOW CONSTRAINTS` with `WHERE` to show only constraints with a `RELATIONSHIP` `entityType`
[source, cypher, test-exclude-cols=id]
----
SHOW CONSTRAINTS
...
[[show-constraints-result-columns]]
== Return columns

.show constraints output
...
| enforcedLabel
| The implied labels required by a node label existence constraint or the source node and target node labels required by a relationship source or target label constraint, or `null` for other constraints which do not constrain labels. label:default-output[] label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.02]
| STRING
...
| classification
| The status of a constraint in a graph type (`dependent`, `undesignated`, or `independent`). label:cypher[Cypher 25 only] label:new[Introduced in Neo4j 2026.02]
| STRING
