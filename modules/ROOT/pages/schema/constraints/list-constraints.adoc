...
.Show all constraints with default output columns
======
...
.Show only specific constraint types
======
...
.Show only constraints with a `RELATIONSHIP` `entityType`
[source, cypher, test-exclude-cols=id]
----
SHOW CONSTRAINTS
...
== Result columns for show constraints
