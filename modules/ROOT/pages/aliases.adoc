...
The composite database name and the database alias name parts need to be escaped individually.
Create a database alias named `my alias with spaces` as a constituent in the composite database named `my-composite-database-with-dashes`:
...
Database alias names may also include dots. Though these always need to be escaped, to avoid ambiguity with the composite database and database alias split character.
...
When a composite database `some` exists, the query below will create a database alias named `alias` within the composite database `some`. When no such database exists however, the same query will instead create a database alias named `some.alias`
...
Consider the same query but with parameter:
...
Since the first dot will be used as a divider, the command will attempt to create database alias `withdot.myalias` in composite database `mycompositedatabase`.  If `mycompositedatabase` doesn't exist, the command will create a database alias with the name `mycompositedatabase.withdot.myalias`, which is not part of any composite database.
...
Even more special handling with parameters are needed for database aliases and similarly named composite databases.

Consider the set-up:

.Query
[source, cypher, indent=0, role="noheader"]
----
...
Dropping this alias using parameters will fail with a misleading error about missing alias:
