...
.+CREATE ALIASES FOR COMPOSITE DATABASES+
======
...
.+SHOW ALIASES FOR COMPOSITE DATABASES+
======
...
// Query fails with `key not found: org.neo4j.cypher.docgen.tooling.ErrorOnlyTablePlaceHolder@4f1502d7`if it is placed inside the note 
...
When a database alias has been deleted, it will no longer show up in the `name` column provided by the command `SHOW ALIASES FOR DATABASE`.
