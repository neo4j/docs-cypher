...
To list just one database alias, the `SHOW ALIASES` command an alias name;
...
[source, cypher, indent=0]
----
CREATE ALIAS `remote-northwind-2021` FOR DATABASE FOR DATABASE `northwind-graph-2021` AT 'neo4j+s://location:7687'
...
[source, cypher, indent=0]
----
SHOW DATABASE SHOW ALIASES FOR DATABASE WHERE name STARTS WITH 'garden'
----
