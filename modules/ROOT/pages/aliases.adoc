...
Most of the name restrictions apply only when creating a database alias, but the escaping rules apply to all the different database alias commands.
...
Database alias names may also include dots, though these always needs to be escaped when more than one.
...
The query below will create a local database alias named `my.alias`.

.Query
[source, cypher, indent=0]
----
CREATE ALIAS my.alias FOR DATABASE `northwind-graph`
----
...
Dropping this alias using parameters will fail with a confusing error on missing alias:
