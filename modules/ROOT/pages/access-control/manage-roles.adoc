...
This returns a single `STRING` column, `role`, containing the role name.
...
It will give a result with one row for each user, so if a role is assigned to two users, then it will show up twice.
