...
[abstract]
--
This chapter explains how to manage Neo4j role-based access control and fine-grained security.
--
...
The original built-in roles still exist with almost the exact same access rights, but they are no-longer statically defined (see <<access-control-built-in-roles,Built-in roles>>).
