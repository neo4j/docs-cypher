...
The <<access-control-built-in-roles-admin,`admin` role>> has a number of built-in privileges.
These include:
...
* <<access-control-database-administration-transaction, Transaction management>>
...
Note that this role doesn't allow all DBMS capabilities.
...
In order to use `SHOW ROLES WITH USERS` and `SHOW POPULATED ROLES WITH USERS` administration commands, both this and the `SHOW USER` privileges are required.
...
A user with this privilege is allowed to execute the procedures matched by the <<access-control-name-globbing, name-globbing>>.
...
Both `dbms.killTransaction` and `dbms.killTransactions` are blocked here, as well as any other procedures starting with `dbms.killTransaction`.
...
A user with this privilege is allowed to execute the procedures matched by the <<access-control-name-globbing, name-globbing>> without the execution being restricted to their other privileges.
...
Users with the role `boostedProcedureExecutor` can thus run `db.labels` and `db.relationshipTypes` with full privileges.
...
It also prevents `db.labels` from being elevated.
...
This privilege is equivalent to granting the <<access-control-execute-boosted-procedure,`EXECUTE BOOSTED PROCEDURE` privilege>> on each of the admin procedures.
...
A user with this privilege is allowed to execute the UDFs matched by the <<access-control-name-globbing, name-globbing>>.
...
A user with this privilege is allowed to execute the UDFs matched by the <<access-control-name-globbing, name-globbing>> without the execution being restricted to their other privileges.
...
This is the same behavior as for the <<access-control-execute-boosted-procedure,`EXECUTE BOOSTED PROCEDURE` privilege>>.
...
[NOTE]
====
The name-globbing is subject to the <<cypher-manual#cypher-naming, standard Cypher restrictions on valid identifiers>>,
