...
The DBMS privileges for querying configuration settings can be assigned by using Cypher administrative commands.
They can be granted, denied and revoked like other privileges.
...
The resulting role has privileges that only allow querying settings in the `server.bolt` namespace.
...
In order to allow the querying all but only a few settings, you can grant `SHOW SETTINGS *` and deny the unwanted settings.
...
Access to any setting starting with `dbms.security` are blocked here, but the rest can still be queried.
...
== Procedure, user-defined function and setting name-globbing
...
The examples below only use procedures, but the same rules apply to user defined function and setting names:
