...
[NOTE]
====
When a server is enabled the default server tags are determined by the setting `initial.server.tags`.
====
...
| tags
| list of server tags
| List of server tags used during database allocation and for load balacing and routing policies. 
|===
