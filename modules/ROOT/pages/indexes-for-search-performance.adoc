...
* xref::indexes-for-search-performance.adoc#indexes-create-a-range-index-by-param[]
...
[discrete]
[[indexes-create-a-range-index-by-param]]
===== Create a range index using a parameter

The following statement will create a named range index on all nodes labeled with `Person` and which have the `firstname` property using a parameter for the index name.
...
* xref::indexes-for-search-performance.adoc#indexes-create-a-text-index-by-param[]
...
[discrete]
[[indexes-create-a-text-index-by-param]]
===== Create a text index using a parameter

The following statement will create a named text index on all nodes labeled with `Person` and which have the `favoriteColor` string property using a parameter for the index name.
...
* xref::indexes-for-search-performance.adoc#indexes-create-a-point-index-by-param[]
...
[discrete]
[[indexes-create-a-point-index-by-param]]
...
If it is not known whether an index exists or not, add `IF NOT EXISTS` to ensure it does.
Giving the name directly or through a parameter makes no difference.
...
* xref::indexes-for-search-performance.adoc#indexes-drop-an-index-by-param[]
...
[discrete]
[[indexes-drop-an-index-by-param]]
