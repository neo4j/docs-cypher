...
This example uses a xref:patterns/variable-length-paths.adoc#quantified-relationships[quantified relationship] to find all paths up to `5` hops away, traversing only relationships of type `KNOWS` from the start node `Anna` to other younger `Person` nodes (as defined by the xref:clauses/where.adoc[] clause).
