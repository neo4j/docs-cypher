...
The link:{neo4j-docs-base-uri}/operations-manual/configuration/configuration/configuration-settings/[configuration settings] specified in the `OPTIONS` map (which can, for example, be used when xref:indexes/semantic-indexes/vector-indexes.adoc#create-vector-index[creating a vector index]) are tied to a Neo4j version rather than a Cypher version, meaning updates to these settings will be supported in both Cypher 5 and Cypher 25.
