...
Default in 2026.02 and forward is 2026.02. //TODO: clean up the following paragraph

////
Update documentation
To say Cypher 25 is the default
Include release note
Cypher 25 is the new default query language for new installations.
The new default query language applies to new databases created. Existing databases will keep their default language.
The default query language attached to created databases is determined by the setting `
db.query.default_language=CYPHER_5|CYPHER_25`
New deployments, which use the template neo4j.conf, will get the new default query language.
Upgraded deployments, where neo4j.conf is retained, will keep their current default query language.
Docker deployments, which use the neo4j.conf included in the image, will get the new default query language, while the ones that reuse the existing neo4j.conf, will keep the existing default.
