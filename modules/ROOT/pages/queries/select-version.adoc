...
=== Neo4j 2026.02 or later

Databases created on Neo4j 2026.02 or later explicitly set Cypher 25 as their query language in the _neo4j.conf_ file: `db.query.default_language=CYPHER_25`.
Existing databases retain their _neo4j.conf_ file.
For both, a different query language version can be set with the same setting link:{neo4j-docs-base-uri}/operations-manual/current/configuration/configuration-settings/#config_db.query.default_language[`db.query.default_language`].

[NOTE]
====
If the setting is missing, it defaults to `db.query.default_language=CYPHER_5`.
====
...
[[options-map]]
=== Cypher versions and the `OPTIONS` map

The link:{neo4j-docs-base-uri}/operations-manual/configuration/configuration/configuration-settings/[configuration settings] specified in the `OPTIONS` map (which can, for example, be used when xref:indexes/semantic-indexes/vector-indexes.adoc#create-vector-index[creating a vector index]) are tied to a Neo4j version rather than a Cypher version, meaning updates to these settings will be supported in both Cypher 5 and Cypher 25 unless explicitly specified otherwise.
