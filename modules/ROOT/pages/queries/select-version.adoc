...
For more information about the new, updated, and removed features included in Cypher 25, see the link:https://neo4j.com/docs/cypher-manual/25/deprecations-additions-removals-compatibility/[Additions, deprecations, removals, and compatibility] page for Cypher 25.
...
For more information about Cypher 25, see the link:https://neo4j.com/docs/cypher-manual/25/introduction/[Cypher 25 Manual].
...
Cypher 5 users must migrate to Cypher 25 in order to access new features.
...
For more information about how to change and view the default language of new and existing databases, as well as remote database aliases, see the following links in the Operations Manual:

* link:{neo4j-docs-base-uri}/operations-manual/current/database-administration/standard-databases/create-databases/#_create_a_database_with_default_language[Create a standard database with a `DEFAULT LANGUAGE`]
* link:{neo4j-docs-base-uri}/operations-manual/current/database-administration/composite-databases/create-composite-databases/#composite-databases-default-language[Create a composite database with a `DEFAULT LANGUAGE`]
* link:{neo4j-docs-base-uri}/operations-manual/current/database-administration/aliases/manage-aliases-standard-databases/#_set_a_default_cypher_version_for_remote_database_aliases[Create a remote database alias with a `DEFAULT LANGUAGE`]
* link:{neo4j-docs-base-uri}/operations-manual/current/database-administration/standard-databases/alter-databases/#alter-database-default-language[Alter the `DEFAULT LANGUAGE` of a database]
* link:{neo4j-docs-base-uri}/operations-manual/current/database-administration/aliases/manage-aliases-standard-databases/#_alter_the_default_cypher_version_of_a_remote_database_alias[Alter the `DEFAULT LANGUAGE` of a remote database alias]
* link:{neo4j-docs-base-uri}/operations-manual/current/database-administration/standard-databases/listing-databases/#_show_the_default_cypher_version_of_a_database[Show the `DEFAULT LANGUAGE` of a database]
...
[NOTE]
The option to prepend a query with `CYPHER 5` was introduced in Neo4j 5.21, while the `CYPHER 25` option was introduced in Neo4j 2025.06.
