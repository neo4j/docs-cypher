...
`WHEN` on the other hand enables conditional queries, where different query branches can be made to execute depending on a set of criteria.
Finally, `NEXT` allows for linear composition of queries into a sequence, while making return values from one query available in the next.
