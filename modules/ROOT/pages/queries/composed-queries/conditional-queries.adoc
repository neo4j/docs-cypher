...
image::conditional_query_graph.svg[width="600",role="middle"]
...
The following example demonstrates this logic:
...
`WHEN` can also not positioned as a regular clause in a query.
...
Instead, if `WHEN` constructs are part of a larger query, they must be either be placed within a subquery and/or on different sides of combined `UNION` queries.

== Conditional subqueries

`WHEN` can be used inside one or several chained xref:subqueries/index.adoc[subqueries] to execute a set of operations only when a specified condition evaluates to `true`.
...
It then collects the names and age grouping of their managers (who they `WORK_FOR`).
...
If the queries combined by the `UNION` does not begin with a `WHEN` branch, then enclosing curly braces are not necessary (though they can still be used).
...
| "Charlie" | ["Loves somebody", "Older person"]
