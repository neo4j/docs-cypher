...
.Syntax for conditional subqueries
[source, syntax]
----
[<outerScope>]
<conditionalSubquery> {
...
If the condition evaluates to `true`, the subquery fetches people that the `person` knows; otherwise, it retrieves who the `person` loves.
[NOTE]
...
Disjointed conditional subqueries be chained in succession, each executing depending on a set of defined conditions.
...
.Syntax for combining conditional subqueries with `UNION`
[source, syntax]
----
[<outerScope>]
<conditionalSubquery> {
