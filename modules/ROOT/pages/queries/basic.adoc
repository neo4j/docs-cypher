...
In this example, `ALL SHORTEST` paths are found between the two nodes `Keanu Reeves` and `Tom Cruise`.
The xref:functions/aggregating.adoc#functions-count[`count()`] function is then used to calculate the number of shortest paths returned and the xref:functions/scalar.adoc#functions-length[`length()`] function is used to calculate the length (in terms of traversed relationships) of those paths.
...
The results show that 2 different shortest paths exist between the two nodes.
