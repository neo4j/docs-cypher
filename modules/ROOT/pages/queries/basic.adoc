...
The returbed table shows that he has 13 outgoing relationships connected to 12 different `Movie` nodes (12 have the `ACTED_IN` type and one has the `DIRECTED` type).
...
For example, the below query uses a `NOT` label expression (`!`) to return all relationships connected to `Tom Hanks` that do not contain the specified type, `ACTED_IN`. 
...
For more information, see xref:clauses/delete.adoc[].

For more complex queries, see xref:subqueries/index.adoc[].