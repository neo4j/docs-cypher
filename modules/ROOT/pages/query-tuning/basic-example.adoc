...
Our execution plan is down to a single row and uses the xref::execution-plans/operators.adoc#query-plan-node-index-seek[Node Index Seek] operator which does an index seek (see xref:indexes/indexes-for-search-performance.adoc[]) to find the appropriate node.
