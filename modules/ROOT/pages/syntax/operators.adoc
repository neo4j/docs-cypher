...
* Spatial points cannot be compared using the operators `<`, `+<=+`, `>`, and `>=`.
To compare spatial points within a specific range, use either the xref:functions/spatial.adoc#functions-withinBBox[`point.withinBBox()`] or the xref:functions/spatial.adoc#functions-point[`point()`] function.
