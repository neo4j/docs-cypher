...
[[cypher-expressions-identifiers]]
== Note on identifiers

Identifiers, such as variables, label or relationship types, can have to forms.
Unquoted identifiers are restricted by <<cypher-naming, naming rules>>.

Identifiers can also be quoted with backticks to allow unrestricted name choices.
The following escaping sequences are allowed:
