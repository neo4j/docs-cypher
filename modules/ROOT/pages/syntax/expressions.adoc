...
Notable exceptions are the operators `IS NULL` and `IS NOT NULL` and the type predicate expression.
====
...
It is also possible to verify that a Cypher expression does not have a certain type, using `IS NOT ::`.
...
Assume that we have a graph containing the following nodes:
...
Cypher will thus regard any integral parameter as an `INTEGER`, regardless of if it is declared as e.g. an INT16 or an INT64 in the client programming language.
...
Type predicate expressions allows for some alternative syntax:
...
For verifying that an expression does not have a certain type, the following alternative syntax is supported:
