...
[[type-predicate-expressions-closed-dynamic-unions]]
...
With the release of Neo4j 5.0, a new label expression with an extended set of logical operators is being introduced, in addition to the single colon operator.
...
Neo4j 5.0 introduced an ampersand operator, which is equivalent to the colon conjunction operator.
...
In earlier versions of Neo4j (4.4 and earlier), relationship type expressions only had the pipe operator.
...
Queries that exclusively use syntax from earlier versions of Neo4j (4.4 and earlier) will continue to be supported.
