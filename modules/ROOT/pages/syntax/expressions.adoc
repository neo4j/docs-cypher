...
Note that closed dynamic types (`INNER_TYPE_1 | INNER_TYPE_2...`) cannot be appended generically with `NOT NULL`, instead all inner types must be nullable, or all not nullable. A syntax error will be raised if the inner types are not of the same nullability.
...
This allows for the checking of mixed types:
...
[source, cypher]
----
WITH [1, 0, true, false] AS mixedList
