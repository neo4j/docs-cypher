...
It is more powerful than using a path pattern as it allows full queries to be executed.
...
Any non-updating query is allowed, how it differs from regular queries is that the final `RETURN` clause may be omitted
...
even if a final `RETURN` clause is used. One exception to this is that for `DISTINCT UNIONS` the `RETURN` is still mandatory.
