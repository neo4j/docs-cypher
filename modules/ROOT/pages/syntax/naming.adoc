...
| Node label        | Camel case, beginning with an upper-case character      | `(:Person)`, `(:VehicleOwner)`
| Relationship type | Upper case, using underscore to separate words          | `[:PURCHASED]`, `[:OWNS_VEHICLE]`
| Property          | Lower camel case, beginning with a lower-case character | `name`, `firstName`
|===
