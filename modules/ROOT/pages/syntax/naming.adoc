...
For example, the property `name` is different from the property `Name`.
...
[options="header"]
|===
| Graph entity      | Recommended style                                            | Examples
| Node label        | PascalCase (capitalized words, no separators)                | `(:Person)`, `(:VehicleOwner)`
| Relationship type | SCREAMING_SNAKE_CASE (upper case, underscores as separators) | `[:PURCHASED]`, `[:OWNS_VEHICLE]`
| Property          | camelCase (lower-case first word, capitalized thereafter)    | `name`, `firstName`
|===
