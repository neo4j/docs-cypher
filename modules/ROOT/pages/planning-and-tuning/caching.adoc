...
* Raw query string -> pre-parsed query string + query options, such as `CYPHER key=value`, `EXPLAIN`, and `PROFILE` (pre-parsing)
...
* Raw query string -> physical execution plan (executable query cache, reported in JSON query log as `executableQueryCacheHit`)

[NOTE]
Logging `logicalPlanCacheHit` and `executableQueryCacheHit` in the link:{neo4j-docs-base-uri}/operations-manual/current/monitoring/logging/[JSON query log] requires the configuration setting query log requires the configuration setting `internal.dbms.logs.query.query_cache_usage=true`.
...
[NOTE]
For full information about the metrics on query cache usage, refer to the link:{neo4j-docs-base-uri}/operations-manual/current/monitoring/metrics/reference/#cypher-metrics[Operations Manual -> Cypher planning and caching metrics].
...
Creating an xref:indexes/index.adoc[index] or a xref:schema/index.adoc[constraint] clears the executable query cache, the logical plan cache, and the execution plan cache for the database, forcing every query to go through logical planning and execution planning again on its next execution.
...
.Comparing two unrelated queries before and after an index is created on one of them
[source]
----
+------+-----------------------------------------------+-----------+-----------+----------+
