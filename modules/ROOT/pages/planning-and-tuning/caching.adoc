...
Creating an xref:indexes/index.adoc[index] or a xref:schema/index.adoc[constraint] clears the executable query cache and the logical plan cache for the database, forcing *every query* to go through logical planning again on its next execution.
The execution plan cache, further down the pipeline, is not cleared, however: if logical planning for a given query produces the same plan as before, its execution plan cache entry is still hit.
...
For `:B`, logical planning produces the *same* plan as before, since nothing about the new index is relevant to it.
As a result, its execution plan cache entry is still hit, and the hash stays identical (step 2 equals step 6), even though the top-level executable query cache was missed.
