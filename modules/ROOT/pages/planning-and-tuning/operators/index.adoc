...
| Used to ensure that no property uniqueness constraints are violated when creating nodes.
| label:yes[]
|
|

| xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-asserting-multi-relationship-index-seek[AssertingMultiRelationshipIndexSeek]
| Used to ensure that no property uniqueness constraints are violated when creating relationships.
| label:yes[]
|
|
...
| xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-directed-relationship-vector-index-search[DirectedRelationshipVectorIndexSearch]
| Finds relationships from an index by doing a similarity search.
| label:yes[]
|
|
...
| xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-merge-into[MergeInto]
| Similar to the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-merge[`Merge`] operator, but is used when the start and end node of the pattern is matched outside the `MERGE` pattern.
|
|
| label:new[Introduced in Neo4j 2025.11]

| xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-merge-unique-node[MergeUniqueNode]
| Similar to the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-merge[`Merge`] operator, but is used when there is a property uniqueness constraint on the property used in the `MERGE` statement.
|
|
| label:new[Introduced in Neo4j 2026.02]
...
| xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-node-vector-index-search[NodeVectorIndexSearch]
| Finds nodes from an index by doing a similarity search.
| label:yes[]
|
|
...
| xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-remote-batch-properties[RemoteBatchProperties]
| Fetches properties from shards for a batch of rows to use in subsequent operators.
|
|
| label:new[Introduced in Neo4j 2026.01] label:cypher[Infinigraph only]

| xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-remote-batch-properties-with-filter[RemoteBatchPropertiesWithFilter]
| Fetches properties from shards for a batch if they satisfy a filter criterion.
|
| 
| label:new[Introduced in Neo4j 2026.01] label:cypher[Infinigraph only]

| xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-remote-node-index-seek[RemoteNodeIndexSeek]
| Batches nested index seeks on the right hand side of an apply.
| label:yes[]
|
| label:new[Introduced in Neo4j 2026.07] label:cypher[Infinigraph only]

| xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-remote-batch-properties[RemoteUniqueNodeIndexSeek]
| Batches nested unique index seeks on the right hand side of an apply.
| label:yes[]
|
| label:new[Introduced in Neo4j 2026.07] label:cypher[Infinigraph only]
...
|
| xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-undirected-relationship-vector-index-search[UndirectedRelationshipVectorIndexSearch]
| Finds relationships from an index by doing a similarity search.
| label:yes[]
|
|
