...
The `RemoveBatchProperties` operator is used when .
...
Total database accesses: XXX, total allocated memory: YYY
----
...
The `RemoteBatchPropertiesWithFilter` operator is used when .
...
The `RemoteNodeIndexSeek` operator is used when .
...
The `RemoteUniqueNodeIndexSeek` operator is used when .
