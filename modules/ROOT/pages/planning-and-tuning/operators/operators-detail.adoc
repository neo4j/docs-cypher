...
For an overview of how the Cypher planner uses operators, see xref:planning-and-tuning/execution-plans.adoc[].
...
If the index is backs up a xref:schema/constraints/create-constraints.adoc#create-property-uniqueness-constraints[property uniqueness constraint], the operator is instead called xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-node-unique-index-seek[NodeUniqueIndexSeek].
...
The `NodeUniqueIndexSeek` operator finds nodes using an index seek within an index backing up a xref:schema/constraints/create-constraints.adoc#create-property-uniqueness-constraints[property uniqueness constraint].
The node variable and the index used are shown in the arguments of the operator.
...
The `AssertingMultiNodeIndexSeek` operator is used to ensure that no xref:schema/constraints/create-constraints.adoc#create-property-uniqueness-constraints[property uniqueness constraints] are violated when creating nodes.
...
[[query-plan-asserting-multi-relationship-index-seek]]
=== Asserting Multi Relationship Index Seek

The `AssertingMultiRelationshipIndexSeek` operator is used to ensure that no xref:schema/constraints/create-constraints.adoc#create-property-uniqueness-constraints[property uniqueness constraints] are violated when creating relationships.
The example looks for the presence of a relationship with specified `name` and `id` properties.
...
| +EmptyResult                         |  1 |                                                                                                      |              1 |    0 |       0 |                |                        |           |                     |                                           |
| |                                    +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +-------------------------------------------+
| +Merge                               |  2 | CREATE (person), (location), (person)-[work:WORKS_IN {id: $autoint_0, badgeNumber: $autoint_1}]->(lo |              1 |    1 |       0 |                |                        |           |                     |                                           |
| |                                    |    | cation)                                                                                              |                |      |         |                |                        |           |                     |                                           |
| |                                    +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +-------------------------------------------+
| +AssertingMultiRelationshipIndexSeek |  3 | RANGE INDEX (person)-[work:WORKS_IN(id)]->(location) WHERE id = $autoint_0,                          |              0 |    2 |       4 |            248 |                    3/0 |     0,393 | Fused in Pipeline 0 | constraint_WORKS_IN_badgeNumber_unique: 1 |
|                                      |    | RANGE INDEX (person)-[work:WORKS_IN(badgeNumber)]->(location) WHERE badgeNumber = $autoint_1         |                |      |         |                |                        |           |                     | constraint_WORKS_IN_id_unique: 1          |
+--------------------------------------+----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+-------------------------------------------+
----
...
[[query-plan-node-vector-index-search]]
=== Node Vector Index Search

////
[source, cypher, role=test-setup]
----
DROP INDEX moviePlots IF EXISTS;
...
The `NodeVectorIndexSearch` operator searches a vector index for similar vector embeddings.
...
| Operator               | Id | Details                                                                                              | Estimated Rows | Rows | DB Hits | Memory (Bytes) | Page Cache Hits/Misses | Time (ms) | Pipeline            | Indexes Used  |
+------------------------+----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+---------------+
| +ProduceResults        |  0 | title                                                                                                |             10 |    0 |       0 |              0 |                        |           |                     |               |
| |                      +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +---------------+
| +Projection            |  1 | movie.title AS title                                                                                 |             10 |    0 |       0 |                |                        |           |                     |               |
| |                      +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +---------------+
| +NodeVectorIndexSearch |  2 | SEARCH movie IN (VECTOR INDEX moviePlots FOR RuntimeConstant(vector($autolist_0, $autoint_1, INTEGER |             10 |    0 |       1 |            248 |                    0/0 |     1.364 | Fused in Pipeline 0 | moviePlots: 1 |
|                        |    |  NOT NULL)) LIMIT 4)                                                                                 |                |      |         |                |                        |           |                     |               |
+------------------------+----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+---------------+
...
[[query-plan-directed-relationship-vector-index-search]]
=== Directed Relationship Index Search

////
[source, cypher, role=test-setup]
----
DROP INDEX reviews IF EXISTS;
...
[role=label--new-2026.01]
The `DirectedRelationshipVectorIndexSearch` operator searches a vector index for similar vector embeddings.
...
| Operator                               | Id | Details                                                                                              | Estimated Rows | Rows | DB Hits | Memory (Bytes) | Page Cache Hits/Misses | Time (ms) | Pipeline            | Indexes Used |
+----------------------------------------+----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+--------------+
| +ProduceResults                        |  0 | content, rating                                                                                      |              1 |    0 |       0 |              0 |                        |           |                     |              |
| |                                      +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +--------------+
| +Projection                            |  1 | cache[r.content] AS content, cache[r.rating] AS rating                                               |              1 |    0 |       0 |                |                        |           |                     |              |
| |                                      +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +--------------+
| +CacheProperties                       |  2 | cache[r.content], cache[r.rating]                                                                    |              1 |    0 |       0 |                |                        |           |                     |              |
| |                                      +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +--------------+
| +DirectedRelationshipVectorIndexSearch |  3 | SEARCH ()-[r:REVIEWED]->() IN (VECTOR INDEX reviews FOR RuntimeConstant(vector($autolist_0, $autoint |              1 |    0 |       1 |            248 |                    0/0 |     2.126 | Fused in Pipeline 0 | reviews: 1   |
|                                        |    | _1, INTEGER NOT NULL)) WHERE rating >= $autoint_2 LIMIT 4)                                           |                |      |         |                |                        |           |                     |              |
+----------------------------------------+----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+--------------+
...
[[query-plan-undirected-relationship-vector-index-search]]
=== Undirected Relationship Index Search
...
[role=label--new-2026.01]
The `UndirectedRelationshipVectorIndexSearch` operator searches a vector index for similar vector embeddings.
...
| Operator                                 | Id | Details                                                                                              | Estimated Rows | Rows | DB Hits | Memory (Bytes) | Page Cache Hits/Misses | Time (ms) | Pipeline            | Indexes Used |
+------------------------------------------+----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+--------------+
| +ProduceResults                          |  0 | content, rating                                                                                      |              2 |    0 |       0 |              0 |                        |           |                     |              |
| |                                        +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +--------------+
| +Projection                              |  1 | cache[r.content] AS content, cache[r.rating] AS rating                                               |              2 |    0 |       0 |                |                        |           |                     |              |
| |                                        +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +--------------+
| +CacheProperties                         |  2 | cache[r.content], cache[r.rating]                                                                    |              2 |    0 |       0 |                |                        |           |                     |              |
| |                                        +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +--------------+
| +UndirectedRelationshipVectorIndexSearch |  3 | SEARCH ()-[r:REVIEWED]-() IN (VECTOR INDEX reviews FOR RuntimeConstant(vector($autolist_0, $autoint_ |              2 |    0 |       1 |            248 |                    0/0 |     1.298 | Fused in Pipeline 0 | reviews: 1   |
|                                          |    | 1, INTEGER NOT NULL)) WHERE rating >= $autoint_2 LIMIT 4)                                            |                |      |         |                |                        |           |                     |              |
+------------------------------------------+----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+--------------+
...
`SemiApply` is only applied by the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-slotted-runtime[slotted runtime].
In the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-pipelined-runtime[pipelined runtime], the task of `SemiApply` is instead performed by the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-apply[`Apply`] and xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-limit[`Limit`] operators.
...
`AntiSemiApply` is only applied by the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-slotted-runtime[slotted runtime].
In the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-pipelined-runtime[pipelined runtime], the task of `AntiSemiApply` is instead performed by the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-anti[`Anti`], xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-apply[`Apply`], and xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-limit[`Limit`] operators.
...
+-----------------+----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+----------------------+
| Operator        | Id | Details                                                | Estimated Rows | Rows | DB Hits | Memory (Bytes) | Page Cache Hits/Misses | Indexes Used         |
+-----------------+----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+----------------------+
| +ProduceResults |  0 | `other.name`                                           |              0 |    5 |       0 |                |                    0/0 |                      |
| |               +----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+----------------------+
| +Projection     |  1 | other.name AS `other.name`                             |              0 |    5 |       5 |                |                    0/0 |                      |
| |               +----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+----------------------+
| +AntiSemiApply  |  2 |                                                        |              0 |    5 |       0 |                |                    0/0 |                      |
| |\              +----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+----------------------+
| | +Expand(Into) |  3 | (me)-[:FRIENDS_WITH]->(other)                          |              0 |    0 |       7 |            592 |                    0/0 |                      |
| | |             +----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+----------------------+
| | +Argument     |  4 | me, other                                              |              0 |    6 |       0 |                |                    0/0 |                      |
| |               +----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+----------------------+
| +Filter         |  5 | NOT anon_0 = anon_1 AND other:Person                   |              0 |    6 |       6 |                |                    0/0 |                      |
| |               +----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+----------------------+
| +Expand(All)    |  6 | (location)<-[anon_1:WORKS_IN]-(other)                  |              0 |    7 |       8 |                |                    1/0 |                      |
| |               +----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+----------------------+
| +Expand(All)    |  7 | (me)-[anon_0:WORKS_IN]->(location)                     |              1 |    1 |       2 |                |                    1/0 |                      |
| |               +----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+----------------------+
| +NodeIndexSeek  |  8 | RANGE INDEX me:Person(name) WHERE name = $autostring_0 |              1 |    1 |       2 |                |                    0/1 | range_person_name: 1 |
+-----------------+----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+----------------------+
...
`LetSemiApply` is only applied by the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-slotted-runtime[slotted runtime].
In the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-pipelined-runtime[pipelined runtime], the task of `LetSemiApply` is instead performed by the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-anti[`Apply`], xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-eager-aggregation[`EagerAggregation`], and xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-limit[`Limit`] operators.
...
`LetAntiSemiApply` is only applied by the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-slotted-runtime[slotted runtime].
In the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-pipelined-runtime[pipelined runtime], the task of `LetAntiSemiApply` is instead performed by the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-anti[`Apply`], xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-eager-aggregation[`EagerAggregation`], and xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-limit[`Limit`] operators.
...
`LetSelectOrSemiApply` is only applied by the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-slotted-runtime[slotted runtime].
In the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-pipelined-runtime[pipelined runtime], the task of `LetSelectOrSemiApply` is instead performed by the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-select-or-semi-apply[`SelectOrSemiApply`], xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-eager-aggregation[`EagerAggregation`], xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-limit[`Limit`], and xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-projection[`Projection`] operators.
...
`LetSelectOrAntiSemiApply` is only applied by the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-slotted-runtime[slotted runtime].
In the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-pipelined-runtime[pipelined runtime], the task of `LetSelectOrAntiSemiApply` is instead performed by the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-select-or-semi-apply[`SelectOrSemiApply`], xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-eager-aggregation[`EagerAggregation`], xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-limit[`Limit`], and xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-projection[`Projection`] operators.
...
`RollUpApply` is only applied by the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-slotted-runtime[slotted runtime].
In the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-pipelined-runtime[pipelined runtime], the task of `RollUpApply` is instead performed by the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-apply[`Apply`] and xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-eager-aggregation[`EagerAggregation`] operators. 
...
They allow Cypher to express and match various pattern types, such as xref:patterns/fixed-length-paths.adoc[fixed-length patterns], xref:patterns/variable-length-paths.adoc[variable-length patterns], xref:patterns/shortest-paths.adoc[shortest paths], and xref:patterns/non-linear-paths.adoc[non-linear patterns].
...
| Operator        | Id | Details                                                | Estimated Rows | Rows | DB Hits | Memory (Bytes) | Page Cache Hits/Misses | Time (ms) | Pipeline            | Indexes Used         |
+-----------------+----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+----------------------+
| +ProduceResults |  0 | `other.name`                                           |              0 |    5 |       0 |              0 |                    0/0 |     0.080 |                     |                      |
| |               +----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+                     +----------------------+
| +Projection     |  1 | other.name AS `other.name`                             |              0 |    5 |      10 |                |                    0/0 |     0.114 | In Pipeline 2       |                      |
| |               +----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+----------------------+
| +Apply          |  2 |                                                        |              0 |    5 |       0 |                |                    0/0 |           |                     |                      |
| |\              +----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+----------------------+
| | +Anti         | 10 |                                                        |              0 |    5 |       0 |            936 |                    0/0 |     0.096 | In Pipeline 2       |                      |
| | |             +----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+----------------------+
| | +Limit        |  9 | 1                                                      |              0 |    1 |       0 |            752 |                        |           |                     |                      |
| | |             +----+--------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +----------------------+
| | +Expand(Into) |  3 | (me)-[:FRIENDS_WITH]->(other)                          |              0 |    1 |       7 |            592 |                        |           |                     |                      |
| | |             +----+--------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +----------------------+
| | +Argument     |  4 | me, other                                              |              0 |    6 |       0 |           6520 |                    0/0 |     0.533 | Fused in Pipeline 1 |                      |
| |               +----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+----------------------+
| +Filter         |  5 | NOT anon_0 = anon_1 AND other:Person                   |              0 |    6 |      12 |                |                        |           |                     |                      |
| |               +----+--------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +----------------------+
| +Expand(All)    |  6 | (location)<-[anon_1:WORKS_IN]-(other)                  |              0 |    7 |       8 |                |                        |           |                     |                      |
| |               +----+--------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +----------------------+
| +Expand(All)    |  7 | (me)-[anon_0:WORKS_IN]->(location)                     |              1 |    1 |       2 |                |                        |           |                     |                      |
| |               +----+--------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     +----------------------+
| +NodeIndexSeek  |  8 | RANGE INDEX me:Person(name) WHERE name = $autostring_0 |              1 |    1 |       2 |            376 |                    3/0 |     0.866 | Fused in Pipeline 0 | range_person_name: 1 |
+-----------------+----+--------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+----------------------+
09:13:20   E
...
Given a start node, the `Repeat` operator will traverse xref::patterns/variable-length-paths.adoc#quantified-path-patterns[quantified path patterns] that cannot be solved (or solved efficiently) with the xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-varlength-expand-all[`VarLengthExpand(All)`] operator.
...
This operator is used for the xref:patterns/reference/shortest-paths.adoc#shortest-functions[`shortestPath()` and `allShortestPaths`] functions.
...
`TriadicSelection` is only applied by the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-slotted-runtime[slotted runtime].
In the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-pipelined-runtime[pipelined runtime], the task of `TriadicSelection` is instead performed by the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-triadic-build[`TriadicBuild`], xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-apply[`Apply`], and xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-triadic-filter[`TriadicFilter`] operators.
...
PROFILE
CYPHER runtime=slotted
MATCH (me:Person)-[:FRIENDS_WITH]-()-[:FRIENDS_WITH]-(other) USING SCAN me:Person
WHERE NOT (me)-[:FRIENDS_WITH]-(other)
RETURN other.name
----
...
+-------------------+----+----------------------------------------+----------------+------+---------+----------------+------------------------+
| +ProduceResults   |  0 | `other.name`                           |              1 |   24 |       0 |              0 |                    0/0 |
| |                 +----+----------------------------------------+----------------+------+---------+----------------+------------------------+
| +Projection       |  1 | other.name AS `other.name`             |              1 |   24 |      24 |                |                    0/0 |
| |                 +----+----------------------------------------+----------------+------+---------+----------------+------------------------+
| +Filter           |  2 | NOT anon_2 = anon_0                    |              1 |   24 |       0 |                |                    0/0 |
| |                 +----+----------------------------------------+----------------+------+---------+----------------+------------------------+
| +TriadicSelection |  3 | WHERE NOT (me)--(other)                |              1 |   48 |       0 |                |                    0/0 |
| |\                +----+----------------------------------------+----------------+------+---------+----------------+------------------------+
| | +Expand(All)    |  4 | (anon_1)-[anon_2:FRIENDS_WITH]-(other) |              1 |   48 |      72 |                |                    0/0 |
| | |               +----+----------------------------------------+----------------+------+---------+----------------+------------------------+
| | +Argument       |  5 | anon_1, anon_0                         |             24 |   24 |       0 |                |                    0/0 |
| |                 +----+----------------------------------------+----------------+------+---------+----------------+------------------------+
| +Expand(All)      |  6 | (me)-[anon_0:FRIENDS_WITH]-(anon_1)    |             24 |   24 |     484 |                |                    9/0 |
| |                 +----+----------------------------------------+----------------+------+---------+----------------+------------------------+
| +NodeByLabelScan  |  7 | me:Person                              |            460 |  460 |     461 |                |                    1/0 |
+-------------------+----+----------------------------------------+----------------+------+---------+----------------+------------------------+
...
To do this, `EagerAggregation`, as the name implies, must pull in all data eagerly from its source and build up state, which leads to increased memory pressure in the system.

[NOTE]
====
The cost of memory pressure of the `EagerAggregation` operator is still much lower than that of the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-eager[`Eager` operator].
`EagerAggregation` consumes all input rows and outputs as many rows as there are groups, which generally reduces cardinality (row count).
If there are no grouping keys, all input rows are in a single group, producing a single output row.
In contrast, the `Eager` operator buffers all input rows in memory, then passes them on, with cardinality unchanged.

`EagerAggregation` can be replaced with a less expensive xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-ordered-aggregation[`OrderedAggregation` operator] if the aggregation has a grouping key and the input is ordered by that key.
====
...
`EmptyRow` is only applied by the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-slotted-runtime[slotted runtime].
In the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-pipelined-runtime[pipelined runtime], the task of `EmptyRow` is instead performed by the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-projection[`Projection`] operator.
...
[NOTE]
====
`MergeUniqueNode` was added in Neo4j 2025.11 but it did not work as intended in that version.
For this reason, the operator was disabled in Neo4j 2025.12.
Neo4j 2026.02 fixes the bug and is thus the recommended version to use. 
====
...
[role=label--infinigraph label--new-2026.01]
[[query-plan-remote-batch-properties]]
=== Remote Batch Properties
The `RemoteBatchProperties` operator is used to fetch and cache properties from shards for batches of rows, so that they can be used by subsequent operators.
...
| Operator               | Id | Details                 | Estimated Rows | Rows | DB Hits | Memory (Bytes) | Page Cache Hits/Misses | Time (ms) | Pipeline      |
+------------------------+----+-------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+
| +ProduceResults        |  0 | title                   |             10 |   10 |       0 |              0 |                    0/0 |     0.074 |               |
| |                      +----+-------------------------+----------------+------+---------+----------------+------------------------+-----------+               |
| +Projection            |  1 | cache[m.title] AS title |             10 |   10 |       0 |                |                    0/0 |     0.077 |               |
| |                      +----+-------------------------+----------------+------+---------+----------------+------------------------+-----------+               |
| +RemoteBatchProperties |  2 | cache[m.title]          |             10 |   10 |      10 |            376 |                    0/0 |   477.772 | In Pipeline 1 |
| |                      +----+-------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+
| +NodeByLabelScan       |  3 | m:Movie                 |             10 |   10 |      11 |            376 |                    1/0 |     3.830 | In Pipeline 0 |
+------------------------+----+-------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+

Total database accesses: 21, total allocated memory: 696
----

======

[role=label--infinigraph label--new-2026.01]
[[query-plan-remote-batch-properties-with-filter]]
=== Remote Batch Properties With Filter
The `RemoteBatchPropertiesWithFilter` operator is used to fetch and cache properties from shards for batches of rows, if they satisfy a predicate. The filtering of rows happens on the shards thereby reducing the number of rows processed on the graph shard.
This is only planned if the predicate is applicable to the entity whose properties are being fetched and the expression doesn't depend on any other entities. 
...
| Operator                         | Id | Details                                 | Estimated Rows | Rows | DB Hits | Memory (Bytes) | Page Cache Hits/Misses | Time (ms) | Pipeline      |
+----------------------------------+----+-----------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+
| +ProduceResults                  |  0 | title                                   |              3 |    3 |       0 |              0 |                    0/0 |     0.063 |               |
| |                                +----+-----------------------------------------+----------------+------+---------+----------------+------------------------+-----------+               |
| +Projection                      |  1 | cache[m.title] AS title                 |              3 |    3 |       0 |                |                    0/0 |     0.052 |               |
| |                                +----+-----------------------------------------+----------------+------+---------+----------------+------------------------+-----------+               |
| +RemoteBatchPropertiesWithFilter |  2 | cache[m.title], m.released > $autoint_0 |              3 |    3 |       3 |            376 |                    0/0 |   515.105 | In Pipeline 1 |
| |                                +----+-----------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+
| +NodeByLabelScan                 |  3 | m:Movie                                 |             10 |   10 |      11 |            376 |                    1/0 |     3.656 | In Pipeline 0 |
+----------------------------------+----+-----------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+

Total database accesses: 14, total allocated memory: 696

----

======

[role=label--infinigraph label--new-2026.07]
[[query-plan-remote-node-index-seek]]
=== Remote Node Index Seek

The `RemoteNodeIndexSeek` operator batches index seeks when applied within a loop.
...
----
PROFILE 
MATCH (acc: Account {id: "foo"}), (purchase:Purchase) 
WHERE acc.createdAt = purchase.transactedAt 
RETURN *
----
...
| Operator               | Id | Details                                                                               | Estimated Rows | Rows | DB Hits | Memory (Bytes) | Page Cache Hits/Misses | Time (ms) | Pipeline      | Indexes Used           |
+------------------------+----+---------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+------------------------+
| +ProduceResults        |  0 | acc, purchase                                                                         |              0 |    0 |       0 |                |                    0/0 |     0.000 | In Pipeline 2 |                        |
| |                      +----+---------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+------------------------+
| +Apply                 |  1 |                                                                                       |              0 |    0 |       0 |                |                    0/0 |     0.014 |               |                        |
| |\                     +----+---------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+------------------------+
| | +RemoteNodeIndexSeek |  2 | RANGE INDEX purchase:Purchase(transactedAt) WHERE transactedAt = cache[acc.createdAt] |              0 |    0 |       0 |            128 |                    0/0 |     0.000 | In Pipeline 2 |                        |
| |                      +----+---------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+------------------------+
| +RemoteBatchProperties |  3 | cache[acc.createdAt]                                                                  |              0 |    0 |       0 |            328 |                    0/0 |     1.110 | In Pipeline 1 |                        |
| |                      +----+---------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+------------------------+
| +NodeUniqueIndexSeek   |  4 | UNIQUE acc:Account(id) WHERE id = $autostring_0                                       |              0 |    0 |       1 |            376 |                    0/0 |    25.825 | In Pipeline 0 | constraint_93192f92: 1 |
+------------------------+----+---------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+------------------------+

Total database accesses: 1, total allocated memory: 912
----

======

[role=label--infinigraph label--new-2026.07]
[[query-plan-remote-unique-node-index-seek]]
=== Remote Unique Node Index Seek

The `RemoteUniqueNodeIndexSeek` operator batches unique index seeks within loops.

.RemoteUniqueNodeIndexSeek
======
...
WHERE  purchase.transactedAt > datetime('2019-01-01T00:00:00.000Z')
MATCH (active: Account)
WHERE active.id = purchase.accountId
...
| Operator                     | Id | Details                                                                                              | Estimated Rows | Rows | DB Hits | Memory (Bytes) | Page Cache Hits/Misses | Time (ms) | Pipeline      | Indexes Used      |
+------------------------------+----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+-------------------+
| +ProduceResults              |  0 | active                                                                                               |              0 |    0 |       0 |                |                    0/0 |     0.000 |               |                   |
| |                            +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+               +-------------------+
| +Distinct                    |  1 | active                                                                                               |              0 |    0 |       0 |              0 |                    0/0 |     0.000 | In Pipeline 2 |                   |
| |                            +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+-------------------+
| +Apply                       |  2 |                                                                                                      |              0 |    0 |       0 |                |                    0/0 |     0.009 |               |                   |
| |\                           +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+-------------------+
| | +RemoteNodeUniqueIndexSeek |  3 | UNIQUE active:Account(id) WHERE id = cache[purchase.accountId]                                       |              0 |    0 |       0 |            128 |                    0/0 |     0.000 | In Pipeline 2 |                   |
| |                            +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+-------------------+
| +RemoteBatchProperties       |  4 | cache[purchase.accountId]                                                                            |              0 |    0 |       0 |            328 |                    0/0 |     1.019 | In Pipeline 1 |                   |
| |                            +----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+-------------------+
| +NodeIndexSeekByRange        |  5 | RANGE INDEX purchase:Purchase(transactedAt) WHERE transactedAt > RuntimeConstant(datetime($autostrin |              0 |    0 |       1 |            376 |                    0/0 |    50.717 | In Pipeline 0 | index_64559e44: 1 |
|                              |    | g_0))                                                                                                |                |      |         |                |                        |           |               |                   |
+------------------------------+----+------------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+-------------------+
...
The `AssertSameNode` operator is used to ensure that no node property uniqueness constraints are violated in the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-slotted-runtime[slotted] and interpreted runtime.
...
Had the query been run with the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-pipelined-runtime[pipelined runtime], the task of `AssertSameNode` would have been performed by the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-asserting-multi-node-index-seek[`AssertingMultiNodeIndexSeek`] operator.
...
The `AssertSameRelationship` operator is used to ensure that no relationship property uniqueness constraints are violated in the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-slotted-runtime[slotted] and interpreted runtime.
...
Had the query been run with the xref:planning-and-tuning/runtimes/concepts.adoc#runtimes-pipelined-runtime[pipelined runtime], the task of `AssertRelationship` would have been performed by the xref:planning-and-tuning/operators/operators-detail.adoc#query-plan-asserting-multi-relationship-index-seek[`AssertingMultiRelationshipIndexSeek`] operator.
...
| |                     +----+-----------------------------------+----------------+------+---------+------------------------+
| +ShowCurrentGraphType |  1 | graphTypeAsString, defaultColumns |             10 |    1 |       1 |                    0/0 |
+-----------------------+----+-----------------------------------+----------------+------+---------+------------------------+

Total database accesses: 1, total allocated memory: 64
----

The `graphTypeAsString` was added in Neo4j 2026.07, before that the `ShowCurrentGraphType` `Details` were `defaultColumns`.
======
