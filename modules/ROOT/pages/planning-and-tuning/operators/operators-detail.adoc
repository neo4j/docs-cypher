...
If the index is backs up a xref:schema/constraints.adoc#create-property-uniqueness-constraints[property uniqueness constraint], the operator is instead called xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-node-unique-index-seek[NodeUniqueIndexSeek].
...
The `NodeUniqueIndexSeek` operator finds nodes using an index seek within an index backing up a xref:schema/constraints.adoc#create-property-uniqueness-constraints[property uniqueness constraint].
The node variable and the index used are shown in the arguments of the operator.
...
The `AssertingMultiNodeIndexSeek` operator is used to ensure that no xref:schema/constraints.adoc#create-property-uniqueness-constraints[property uniqueness constraints] are violated.
