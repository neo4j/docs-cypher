...
Given a start node, the `Repeat(Trail)` operator will traverse xref::patterns/variable-length-patterns.adoc#quantified-path-patterns[quantified path patterns] that cannot be solved with the xref::planning-and-tuning/operators/operators-detail.adoc#query-plan-varlength-expand-all[`VarLengthExpand(All)`] operator.
...
The `StatefulShortestPath(Into)` operator finds shortest paths between two previously matched node variables.
It uses a bidirectional breath first search (BFS) algorithm, which performs two BFS invocations at the same time, one from the left boundary node and one from the right boundary node.
...
No valid path exists between both boundary nodes when both BFS invocations terminate without any overlap in the nodes that both invocations found.
...
| Operator                    | Id | Details                                                                                          | Estimated Rows | Rows | DB Hits | Memory (Bytes) | Page Cache Hits/Misses | Time (ms) | Pipeline            |
+-----------------------------+----+--------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+
| +ProduceResults             |  0 | p                                                                                                |              1 |    2 |       0 |              0 |                    0/0 |     0.068 |                     |
| |                           +----+--------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+                     |
| +Projection                 |  1 | (chris) ((anon_12)-[anon_14]-(anon_13)-[anon_11]-())* (stefan) AS p                              |              1 |    2 |       0 |                |                    0/0 |     1.812 |                     |
| |                           +----+--------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+                     |
| +StatefulShortestPath(Into) |  2 | SHORTEST 1 GROUPS (chris) ((`anon_5`)-[`anon_6`]-(`anon_7`)-[`anon_8`]-(`anon_9`)){1, } (stefan) |              1 |    2 |      39 |          22237 |                    0/0 |    39.258 | In Pipeline 3       |
| |                           +----+--------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+
| +CartesianProduct           |  3 |                                                                                                  |              0 |    1 |       0 |           1864 |                        |     0.847 | In Pipeline 2       |
| |\                          +----+--------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+
| | +Filter                   |  4 | stefan.name = $autostring_1                                                                      |              1 |    1 |      28 |                |                        |           |                     |
| | |                         +----+--------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     |
| | +NodeByLabelScan          |  5 | stefan:Person                                                                                    |             14 |   14 |      15 |            392 |                    1/0 |     0.482 | Fused in Pipeline 1 |
| |                           +----+--------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+
| +Filter                     |  6 | chris.name = $autostring_0                                                                       |              1 |    1 |      28 |                |                        |           |                     |
| |                           +----+--------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     |
| +NodeByLabelScan            |  7 | chris:Person                                                                                     |             14 |   14 |      15 |            376 |                    3/0 |     2.092 | Fused in Pipeline 0 |
+-----------------------------+----+--------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+

Total database accesses: 125, total allocated memory: 24349
...
The `StatefulShortestPath(All)` operator finds shortest paths from a previously matched node variable to an endpoint that was not previously matched.
It uses a breath first search algorithm.
...
----
PROFILE
MATCH (chris {name: 'Chris'})
CALL {
  WITH chris
  MATCH
    p = ALL SHORTEST (chris)(()-[]-()-[]-()){1,}(location:Location)
  RETURN length(p) AS pathLength, location.name AS locationName
}
RETURN pathLength, locationName
----
...
| Operator                   | Id | Details                                                                                            | Estimated Rows | Rows | DB Hits | Memory (Bytes) | Page Cache Hits/Misses | Time (ms) | Pipeline            |
+----------------------------+----+----------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+
| +ProduceResults            |  0 | pathLength, locationName                                                                           |             35 |   20 |       0 |              0 |                    0/0 |     0.064 |                     |
| |                          +----+----------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+                     |
| +Projection                |  1 | length((chris) ((anon_12)-[anon_14]-(anon_13)-[anon_11]-())* (location)) AS pathLength,            |             35 |   20 |      40 |                |                    0/0 |     4.978 |                     |
| |                          |    | location.name AS locationName                                                                      |                |      |         |                |                        |           |                     |
| |                          +----+----------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+                     |
| +StatefulShortestPath(All) |  2 | SHORTEST 1 GROUPS (chris) ((`anon_5`)-[`anon_6`]-(`anon_7`)-[`anon_8`]-(`anon_9`)){1, } (location) |             35 |   20 |     179 |          37703 |                    1/0 |    58.855 | In Pipeline 1       |
| |                          |    |         expanding from: chris                                                                      |                |      |         |                |                        |           |                     |
| |                          |    |     inlined predicates: location:Location                                                          |                |      |         |                |                        |           |                     |
| |                          +----+----------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+
| +Filter                    |  3 | chris.name = $autostring_0                                                                         |              2 |    1 |      35 |                |                        |           |                     |
| |                          +----+----------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+                        |           |                     |
| +AllNodesScan              |  4 | chris                                                                                              |             35 |   35 |      36 |            376 |                    2/0 |     4.102 | Fused in Pipeline 0 |
+----------------------------+----+----------------------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------------+

Total database accesses: 290, total allocated memory: 38039
