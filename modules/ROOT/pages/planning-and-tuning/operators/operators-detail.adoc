...
The `AssertingMultiNodeIndexSeek` operator is used to ensure that no xref:schema/constraints/create-constraints.adoc#create-property-uniqueness-constraints[property uniqueness constraints] are violated.
...
[role=label--infinigraph-only label--new-2026.01]
...
[role=label--infinigraph-only label--new-2026.07]
...
The `graphTypeAsString` was added in Neo4j 2026.06, before that the `ShowCurrentGraphType` `Details` were `defaultColumns`.
======
