...
The statistical information maintained by Neo4j includes the following: the number of nodes having a certain label, the number of relationships by type, selectivity per index, and the number of relationships by type, ending with or starting from a node with a specific label.
]
...
[[runtimes-lazy-eager-evaluation]]
...
Such operators need to complete execution in its entirety before any rows are sent to their parents as input, and are sometimes required to enforce Cypher semantic.
...
In general, users of Neo4j Enterprise Edition should not have to use slotted runtime. However, there are scenarios where the fast planning phase of the slotted runtime may be useful. For example, if using an application that generates queries which are not cached (i.e. never, or very rarely, repeated), then slotted runtime may be preferable because of its faster planning time. 
...
The pipelined runtime is ideal for transactional use cases, with a big number of queries running in parallel on the system. This covers most usage scenarios, and for this reason, it is the default Neo4j runtime.
...
In most situations where multiple CPU cores are available, queries that require more than a few milliseconds to run (e.g. >500ms) on the pipelined runtime will run significantly faster on the parallel runtime.
The parallel runtime is thus suited for analytical processing, where relatively few demanding queries are constructed to target a large section of the graph to gain valuable data insights.
