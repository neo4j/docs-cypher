...
The same clause, with `IS {two-colons} STRING NOT NULL` added, would, however, be able to access an existing `TEXT` index:
...
* It is possible to use the xref:functions/string.adoc#functions-tostring[`toString()`] function to convert an expression to a `STRING` values.
