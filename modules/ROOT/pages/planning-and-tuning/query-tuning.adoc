...
(See xref:/planning-and-tuning/execution-plans/#runtimes-reading-execution-plans[Cypher Manual -> Execution plans and query tuning -> Understanding execution plans])
This option allows to control to what extent it should do that:
...
| inferSchemaParts=most_selecive_label
| Relationship types are used to infer labels on the relationships' end nodes and the label corresponding to the least number of nodes is used to estimate rows.
Not inferring more labels yields better results for nodes with several labels which are dependent, such as every `:Actor` being a `:Person`.
