...
Planning a query can be an expansive operation.
Once an execution plan is obtained for a query, it is placed in a cache.
The next time when the exact same query arrives, planning is not needed. The execution plan can be obtained from the cache.

Replanning refers to the event when a query has been planned before, but needs to be planned again.
...
* When the cached query plan has notifications that have become invalid. Consider that a query plan had the notification `01N50: Label does not exist`. After adding a node with the label that did not existed before and running the same query again, replanning the query is required.
...
When a schema change was commited during planning, replanning occurs.
Dropping an index is an example of a schema change.
...
The Cypher option `replan` does not have any affect on replanning due to schema changes.
