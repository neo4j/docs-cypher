...
CALL { // <3>
    ...
    WITH moviesList, batchStart, [movie IN moviesList[batchStart .. batchStart + batchSize] | movie.title || ': ' || movie.plot] AS batch // <4>
    CALL genai.vector.encodeBatch(batch, 'OpenAI', { token: $token }) YIELD index, vector 
    CALL db.create.setNodeVectorProperty(moviesList[batchStart + index], 'embedding', vector) // <5>
} IN TRANSACTIONS OF 1 ROW <6>
----

<1>  Defines a `batchSize` for processing the nodes in `moviesList`.
Because vector embeddings can be very large, increasing the batch size may require significantly more memory, and may exceed the provider's thresholds.
<2> Processes `Movie` nodes in increments of `batchSize`.
<3> A xref:subqueries/subqueries-in-transactions.adoc[`CALL` subquery] is used to execute separate, inner transactions producing intermediate commits for each batch.
<4> Extracts the `title` and `plot` from each batch into separate lists of strings.
<5> `moviesList[batchStart + index]` sets the `embedding` for each movie in the current batch.
<6> Sets the amount of batches committed for each inner transaction to `1`.
====

[NOTE]
This example may not be performant if replicated on a larger dataset.
Specifically, collecting a large amount of nodes or relationship properties in a list may consume significant memory.
