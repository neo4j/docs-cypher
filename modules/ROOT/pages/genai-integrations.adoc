...
If you want to generate embeddings for the text content of all nodes with label `Tweet`, you can use `CALL ... IN TRANSACTIONS` to split the work up into batches and issue one API request per batch.

Assuming nodes with the `Tweet` have a `text` property, you can generate vector embeddings for each one and write them to the `embedding` property on each one in batches of 1000, for example:
...
The following AI providers are supported for generating vector embeddings.
