...
A precondition for using both features is that vector embeddings have been set as properties of these entities.
The GenAI plugin, which contains the function `genai.vector.encode()` and the procedure `genai.vector.encodeBatch()`,enables the creation of such embeddings using GenAI providers.
