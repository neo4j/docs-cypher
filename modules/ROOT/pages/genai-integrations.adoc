...
<1>  Define a `batchSize` for processing the nodes in `moviesList`.
...
<2> Processe `Movie` nodes in increments of `batchSize`.
...
<4> Extract the `title` and `plot` from each batch into separate lists of strings.
<5> `moviesList[batchStart + index]` sets the `embedding` for each movie in the current batch.
<6> Set the amount of batches committed for each inner transaction to `1`.
====

[NOTE]
This example may not be performant if replicated on a larger dataset.
Specifically, collecting a large amount of nodes or relationship properties in a list may consume a lot of memory.
