...
----
pathPattern ::= 
  [ pathVariableDeclaration ] [ shortestPathSelector ] pathPatternExpression
...
_Keyword-based specification of shortest path selection was introduced in Neo4j 5.17._
...
When a selective path selector is specified, then the paths matched by the path pattern are partitioned by distinct pairs of start and end nodes.
Within each partition, they are put in ascending order of path length.
Within each partition, paths are then selected according to the specified path selector as follows:
...
When a selective shortest path selector is specified for a path pattern, it can be the only path pattern in the graph pattern.
...
For every single shortest path connecting each distinct pair of nodes, only return those have path length that is an even number (so fewer results than the previous example):
...
Prior to the introduction of keyword-based specification of shortest path selection in Neo4j 5.17, the only available syntax for shortest paths were the functions `shortestPath()` and `allShortestPaths()`.
...
Both functions will continue to be available.
