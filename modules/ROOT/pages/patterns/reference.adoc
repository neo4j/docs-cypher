...
Note that `*` used here on its own is not the same as the Kleene star (an operator that represents 0 or more repetitions), as the Kleene star has a lower bound of zero.
