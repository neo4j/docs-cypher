...
* `ACYCLIC`: nodes cannot be repeated within a path. A node can be repeated across paths, enabling the formation of xref:patterns/non-linear-patterns.adoc#equijoins[equijoins].
...
When a node variable is repeated in separate path patterns within the same `MATCH` clause, it specifies an xref:patterns/non-linear-patterns.adoc#equijoins[equijoin].
