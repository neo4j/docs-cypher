...
Conceptually, the above is expanded as part of the matching process by repeating the path pattern contained in the outer parentheses three times is as follows:
...
As a result two pairs of node patterns end up juxtaposed with each other: `(y1:Y)(x2:X)` and `(y2:Y)(x3:X)`.
During the matching process, each pair of juxtaposed node patterns will match the same nodes, and those nodes will satisfy the conjunction of the predicates in the node patterns.
For the first pair, both `y1` and `x2` will bind to the same node, and that node will both have label `X` and label `Y`.

This expansion and juxtaposition is depicted in the following diagram:

image::patterns_juxtaposition.svg[width="500", role="middle"]

[[juxtaposition-simple-path-patterns-and-quantified-path-patterns]]
=== Simple path patterns and quantified path patterns

Juxtaposition also comes about when a simple path pattern is placed next to a quantified path.
...
After expanding the iterations of the quantified path pattern, the right-hand node pattern `(:B)` is juxtaposed with the left-hand node pattern `(:X)`.
...
If the simple path pattern is on the right of the quantified path pattern, its leftmost node `(:A)` is merged with the rightmost node `(:Y)` of the last iteration of the quantified path pattern.
...
[[juxtaposition-adjacent-quantified-path-patterns]]
=== Adjacent quantified path patterns
...
[[juxtaposition-forbidden-juxtapositions]]
=== Forbidden juxtapositions

Juxtaposition is only permitted when at least one of the path patterns being juxtaposed is a quantified path pattern. The following juxtapositions of simple path patterns are not valid:
