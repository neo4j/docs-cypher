...
(The same is not true if a query uses xref:patterns/repeatable-relationship-and-node-matching.adoc[`REPEATABLE ELEMENTS`]).
...
Explicit path modes like `ACYCLIC` or `TRAIL` cannot be used with the xref:patterns/repeatable-relationship-and-node-matching.adoc[`REPEATABLE ELEMENTS`] match mode.
For example:
