...
This page contains reference material for looking up the syntax and semantics of specific elements of graph pattern matching.
It includes the following sections:

* xref:patterns/reference.adoc#node-patterns[Node patterns]
* xref:patterns/reference.adoc#relationship-patterns[Relationship patterns]
...
* xref:patterns/reference.adoc#quantified-path-patterns[Quantified path patterns]
* xref:patterns/reference.adoc#quantified-relationships[Quantified relationships]
* xref:patterns/reference.adoc#quantifiers[Quantifiers]
* xref:patterns/reference.adoc#variable-length-relationships[Variable-length relationships]
* xref:patterns/reference.adoc#shortest-paths[Shortest paths]
* xref:patterns/reference.adoc#shortest-functions[The `shortestPath()` and `allShortestPaths()` functions]
* xref:patterns/reference.adoc#graph-patterns[Graph patterns]
* xref:patterns/reference.adoc#match-modes[Match modes]
* xref:patterns/reference.adoc#path-modes[Path modes]
* xref:patterns/reference.adoc#node-pattern-pairs[Node pattern pairs]
...
[[shortest-paths-rules-match-modes-and-path-modes]]
==== Match modes and path modes

When combined with a xref:patterns/reference.adoc#match-modes[match mode] such as `REPEATABLE ELEMENTS`, `SHORTEST` selective path selectors are placed after the specified match mode.
...
When combined with a xref:patterns/reference.adoc#path-modes[path mode] such as `ACYCLIC`, `SHORTEST` selective path selectors are placed before the path mode.
This is because the selective path selector governs which paths are returned, while the path mode constrains the structure of those paths.
...
Whether or not a graph pattern can match a relationship more than once is governed by the selected xref:patterns/reference.adoc#match-modes[match mode].

image::patterns-graph-pattern-reference.svg[Cypher example of a graph pattern including path patterns,width=600,role=popup]
...
* A relationship can only be bound once within a relationship xref:patterns/reference.adoc#quantified-path-patterns-rules-group-variables[group variable].

No solutions can be returned if a relationship variable occurs more than once across a graph pattern.
For example, the following patterns, where the `r` variable acts as an xref:patterns/reference.adoc#graph-patterns-rules-equijoin[equijoin] requiring all occurrences to match the same relationship, cannot return paths:
...
For example, the following two graph patterns, where the `r` variable acts as an xref:patterns/reference.adoc#graph-patterns-rules-equijoin[equijoin] requiring all occurrences to match the same relationship, can return paths:
...
The following applies to match modes and the number of path patterns with xref:patterns/reference.adoc#shortest-paths-rules-selective[selective path selectors] (`SHORTEST k`, `SHORTEST k GROUPS`, `ALL SHORTEST`, and `ANY k`).
...
image::match-modes-reference.svg[width=400, role=popup]
