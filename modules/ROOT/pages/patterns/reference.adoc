...
* `ACYCLIC`: nodes cannot be repeated within a path. A node can be repeated across paths, enabling xref:patterns/non-linear-patterns.adoc#equijoins[equijoins] to be formed.
...
[[all-path-modes-in-match-must-be-the-same]]
...
[[mixing-restrictive-path-selections-and-explicit-match-modes]]
