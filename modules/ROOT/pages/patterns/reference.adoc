...
The value expression can be any expression as listed in the section on xref:expressions/expressions-overview.adoc[expressions], except for path patterns (which will throw a syntax error) and regular expressions (which will be treated as string literals).
