...
is equivalent to the following quantified path pattern that matches paths with between m and n relationships of type `R` pointing left-to-right, starting at node of type `A`, ending at node of type `B`, and with any nodes in-between:
