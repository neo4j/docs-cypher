...
(The same is not true if a query uses xref:patterns/repeatable-relationship-and-node-patterns.adoc[`REPEATABLE ELEMENTS`]).
...
Explicit path modes like `ACYCLIC` or `TRAIL` cannot be used with the xref:patterns/repeatable-relationship-and-node-patterns.adoc[`REPEATABLE ELEMENTS`] match mode.
For example:
