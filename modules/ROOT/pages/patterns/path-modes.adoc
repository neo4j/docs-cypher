...
The scope of a path mode is the path pattern, whereas the scope of a match mode is all of the path patterns under a single `MATCH`.
...
Note that with the `ACYCLIC` path mode, it is not necessary to use predicates to exclude visiting the start or end nodes.
...
You can also use `ACYCLIC` to examine the number of possible routes from the source to the destination router that pass through each of the intermediate routers.
...
Only after these paths are generated does a xref:clauses/where.adoc[`WHERE`] filter discard those containing cycles.
