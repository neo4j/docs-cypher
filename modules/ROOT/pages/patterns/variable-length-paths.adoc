...
.Query
// tag::patterns_variable_length_paths_qpp[]
[source, cypher]
----
MATCH (:Station { name: 'Denmark Hill' })<-[:CALLS_AT]-(d:Stop)
      ((:Stop)-[:NEXT]->(:Stop)){1,3}
...
RETURN d.departs AS departureTime, a.arrives AS arrivalTime
----
// end::patterns_variable_length_paths_qpp[]
...
.Query
// tag::patterns_variable_length_paths_quantified_relationships[]
[source, cypher]
----
...
RETURN n.departs AS departureTime
----
// end::patterns_variable_length_paths_quantified_relationships[]
...
* Type expressions are limited to the xref:patterns/reference/node-and-relationship-patterns.adoc#label-expressions[disjunction operator].
* The xref:clauses/where.adoc[WHERE] clause is not allowed.

For more information, see the reference section on xref:patterns/reference/variable-length-paths.adoc#variable-length-relationships[variable-length relationships].
====
...
.Query
// tag::patterns_variable_length_paths_predicates_in_qpp[]
[source,cypher]
----
MATCH (bfr:Station {name: "London Blackfriars"}),
      (ndl:Station {name: "North Dulwich"})
...
RETURN reduce(acc = 0, r in relationships(p) | round(acc + r.distance, 2))
  AS distance
----
// end::patterns_variable_length_paths_predicates_in_qpp[]
