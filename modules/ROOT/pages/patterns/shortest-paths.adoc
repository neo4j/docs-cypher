...
[NOTE]
This rule is imposed to avoid the number of matches to a graph pattern being non-deterministic, which would result from the interaction of:
the rule that relationships must be unique with a given graph pattern match;
the principle that the path patterns in a graph pattern are matched independently,
and the fact that `SHORTEST` itself is non-deterministic.
