...
[TIP]
The relationship patterns in this and the other examples in the section do not have a direction i.e. they have no `>` or `<` arrow heads. This is because paths can ignore relationship directions as the relationships represent tracks between `Stations` that have trains going in both directions.
