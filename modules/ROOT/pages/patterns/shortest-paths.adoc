...
For more information, see xref:patterns/reference/shortest-paths.adoc#shortest-functions[Syntax and semantics -> The `shortestPath()` and `allShortestPaths()` functions].
...
* You need to specify complex graph navigation via xref:patterns/variable-length-paths.adoc#quantified-path-patterns[quantified path patterns].
...
The paths matched by a xref:patterns/fixed-length-paths.adoc#path-patterns[path pattern] can be restricted to only the shortest (by number of hops) by including the keyword `SHORTEST k`, where `k` is the number of paths to match.
...
The `+` quantifier means that one or more relationships should be matched. For more information, see xref:patterns/reference/variable-length-paths.adoc#quantified-relationships[Syntax and semantics - quantified relationships].
...
The result is a significantly faster query (down from 45 to 4 milliseconds):
...
Another way to inform the planner of the uniqueness of the target node in a shortest path is to create an xref:indexes/search-performance-indexes/index.adoc[index] or xref:constraints/create-constraints.adoc#create-property-uniqueness-constraints[property uniqueness]/xref:constraints/create-constraints.adoc#create-key-constraints[key constraint] (both of which are xref:constraints/create-constraints.adoc#constraints-and-backing-indexes[index-backed]) on a property belonging to the matched nodes in the shortest path.
...
With one source and many targets, rewriting a shortest path query using a `CALL` subquery forces the planner to use either the  `ShortestPath` or `StatefulShortestPath(Into)` operator, which runs once per target node.
...
If the query is rewritten with a `CALL` subquery the planner will use the `StatefulShortestPath(Into)` operator which performs separate traversals for each individual source-target node pairs.
...
On the contrary, doing so ensures that the `StatefulShortestPath(Into)` operator is executed `19682` times, once for each source-target node pair, thereby generating a more expensive query.
...
* The xref:patterns/reference/shortest-paths.adoc#shortest-functions[`shortestPath()` or `allShortestPaths()`] functions are used.
...
* If the pattern is a xref:patterns/variable-length-paths.adoc#quantified-path-patterns[quantified path pattern] in the form of `\(()-[]-())`, or is a  xref:patterns/variable-length-paths.adoc#quantified-relationships[quantified-relationship], and it uses a filter that can be applied directly to the relationship.
* If the pattern is a quantified path pattern or a quantified relationship and the lower bound of the xref:patterns/reference/variable-length-paths.adoc#quantifiers[quantifier] is 0 or 1.
