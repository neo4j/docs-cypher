...
When the subexpression of the statefulShortestPath-operator matches only one of both boundary nodes, then a `StatefulShortestPath(All)` is planned.

When the subexpression of the statefulShortestPath-operator matches both boundary nodes, then a `StatefulShortestPath(Into)` is planned only when both boundary nodes match to at most one data node. Otherwise, this plan is avoided when possible.
Another plan will be favoured where the subexpression matches only one of both boundary nodes followed by an `StatefulShortestPath(All)`.
...
In case you want the `StatefulShortestPath(Into)` to be chosen, then you can rewrite your query using a `CALL subquery`.
...
makes use of `StatefulShortestPath(Into)`, because the CALL subroutine is executed for each incoming row.  Each incoming row matches `a` to exactly one station node and `b` to exactly one station node.
[role="queryplan", subs="attributes+"]
----
+-----------------------------+----+-----------------------------------------------------------------------------------------+----------------+------+---------+----------------+------------------------+-----------+---------------+
| Operator                    | Id | Details                                                                                 | Estimated Rows | Rows | DB Hits | Memory (Bytes) | Page Cache Hits/Misses | Time (ms) | Pipeline      |
