...
Another way to inform the planner of the uniqueness of the target node in a shortest path is to create an xref:indexes/search-performance-indexes/index.adoc[index] or xref:schema/constraints.adoc#create-property-uniqueness-constraints[property uniqueness]/xref:schema/constraints.adoc#create-key-constraints[key constraint] (both of which are xref:schema/constraints.adoc#constraints-and-backing-indexes[index-backed]) on a property belonging to the matched nodes in the shortest path.
