...
For example, the following query with two path patterns is not valid:
...
This query can be made valid by putting the path patterns in separate `MATCH` clauses:
...
Separating selective path patterns into different `MATCH` clauses is necessary to ensure that the number of matches to a graph pattern remains deterministic. In general, this non-determinism would result from the interaction of the following factors:

- The requirement that relationships must be unique for a given graph pattern match.
- The principle that path patterns in a graph pattern are matched independently.
- The inherent non-determinism of selective path patterns.
