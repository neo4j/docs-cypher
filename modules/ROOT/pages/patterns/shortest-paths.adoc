...
Using `StatefulShortestPath(Into)` requires 27 bidirectional breath-first search (BFS) invocations, while `StatefulShortestPath(All)` requires only 3 unidirectional BFS invocations expanding from the right boundary node.
