...
.Query
// tag::patterns_non_linear_paths_equijoin[]
[source, cypher]
----
...
----
// end::patterns_non_linear_paths_equijoin[]
...
In addition to the single xref:patterns/fixed-length-paths.adoc#path-patterns[path patterns], multiple path patterns can be combined in a comma-separated list to form a graph pattern.
...
This is achieved by placing the xref:patterns/variable-length-paths.adoc#quantified-relationships[quantified relationship] `-[:NEXT]\->*` on each side of node `b`.
...
By default, Cypher only allows a relationship to be traversed once in a given match for a graph pattern.
(The same is not true for queries using xref::patterns/repeatable-node-and-relationship-paths.adoc[`REPEATABLE ELEMENTS`].)
...
.Query
// tag::patterns_non_linear_paths_graph_pattern[]
[source, cypher]
----
...
----
// end::patterns_non_linear_paths_graph_pattern[]
