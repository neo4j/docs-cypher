...
The following query finds the total distance from `London Blackfriars` to `North Dulwich` following the path yielded by `SHORTEST`:
...
MATCH (bfr:Station {name: 'London Blackfriars'}),
      (ndl:Station {name: 'North Dulwich'})
MATCH p = SHORTEST 1 (bfr)-[:LINK]-+(ndl)
...
`SHORTEST` finds the shortest path by number of hops.
Whether this corresponds to the shortest path by distance can be checked by looking at each path between the two end `Stations` and returning the first result after ordering by `distance`:
...
To compose the predicate, the xref:functions/spatial.adoc#functions-distance[point.distance()] function is used to compare the distance of the left-hand `Station` (`a`) and the right-hand `Station` (`b`) in each node-pair along the path to the destination `North Dulwich`:
