...
For more information, see xref:patterns/reference/match-modes-and-path-modes.adoc#match-modes[Syntax and semantics -> Match modes].
...
The section on xref:patterns/unique-relationship-paths.adoc#default-traversal-rules-and-konigsberg-bridges-problem[Cypher's default traversal rules and the Königsberg bridges problem] showed the impossibility of crossing each of the seven bridges in Königsberg exactly once in a single walk.
...
// tag::patterns_repeatable_elements[]
[source, cypher]
----
...
----
// end::patterns_repeatable_elements[]
...
For more information about `REPEATABLE ELEMENTS`, see xref:patterns/reference/match-modes-and-path-modes.adoc#match-modes-rules-repeatable-elements[Syntax and semantics -> Repeatable relationship and node matching].
...
To ensure that a `MATCH` will return a finite number of solutions in a finite amount of time, xref:patterns/reference/variable-length-paths.adoc#quantifiers-rules[unbounded quantifiers] that do not impose an upper bound on a pattern, such as `*`, `+`, or `{1,}` are not allowed in combination with `REPEATABLE ELEMENTS`.
