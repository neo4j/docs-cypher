...
// tag::patterns_fixed_length_paths_node_pattern[]
[source, cypher]
----
MATCH (n:Station WHERE n.name STARTS WITH 'Preston')
RETURN n
----
// end::patterns_fixed_length_paths_node_pattern[]


See the xref:patterns/reference/node-and-relationship-patterns.adoc#node-patterns[node patterns] reference section for more details.
...
See xref:patterns/fixed-length-paths.adoc#path-patterns[path patterns] for more details.
...
See the xref:patterns/reference/node-and-relationship-patterns.adoc#relationship-patterns[relationship patterns] reference section for more details.
...
.Query
// tag::patterns_fixed_length_paths_path_pattern[]
[source, cypher]
----
MATCH (s:Stop)-[:CALLS_AT]->(:Station {name: 'Denmark Hill'})
RETURN s.departs AS departureTime
----
// end::patterns_fixed_length_paths_path_pattern[]
