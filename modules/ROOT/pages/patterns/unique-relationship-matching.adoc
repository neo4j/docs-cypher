...
For more information about this Cypher's default `MATCH` behavior, see xref:patterns/reference.adoc#match-modes-rules-different-relationships[Syntax & semantics -> Match modes].

[[non-default-match-behavior]]
== Non-default `MATCH` behavior

Cypher's default `MATCH` behavior is applicable to most use cases.
It is, however, possible to alter how paths are traversed to solve specific problems, either by removing the constraint to re-traverse relationships using `REPEATABLE ELEMENTS` or by not allowing the re-traversal of nodes in the same path with the `ACYCLIC` keyword.
