...
[source, syntax]
----
MATCH (:Station {name: "London Euston"})<-[:CALLS_AT]-(:Stop)-[:NEXT]->(:Stop)
...
The following code block is used for the examples illustrating quantified path patterns:
...
So the quantifier applied to the wrapper parentheses is the range 1 to 3, expressed as `{1,3}`:
...
This also includes repetitions of 2, but in this case this repetition will not return matches.
...
[NOTE]
====
Prior to the introduction of quantified path patterns and quantified relationships, the only method in Cypher to match paths of a variable length was through variable-length relationship. 
...
This section uses the example of Stations and Stops, but with an additional property `distance` added to the NEXT relationships:
...
| departureTime | totalDistance

| ["17:07", "17:11", "17:13", "17:19"] | 1.4
| ["17:10", "17:17"] | 1.4
...
The following code block creates the data for the the next two sections:
...
If the evaluation of the `WHERE` clause conditions is forced to happen after the `shortestPath` function returns a solution, e.g., by moving the `WHERE` clause so it comes after a `WITH` clause, the shortest path found will include `LINK`s with `distance` greater than `20`, which will subsequently get filtered out:
...
See xref::execution-plans/shortestpath-planning.adoc[] for more information.
...
The same example used in the xref:patterns/concepts.adoc#shortest-path[] section above is used here to illustrate the use of inline predicates.
...
But in cases where this is not known, one alternative would be to make the pattern more specific by, e.g,. adding node labels, or by specifying a relationship direction.
...
The following code block creates the data for this section:
...
It creates the following graph, showing the train services the passenger can use:
...
| 11:11Z | "Harrogate" | 11:20Z | 12:07Z
