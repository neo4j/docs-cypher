...
As the results show, a node may be traversed several times in the same path, but a relationship (bridge) is never re-traversed (for example, the path `[6, 4, 1, 5, 6]`, which traverses the same bridge more than once, cannot be matched).
...
_In the `samplePath` with a length of `6`, all bridges are traversed except for bridge `3`._
...
Cypher 25 does, however, enable altering how paths are traversed to solve specific problems, either by permitting re-traversal of relationships using `REPEATABLE ELEMENTS` or by not allowing the re-traversal of nodes in the same path with the `ACYCLIC` keyword.

For more information, see:

* link:patterns/repeatable-node-and-relationship-paths.adoc[]
* link:patterns/acyclic-paths.adoc[]

Cypher 25 also allows explicitly defining the default pattern matching behavior described above using the `DIFFERENT RELATIONSHIPS` keyword.