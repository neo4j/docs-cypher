...
When combined with a xref:patterns/reference/match-modes-and-path-modes.adoc#match-modes[match mode] such as `REPEATABLE ELEMENTS`, `SHORTEST` selective path selectors are placed after the specified match mode.
...
When combined with a xref:patterns/reference/match-modes-and-path-modes.adoc#path-modes[path mode] such as `ACYCLIC`, `SHORTEST` selective path selectors are placed before the path mode.
