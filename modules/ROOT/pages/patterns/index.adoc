...
* xref:patterns/relationship-uniqueness-patterns.adoc[]: information about Cypher's default behavior of not re-traversing relationships in graph pattern matches and the `DIFFERENT RELATIONSHIPS` keyword.
* xref:patterns/no-uniqueness-patterns.adoc[]: information about how to enable re-traversal of relationships in graph pattern matches using `REPEATABLE ELEMENTS`. label:new[Introduced in Neo4j 2025.06] label:cypher[Cypher 25 only]
