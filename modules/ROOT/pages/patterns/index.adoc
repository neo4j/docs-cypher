...
* xref:patterns/unique-relationship-patterns.adoc[]: information about Cypher's default behavior of not re-traversing relationships in graph pattern matches and the `DIFFERENT RELATIONSHIPS` keyword.
* xref:patterns/repeatable-relationship-and-node-patterns.adoc[]: information about how to enable re-traversal of relationships in graph pattern matches using `REPEATABLE ELEMENTS`. label:new[Introduced in Neo4j 2025.06] label:cypher[Cypher 25 only]
* xref:patterns/acyclic-patterns.adoc[]: information about `ACYCLIC` patterns. label:new[Introduced in Neo4j 2026.03] label:cypher[Cypher 25 only]
